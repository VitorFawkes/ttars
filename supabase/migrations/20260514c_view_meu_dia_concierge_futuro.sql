-- =====================================================================
-- 20260514c: v_meu_dia_concierge expoe tarefas.concierge_futuro_em
--
-- Base: 20260512_view_meu_dia_concierge_full_name.sql (versao mais recente).
-- Unica mudanca: adiciona t.concierge_futuro_em na lista de colunas.
--
-- Frontend usa esse campo em computeEstadoFunil pra decidir se o
-- atendimento esta na coluna "Agendados para o futuro" (sticky).
-- =====================================================================
...
CREATE VIEW public.v_meu_dia_concierge AS
 SELECT t.id AS tarefa_id,
    ...
    -- Card raiz: o card principal da viagem. Se a linha ja e do principal,
    -- root = proprio card. Se e de um sub-card, root = parent_card_id.
   ...
   FROM public.tarefas t
     ...
     -- Root card: ignoramos pais soft-deletados — atendimentos orfaos exibem
     -- root_* como NULL e o frontend cai no fallback para campos do proprio card.
