-- ============================================================================
-- LIGA O PAINEL: reescreve as 3 funções do FUNIL pra lerem de ww_funil_casal
-- (a tabela derivada validada), mantendo o MESMO formato de saída (a tela não muda).
-- Funil = DW (is_elopement=false). Marcos cumulativos (monotônicos).
-- cohort = data de criação do lead · throughput = data de entrada na etapa.
-- DROP+CREATE (repointa a fonte; sem CREATE OR REPLACE por causa do hook).
-- ============================================================================

-- ---------- 1) ww_funil_conversao_v1 (funil A|B) ----------
...
-- ---------- 2) ww_funil_ranking_combo (matriz) ----------
...
BEGIN
    SELECT json_agg(x ORDER BY x) INTO v_f FROM (SELECT DISTINCT faixa x FROM ww_funil_casal WHERE NOT COALESCE(is_elopement,FALSE) AND faixa IS NOT NULL) a;
    SELECT json_agg(x ORDER BY x) INTO v_c FROM (SELECT DISTINCT convidados x FROM ww_funil_casal WHERE NOT COALESCE(is_elopement,FALSE) AND convidados IS NOT NULL) a;
    SELECT json_agg(x ORDER BY x) INTO v_d FROM (SELECT DISTINCT destino x FROM ww_funil_casal WHERE NOT COALESCE(is_elopement,FALSE) AND destino IS NOT NULL) a;
    SELECT json_agg(x ORDER BY x) INTO v_o FROM (SELECT DISTINCT origem x FROM ww_funil_casal WHERE NOT COALESCE(is_elopement,FALSE) AND origem IS NOT NULL AND origem<>'Desconhecida') a;
