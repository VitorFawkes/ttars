-- ============================================================
-- Reativação v3 — Novas dimensões de inteligência
-- Aniversário, família, última interação, indicações, presentes
-- ============================================================

-- 1. Novas colunas na tabela
ALTER TABLE reactivation_patterns
  ADD COLUMN IF NOT EXISTS birthday_date DATE,
  ADD COLUMN IF NOT EXISTS days_until_birthday INT,
  ...
  ADD COLUMN IF NOT EXISTS days_since_interaction INT,
  ADD COLUMN IF NOT EXISTS referral_count INT DEFAULT 0,
  ADD COLUMN IF NOT EXISTS is_referrer BOOLEAN DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS last_gift_date DATE,
  ADD COLUMN IF NOT EXISTS gifts_sent_count INT DEFAULT 0;
...
BEGIN
  ...
  FOR rec IN
    ...
    -- NOVO: Acompanhantes frequentes (família)
    companions AS (
      SELECT
        wt.contact_id,
    ...
    -- NOVO: Última interação (WhatsApp, atividades, tarefas)
    last_interaction AS (
      SELECT
        contact_id,
        interaction_date,
      ...
      FROM (
        ...
        SELECT
          co.id AS contact_id,
          co.last_whatsapp_sync::DATE AS interaction_date,
          'whatsapp' AS interaction_type
        FROM contatos co
        WHERE co.org_id = v_org
          AND co.last_whatsapp_sync IS NOT NULL
          AND co.deleted_at IS NULL

        UNION ALL

        -- Tarefas concluídas vinculadas ao contato via card
        SELECT
          c.pessoa_principal_id AS contact_id,
          t.concluida_em::DATE AS interaction_date,
          'tarefa' AS interaction_type
        FROM tarefas t
        ...
        -- Atividades no card do contato
        SELECT
          c.pessoa_principal_id AS contact_id,
          a.created_at::DATE AS interaction_date,
          'atividade' AS interaction_type
        FROM activities a
    ...
    last_interaction_per_contact AS (
      SELECT DISTINCT ON (contact_id)
        contact_id, interaction_date, interaction_type
      FROM last_interaction
      ORDER BY contact_id, interaction_date DESC
    ),
    -- NOVO: Indicações feitas
    referrals AS (
      SELECT
        c.indicado_por_id AS contact_id,
        COUNT(*) AS referral_count
      FROM cards c
      WHERE c.org_id = v_org
        AND c.indicado_por_id IS NOT NULL
      GROUP BY c.indicado_por_id
    ),
    -- NOVO: Presentes enviados
    gifts AS (
      SELECT
        g.contato_id AS contact_id,
      ...
      WHERE g.contato_id IS NOT NULL
        AND g.status NOT IN ('cancelado')
    ...
    contact_agg AS (
      ...
      FROM (
        SELECT wt2.*,
          CASE WHEN LAG(wt2.data_viagem_inicio) OVER (
            PARTITION BY wt2.contact_id ORDER BY wt2.data_viagem_inicio
          ) IS NOT NULL
          THEN (wt2.data_viagem_inicio - LAG(wt2.data_viagem_inicio) OVER (
            PARTITION BY wt2.contact_id ORDER BY wt2.data_viagem_inicio
          )) END AS gap_days
        FROM won_trips wt2
      ) wt
      WHERE wt.contact_id NOT IN (SELECT contact_id FROM active_contacts)
    ...
    seasonality AS (
      ...
      FROM (
        SELECT ca.contact_id, m AS month_num,
          COUNT(*)::NUMERIC / ca.total_trips AS pct_of_trips
      ...
      FROM (
        ...
        FROM won_trips wt,
          LATERAL jsonb_array_elements_text(
            CASE WHEN wt.produto_data IS NOT NULL
              AND wt.produto_data ? 'destinos'
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Aniversário: próximo aniversário
      IF rec.data_nascimento IS NOT NULL THEN
        ...
        EXCEPTION WHEN OTHERS THEN
          v_birthday := NULL;
          v_days_until_bday := NULL;
        END;
      END IF;

      -- Dias desde última interaç��o
      ...
      -- ============================================================
      -- SCORING (0-100)
      -- freq(0-20) + recency(0-20) + value(0-20) + season(0-10)
      -- + timing(0-5) + interest(0-10) + engagement(0-15)
      -- ============================================================
      ...
      v_recency_score := CASE
        WHEN v_days_since <= 120 THEN 20
        WHEN v_days_since <= 240 THEN 15
        WHEN v_days_since <= 365 THEN 10
        WHEN v_days_since <= 730 THEN 4
        ELSE 0
      END;
      ...
      -- Seasonality (0-10)
      IF ARRAY_LENGTH(rec.peak_months, 1) >= 2 AND rec.peak_confidence >= 0.30 THEN
        v_season_score := 10;
      ELSIF ARRAY_LENGTH(rec.peak_months, 1) = 1 AND rec.peak_confidence >= 0.25 THEN
        v_season_score := 5;
      END IF;

      -- Timing (0-5)
      IF v_ideal_contact IS NOT NULL THEN
        IF v_ideal_contact BETWEEN CURRENT_DATE - 30 AND CURRENT_DATE + 30 THEN
          v_timing_bonus := 5;
        ELSIF v_ideal_contact BETWEEN CURRENT_DATE - 90 AND CURRENT_DATE - 31 THEN
          v_timing_bonus := 3;
        END IF;
      END IF;

      -- Interest: deal perdido recente (0-10)
      IF rec.lost_count > 0 AND rec.last_lost_date >= CURRENT_DATE - INTERVAL '6 months' THEN
        v_interest_bonus := 10;
      ELSIF rec.lost_count > 0 AND rec.last_lost_date >= CURRENT_DATE - INTERVAL '12 months' THEN
        v_interest_bonus := 5;
      END IF;

      -- ============================================================
      -- ENGAGEMENT SCORE (0-15) — NOVO
      -- Combina: interação recente, indicações, aniversário próximo
      -- ============================================================
      DECLARE
        v_engagement INT := 0;
      BEGIN
        -- Interação recente = cliente ainda engajado (0-5)
        IF v_days_since_interaction IS NOT NULL THEN
          ...
          ELSIF v_days_since_interaction <= 180 THEN v_engagement := v_engagement + 1;
          END IF;
        END IF;

        -- Indicou alguém = cliente fiel (0-5)
        IF rec.referral_count >= 3 THEN v_engagement := v_engagement + 5;
        ELSIF rec.referral_count >= 1 THEN v_engagement := v_engagement + 3;
        END IF;

        -- Aniversário próximo = oportunidade de presente/contato (0-5)
        IF v_days_until_bday IS NOT NULL THEN
          IF v_days_until_bday BETWEEN 0 AND 30 THEN v_engagement := v_engagement + 5;
          ELSIF v_days_until_bday BETWEEN 31 AND 60 THEN v_engagement := v_engagement + 3;
          END IF;
        END IF;
        ...
        v_score := LEAST(
          v_freq_score + v_recency_score + v_value_score + v_season_score
          + v_timing_bonus + v_interest_bonus + v_engagement,
          100
        );
  ...
  -- Limpar contatos que não qualificam ou têm deals ativos
