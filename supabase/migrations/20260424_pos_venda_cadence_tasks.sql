-- ============================================================================
-- MIGRATION: Importação Pós-Venda — criar tarefas da cadência padrão
-- Date: 2026-04-24
--
-- CONTEXTO
-- A versão anterior da RPC bulk_create_pos_venda_cards criava uma tarefa
-- "App Enviado para o Cliente" que não pertence à cadência padrão do estágio
-- App & Conteúdo. Isso causava ruído: o usuário via uma tarefa extra que não
-- batia com o fluxo oficial (Criar App → Conferir Vouchers → Adicionar
-- vouchers no App → Liberar App).
--
-- Esta migration reescreve a RPC para:
--
-- 1. Na CRIAÇÃO do card: em vez de criar "App Enviado para o Cliente",
--    criar as 4 tarefas padrão da cadência "Pós-venda: App & Conteúdo",
--    respeitando:
--      - os nomes exatos da cadência
--      - o responsável configurado em cada step (task_config.assign_to_user_id)
--      - a prioridade do step
--    As tarefas já nascem marcadas como CONCLUÍDAS quando o CSV indica:
--      - Criar App               → concluída se app_enviado_concluida = true
--      - Conferir Vouchers       → concluída se todos os produtos têm voucher
--      - Adicionar vouchers no App → concluída se todos os produtos têm voucher
--      - Liberar App             → concluída se (app enviado) E (todos voucher)
--
-- 2. Na ATUALIZAÇÃO de card existente:
--      - NÃO cria mais "App Enviado para o Cliente" (legado)
--      - Se existir tarefa antiga "App Enviado para o Cliente" pendente e
--        o CSV indicar app enviado, marca como concluída (backward compat)
--      - Atualiza as 4 tarefas padrão da cadência, marcando como concluídas
--        quando o CSV indica (não desmarca nunca)
--      - Atualiza também o título do card para o padrão novo gerado pelo frontend
--
-- Não dispara cadence_instance — as tarefas são criadas diretamente. Se no
-- futuro o card transicionar de etapa e o trigger stage_enter disparar a
-- cadência, o comportamento atual do sistema segue. Essa decisão evita o
-- risco de duplicar tarefas agora.
-- ============================================================================
...
DECLARE
    ...
    -- Vars para criação das tarefas da cadência
    v_card_org_id UUID;
...
BEGIN
    ...
    LOOP
        ...
        -- Calcular flags de prontidão com base no payload
        ...
        IF v_existing_card_id IS NOT NULL THEN
            ...
            -- Backward compat: tarefa legada "App Enviado para o Cliente".
            -- NÃO cria nova, mas se existir pendente e o CSV indicar feito, marca concluída.
            IF v_has_app THEN
            ...
            -- Marcar tarefas da cadência padrão como concluídas conforme CSV indica.
            -- Nunca desmarca; só completa quem estava pendente.
            IF v_has_app THEN
                UPDATE tarefas SET concluida = true, concluida_em = COALESCE(concluida_em, NOW())
        ...
        ELSE
            ...
            -- ─── Tarefas da cadência "Pós-venda: App & Conteúdo" ──────
            -- Em vez de criar "App Enviado para o Cliente" (fora do padrão),
            -- cria as 4 tarefas da cadência com os mesmos nomes/assignees
            -- e marca como concluídas conforme o CSV indica.
            ...
            IF v_cadence_template_id IS NOT NULL THEN
                ...
                LOOP
                    ...
                    v_step_priority := COALESCE(NULLIF(v_step.task_config->>'prioridade', ''), 'medium');

                    -- Regra de conclusão por nome da tarefa
