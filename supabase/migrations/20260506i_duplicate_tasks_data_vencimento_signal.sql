-- ============================================================================
-- MIGRATION: Adicionar sinal "mesmo vencimento" a find_duplicate_tasks_tiered
-- Date: 2026-05-06
--
-- PROBLEMA:
-- A RPC bucketiza por (card_id, tipo, titulo_norm), mas só promove para
-- tier_exact se houver same_cadence_step OU within_5min OU same_external_id.
-- Tarefas com mesmo título + mesmo vencimento + mesmo card, mas criadas
-- longe no tempo e sem cadence_step, caem em tier_possible — perdem
-- visibilidade. Para o usuário, mesmo título + mesmo vencimento é
-- duplicata óbvia.
--
-- FIX:
-- Adicionar sinal same_data_vencimento (granularidade segundo) ao conjunto
-- de gatilhos do tier_exact. Mantém todo o resto da função intacta.
-- ============================================================================
...
BEGIN
    ...
    bucket_signals AS (
        SELECT
            ...
            (
                SELECT COUNT(DISTINCT date_trunc('second', v)) = 1
                       AND BOOL_AND(v IS NOT NULL)
                FROM unnest(b.vencs) AS v
            ) AS same_data_vencimento
        FROM bucket_dup b
    ),
    tier_exact AS (
        SELECT
            ...
            CASE
                ...
                WHEN bs.same_data_vencimento THEN 'Mesmo título e vencimento'
        ...
        WHERE (bs.same_cadence_step OR bs.within_5min OR bs.same_external_id OR bs.same_data_vencimento)
    ...
    tier_possible AS (
        SELECT
            ...
            bs.has_reschedule_chain
        FROM bucket_signals bs
        WHERE NOT (bs.same_cadence_step OR bs.within_5min OR bs.same_external_id OR bs.same_data_vencimento)
          AND NOT bs.has_reschedule_chain
    ),
    tier_chain AS (
        SELECT
