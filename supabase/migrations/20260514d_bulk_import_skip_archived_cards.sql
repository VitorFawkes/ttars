-- ============================================================================
-- MIGRATION: bulk_import_financial_items v8 — pular cards arquivados
-- Date: 2026-05-14
--
-- Base: v7 (20260511b_bulk_import_financial_items_cancelamento.sql).
-- Releitura confirmada de v4 (20260414), v5 (20260507h), v6 (20260507j), v7 (20260511b).
-- Todas as correções incrementais foram preservadas.
--
-- Mudança única vs v7:
--   No início de cada iteração de card, validar que o card-alvo não está
--   arquivado (archived_at IS NULL). Se estiver, pular silenciosamente e
--   somar a um contador de cards_skipped.
--
-- Motivo:
--   Cards arquivados representam negócios encerrados (perdidos do ponto de
--   vista comercial). Quando o frontend faz match por numero_venda_monde,
--   colisões entre card ativo + card arquivado (mesmo número) já são
--   filtradas no SELECT do VendasMondePage. Esta validação no RPC é
--   defesa em profundidade: garante que qualquer chamador futuro (RPC
--   manual, script, fila) também respeite a regra.
--
-- DROP + CREATE em vez de CREATE OR REPLACE porque a função não tem grants
-- externos custom no projeto (verificado via grep) e v7 já usou esse padrão.
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- ─── Backstop: pular cards arquivados ───
    -- Regra de negócio: cards arquivados são tratados como inexistentes para
    -- fins de import Monde. Se o card-alvo está arquivado, pula sem tocar
    -- em nada (não importa produtos, não recalcula totais, não loga erro).
