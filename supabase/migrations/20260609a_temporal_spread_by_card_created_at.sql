-- Feature: espalhar disparos temporais pelo HORÁRIO DE CRIAÇÃO do card.
-- Antes: fn_enqueue_temporal_events enfileirava tudo com execute_at=NOW() →
-- todos os matches do dia disparavam juntos às 9h (rajada → risco de ban Meta).
-- Agora: execute_at = HOJE (SP) na MESMA hora:min em que o card foi criado
-- (c.created_at em America/Sao_Paulo), SEM travar horário. Os cards entram em
-- horários variados → os envios se espalham pelo dia. O dispatcher
-- (process-cadence-engine, a cada 2 min) respeita execute_at e manda na hora.
--
-- Cron movido de 9h SP (0 12 UTC) para 5h SP (0 8 UTC): a avaliação roda cedo
-- pra que cards criados de manhã sejam agendados no horário deles (e não caiam
-- todos às 9h). Cards criados antes das 5h disparam ~5h.
--
-- Mantém: UTC::date nas datas de viagem (20260608d), COALESCE offset_days/
-- days_offset (20260608b), cc.ordem=1 (20260608a).
...
-- Reagenda a avaliação diária: 9h SP -> 5h SP (0 8 UTC)
DO $remove$
BEGIN
...
SELECT cron.schedule(
  ...
  '0 8 * * *',
  $cron$SELECT public.fn_enqueue_temporal_events()$cron$
);
