-- ============================================================================
-- MIGRATION: bulk_import_financial_items v12 — fix org_id em passageiros
-- Date: 2026-05-19
--
-- Bug encontrado durante backfill v11:
--   INSERT em financial_item_passengers não passa org_id. A coluna tem default
--   `requesting_org_id()` que funciona quando há JWT (uploads via VendasMondePage),
--   mas retorna NULL/fallback quando chamada via Management API ou de SECURITY
--   DEFINER sem contexto. Resultado: backfill falha com 23502 (null org_id).
--
-- Fix v12:
--   Passar org_id explicitamente, derivando do card_id correspondente.
--
-- Lógica de match/diff/cleanup idêntica à v11 (que corrigiu o bug FOUND).
-- ============================================================================
...
COMMENT ON FUNCTION bulk_import_financial_items(JSONB) IS
  'v12 (2026-05-19): passa org_id explicitamente em financial_item_passengers (derivado do card). Necessário pra backfill via Management API funcionar (requesting_org_id() retorna NULL sem JWT).';
