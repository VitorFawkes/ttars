-- ============================================================================
-- MIGRATION: bulk_import_financial_items v9 — preservar is_ready no cleanup
-- Date: 2026-05-15
--
-- Base: v8 (20260514d_bulk_import_skip_archived_cards.sql).
-- Releitura confirmada de v4 (20260414), v5 (20260507h), v6 (20260507j),
-- v7 (20260511b), v8 (20260514d). Todas as correções preservadas.
--
-- Mudança única vs v8 (no ramo "cleanup órfãos"):
--   Items com is_ready=true (operação conferiu) NÃO podem ser arquivados
--   pelo cleanup automático. Regra do Vitor:
--   "NUNCA desmarcar/arquivar produto que a operação já confirmou."
--
-- Linha alterada:
--   ANTES: AND archived_at IS NULL
--   DEPOIS: AND archived_at IS NULL AND is_ready = FALSE
--
-- Caso o usuário PRECISE arquivar um item is_ready=true (ex: produto realmente
-- cancelado no Monde), o caminho oficial é via data_cancelamento (Ramo
-- CANCELAMENTO já existe e funciona independente do is_ready) ou via UI.
--
-- Backfill: restaura items arquivados em 14/05 com is_ready=true (241 items)
-- e arquiva possíveis duplicatas ativas recém-criadas que coincidam.
--
-- DROP + CREATE: v8 já usou esse padrão, sem grants externos custom.
-- Releitura cega bloqueada por warn-function-rebase — feita acima.
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- ────────────────────────────────────────────────────────────────────
    -- CLEANUP — MUDANÇA v9: arquivar apenas items NÃO conferidos
    -- Items com is_ready=true (operação confirmou) jamais são arquivados aqui.
    -- Regra do Vitor: "NUNCA desmarcar produto que a operação já confirmou."
    -- ────────────────────────────────────────────────────────────────────
...
-- ============================================================================
-- BACKFILL 1: Restaurar items 'monde_reupload_removed' com is_ready=true
-- (operação tinha conferido — jamais deveriam ter sido arquivados)
-- ============================================================================
...
WITH restored AS (
  UPDATE card_financial_items
     SET archived_at = NULL,
         archived_reason = NULL,
         updated_at = NOW()
   WHERE archived_reason = 'monde_reupload_removed'
     AND is_ready = TRUE
...
-- ============================================================================
-- BACKFILL 2: Arquivar duplicatas ATIVAS recém-criadas que coincidem com items
-- restaurados (mesma description+fornecedor+sale_value+datas, mesmo card,
-- vendas diferentes). Mantém o item conferido pela operação.
-- ============================================================================

WITH duplicatas AS (
  SELECT DISTINCT a.id
  FROM card_financial_items a
  JOIN card_financial_items b
    ON b.card_id = a.card_id
    AND b.id != a.id
    AND b.archived_at IS NULL
    AND b.is_ready = TRUE
    AND LOWER(COALESCE(b.description,'')) = LOWER(COALESCE(a.description,''))
    AND LOWER(COALESCE(b.fornecedor,''))  = LOWER(COALESCE(a.fornecedor,''))
    AND COALESCE(b.sale_value, 0)         = COALESCE(a.sale_value, 0)
    AND COALESCE(b.data_inicio, DATE '1900-01-01') = COALESCE(a.data_inicio, DATE '1900-01-01')
    AND COALESCE(b.data_fim,    DATE '1900-01-01') = COALESCE(a.data_fim,    DATE '1900-01-01')
  WHERE a.archived_at IS NULL
    AND a.is_ready = FALSE
    AND a.created_at >= '2026-05-14'
    AND a.monde_venda_num IS DISTINCT FROM b.monde_venda_num
),
arquivadas AS (
  UPDATE card_financial_items
     SET archived_at = NOW(),
         archived_reason = 'monde_duplicata_pos_restauracao_20260515',
         updated_at = NOW()
   WHERE id IN (SELECT id FROM duplicatas)
  RETURNING card_id
),
cards_dedup AS (
  SELECT DISTINCT card_id FROM arquivadas
)
UPDATE cards c
   SET valor_final = COALESCE((
         SELECT SUM(sale_value) FROM card_financial_items
       ...
       updated_at = NOW()
 WHERE c.id IN (SELECT card_id FROM cards_dedup);
