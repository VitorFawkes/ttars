-- ============================================================
-- Fix: "numeros_venda_monde_historico alterado [object Object]"
-- Problema: campo sem label legível no trigger genérico +
--           valor JSONB array renderizado como [object Object]
-- Solução: adicionar ao mapa de campos conhecidos com descrição
--          legível e armazenar valor textual no metadata
-- ============================================================
...
DECLARE
    ...
    -- Keys to SKIP from generic logging (technical/internal fields)
...
BEGIN
    ...
    IF v_old_data IS DISTINCT FROM v_new_data THEN
        ...
        IF v_all_keys IS NOT NULL THEN
            FOREACH v_key IN ARRAY v_all_keys LOOP
                -- Pular campos técnicos/internos
    ...
    -- 5b. Log dedicado para numero_venda_monde (campo principal, não o histórico)
    -- O histórico muda junto mas é ruído — logamos só o número principal
    IF (v_old_data->>'numero_venda_monde') IS DISTINCT FROM (v_new_data->>'numero_venda_monde') THEN
        -- Já foi logado pelo loop genérico acima (numero_venda_monde tem label em system_fields)
