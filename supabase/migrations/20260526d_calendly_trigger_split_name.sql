-- Fix: tabela contatos exige sobrenome (trigger de validação).
-- Splita invitee_name pelo primeiro espaço; se não houver espaço,
-- usa o nome inteiro também como sobrenome (placeholder válido).
  ...
  LOOP
    ...
    ELSIF coalesce((v_trigger.event_config->>'create_card_if_missing')::BOOLEAN, FALSE) THEN
      ...
      IF v_resolved_contato_id IS NULL THEN
        -- Split nome em nome/sobrenome (split pelo primeiro espaço)
        ...
        ELSE
          ...
          v_last_name  := v_full_name;  -- placeholder, satisfaz constraint
        END IF;
