-- 20260616j_ww_lead_ideal_periodo_robusto.sql
--
-- Lead Ideal × Pipeline (ww_v2_lead_ideal): blindagem do filtro de PERÍODO nas vendas e nos
-- leads do período (a garantia que o Vitor pediu). Duas correções que NÃO mudam nenhum número
-- atual (0 casais ganho com ganho_at nulo; frontend sempre passa datas), mas tornam a regra
-- robusta em vez de incidental:
--
--  1) Fallback da janela histórica: sem datas explícitas, usa p_historico_meses (default 12m)
--     em vez de 1970→agora (que ignorava o período). O param p_historico_meses estava morto.
--  2) "Vendas" (ganho) conta ESTRITAMENTE com ganho_at dentro da janela — remove o
--     'ganho_at IS NULL OR' que deixava uma venda sem data contar fora de qualquer período.
--
-- Tipo (DW/Elopement) e período já eram aplicados nos 3 universos (vendas/lead atual/leads do
-- período) e em todas as saídas (comparações, cruzamento, top perfis) — auditado e testado ao vivo.
