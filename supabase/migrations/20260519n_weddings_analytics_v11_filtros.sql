-- v11 — filtros granulares e segmentos rapidos
--
-- REBASE CONFIRMADO. Base: v10 (20260519m). Mesma lógica de estado, KPIs,
-- breakdowns e meeting_state.
--
-- A v11 ADICIONA params (compat — todos default NULL):
--   - p_inbound_min INT, p_inbound_max INT: filtro de profundidade (qtd inbound)
--   - p_meeting_states TEXT[]: 'meeting_scheduled', 'meeting_done', 'none'
--                              ('none' = sem reunião ainda no funil)
--   - p_stage_phases TEXT[]: 'sdr', 'closer', 'pos_venda', 'resolucao', 'none'
--   - p_stage_names TEXT[]: nome exato do stage (Reunião Agendada, etc)
--
-- Os campos retornados PERMANECEM idênticos à v10.
...
BEGIN
  ...
  -- ★ v11: catálogo de stages disponíveis (pra UI montar dropdown)
  stages_catalog AS (
    SELECT json_agg(jsonb_build_object(
...
COMMENT ON FUNCTION analytics_weddings_conversations IS
  'v11 (rebase confirmado da v10 - 20260519m): adiciona filtros p_inbound_min/max, p_meeting_states, p_stage_phases, p_stage_names + catalogo stages_catalog. Preserva tudo da v10.';
