-- 20260616i_ww_onde_estao_snapshot_e_closer_etapas.sql
--
-- "Onde estão agora" — 2 correções (pedido do Vitor, print):
--  1) SNAPSHOT: deixa de filtrar por período. É o estado ATUAL dos casais em aberto,
--     independente do recorte de tempo da tela (dimensões origem/faixa/tipo seguem valendo).
--  2) Classifica pela ETAPA ATUAL (cur_stage da cache), NÃO pelo histórico de "fez closer".
--     - Casal cuja etapa atual está FORA de SDR/Closer (Convidados, Planejamento, Elopment,
--       Produção…) vira 'OUT' e NÃO aparece (corrige o balde "Em contato" que misturava
--       casais de outros pipelines).
--     - Closer mantém 4 linhas (1ª Reunião + Reagendamento entram em "Em contato"; Standby-Closer
--       fica com "Aguardando dados") + NOVA linha "Oportunidade futura" (etapa 221), que estava
--       sendo rotulada errado como "Em contato".
--
