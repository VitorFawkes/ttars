-- =====================================================================
-- ROLLBACK de 20260507a_view_meu_dia_root_card.sql
--
-- Restaura a view v_meu_dia_concierge ao estado anterior (definição de
-- 20260506a_view_meu_dia_includes_contato_principal.sql), removendo as
-- colunas root_*, is_from_sub_card e os JOINs com o card pai.
--
-- Use este rollback APENAS se a migration 20260507a estiver causando
-- regressão em produção. Após rodar:
--   1) o card detail do principal volta a NÃO listar atendimentos de
--      sub-cards;
--   2) o kanban /concierge volta a tratar sub-card como "viagem" separada;
--   3) o frontend usa fallback (item.card_titulo, item.pessoa_principal_nome
--      etc.) — nada quebra mesmo sem as colunas root_*.
--
-- Aplicar com: bash .claude/hooks/promote-to-prod.sh supabase/migrations/_rollback/20260507a_rollback_view_meu_dia_root_card.sql
-- =====================================================================
