-- 20260615b — ww_funil_casal: ganho passa a usar o CAMPO "Data-Hora Ganho" (AC 87)
-- a partir de jan/2026, SEM mexer no passado.
--
-- CONTEXTO (achado pelo Vitor): a Visão Geral mostrava 6 ganhos em maio/2026, mas o
-- campo "Data-Hora Ganho" (AC field 87) tinha 5. A divergência é porque hoje o "ganho"
-- de ww_funil_casal é a ENTRADA NO PLANEJAMENTO (ganho_fonte='andamento'), não o campo.
-- Caso real: DW | Álvaro (Maria e Valdir) — campo 87 = 27/04/2026, mas entrou no
-- Planejamento em 05/05/2026 → contava em maio pela régua antiga.
--
-- DECISÃO (Vitor, 2026-06-15):
--   1) A régua do corte é a DATA DO PRÓPRIO CAMPO 87. Se campo87 >= 2026-01-01 → usa o campo.
--   2) Pós-corte: SÓ é ganho se o campo estiver preenchido (fonte única de verdade). Casal que
--      entrou no Planejamento em 2026 sem campo 87 NÃO conta como ganho até preencherem (fica
--      aberto, não perdido).
--   3) Passado preservado: se campo87 é NULL ou < 2026-01-01, mantém a lógica antiga
--      (ganho = entrou no Planejamento / assinatura elopement, na data do evento).
--
-- IMPACTO MEDIDO (172 ganhos atuais, snapshot 2026-06-15):
--   - 2025 e antes: 0 mudanças (régua exige campo >= jan/2026).
--   - 2026: Álvaro muda mai→abr; "Karen" (planej fev/2026 sem campo) sai dos ganhos;
--     1 casal de fev migra de mês pelo campo. Total 2026: 29 → 27.
--
-- O QUE MUDA NESTA MIGRATION (3 pontos cirúrgicos sobre a definição VIVA de produção,
-- capturada via pg_get_functiondef em 2026-06-15 — TOP-5 #5 guard de rebase):
--   a) CTE cf: agrega min(ganho_at) do cache = campo 87 do casal (campo87_at).
--   b) CTE j: projeta cf.campo87_at adiante.
--   c) SELECT final: ganho / ganho_at / ganho_fonte viram condicionais por data.
-- Todo o resto (evidência WW, ev, dims, fez_sdr/closer régua 24h, is_perdido) é idêntico.
...
BEGIN
    ...
    WITH ww_contacts AS (
        ...
        SELECT
            ...
            -- 20260612c: 'DW' deixou de engolir os não classificados óbvios.
            -- Prioridade: campo 30 explícito > jornada na esteira Elopment >
            -- declarou "Apenas o casal" (94% dos que fecharam viraram Elopement:
            -- 15/16; o único DW tinha campo 30 = DW e fica DW) > default DW.
            CASE WHEN cf.campo30 = 'Elopment Wedding'  THEN 'Elopement'
                 WHEN cf.campo30 = 'DW com convidados' THEN 'DW'
                 WHEN COALESCE(ev.is_elo, cf.is_elo_pipe, FALSE) THEN 'Elopement'
                 WHEN _ww2_norm_conv_strict(cf.convidados_raw) = 'Apenas o casal' THEN 'Elopement'
                 ELSE 'DW' END AS tipo_final,
...
COMMENT ON FUNCTION public.refresh_ww_funil_casal() IS
  'Reconstrói ww_funil_casal (org Weddings) a partir de ww_ac_deal_funnel_cache + ww_deal_event. '
  'v6 (20260615b): ganho usa o campo "Data-Hora Ganho" (AC 87) quando campo87_at >= 2026-01-01 '
  '(fonte única; sem campo no período = não-ganho); antes disso mantém a régua de entrada no '
  'Planejamento (ganho_fonte=andamento). Passado <2026 inalterado.';
