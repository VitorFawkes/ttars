-- ============================================================================
-- ww_perfil_lead_ideal v2 — 2 períodos independentes + cruzamentos + top combos
--
-- Preserva v1 (20260527y): comparações por dimensão isolada (faixa/conv/dest)
-- com lift e delta_pp.
--
-- NOVO v2:
--   - Aceita p_historico_start e p_historico_end (janelas independentes da
--     "atual"). Se NULL, cai no comportamento antigo via p_historico_meses.
--   - Cruzamentos 2D: faixa×convidados, faixa×destino, convidados×destino
--     com qtd e pct em ambos os universos por célula.
--   - top_perfis_historico e top_perfis_atual: top 10 combos 3D (faixa,
--     destino, convidados) em cada universo.
-- ============================================================================

DROP FUNCTION IF EXISTS public.ww_perfil_lead_ideal(TIMESTAMPTZ, TIMESTAMPTZ, UUID, INT, INT);

CREATE FUNCTION public.ww_perfil_lead_ideal(
...
BEGIN
    ...
    ELSIF COALESCE(p_historico_meses, 12) = 0 THEN
      v_hist_start := '1970-01-01'::timestamptz;
      v_hist_end := NOW();
    ELSE
      v_hist_start := NOW() - (p_historico_meses || ' months')::interval;
      v_hist_end := NOW();
    END IF;
    ...
    SELECT _ww2_norm_faixa_strict(c.produto_data->>'ww_mkt_orcamento_form') AS faixa,
     ...
     WHERE c.deleted_at IS NULL AND c.archived_at IS NULL
       ...
       AND NULLIF(c.produto_data->>'ww_closer_data_ganho','')::TIMESTAMPTZ <= v_hist_end;
    ...
    -- Comparação por dimensão isolada (preservado v1)
    WITH dims AS (
    ...
    -- Cruzamentos 2D (NOVO v2)
    SELECT json_build_object(
      'faixa_x_convidados', (
        WITH h AS (SELECT faixa AS x, convidados AS y, COUNT(*) AS qtd FROM _ww_pli_h WHERE faixa IS NOT NULL AND convidados IS NOT NULL GROUP BY faixa, convidados),
             a AS (SELECT faixa AS x, convidados AS y, COUNT(*) AS qtd FROM _ww_pli_a WHERE faixa IS NOT NULL AND convidados IS NOT NULL GROUP BY faixa, convidados),
        ...
        FROM cells
        LEFT JOIN h ON h.x = cells.x AND h.y = cells.y
        LEFT JOIN a ON a.x = cells.x AND a.y = cells.y
      ),
      'faixa_x_destino', (
        WITH h AS (SELECT faixa AS x, destino AS y, COUNT(*) AS qtd FROM _ww_pli_h WHERE faixa IS NOT NULL AND destino IS NOT NULL GROUP BY faixa, destino),
             a AS (SELECT faixa AS x, destino AS y, COUNT(*) AS qtd FROM _ww_pli_a WHERE faixa IS NOT NULL AND destino IS NOT NULL GROUP BY faixa, destino),
             cells AS (SELECT DISTINCT x, y FROM (SELECT x, y FROM h UNION ALL SELECT x, y FROM a) z)
        SELECT COALESCE(json_agg(json_build_object(
          'x', cells.x, 'y', cells.y,
          'hist_qtd', COALESCE(h.qtd, 0),
      ...
      'convidados_x_destino', (
        WITH h AS (SELECT convidados AS x, destino AS y, COUNT(*) AS qtd FROM _ww_pli_h WHERE convidados IS NOT NULL AND destino IS NOT NULL GROUP BY convidados, destino),
             a AS (SELECT convidados AS x, destino AS y, COUNT(*) AS qtd FROM _ww_pli_a WHERE convidados IS NOT NULL AND destino IS NOT NULL GROUP BY convidados, destino),
             cells AS (SELECT DISTINCT x, y FROM (SELECT x, y FROM h UNION ALL SELECT x, y FROM a) z)
        SELECT COALESCE(json_agg(json_build_object(
          'x', cells.x, 'y', cells.y,
          'hist_qtd', COALESCE(h.qtd, 0),
    ...
    FROM (
      SELECT faixa, destino, convidados, COUNT(*) AS qtd
        FROM _ww_pli_h WHERE faixa IS NOT NULL AND destino IS NOT NULL AND convidados IS NOT NULL
    ...
    FROM (
      SELECT faixa, destino, convidados, COUNT(*) AS qtd
        FROM _ww_pli_a WHERE faixa IS NOT NULL AND destino IS NOT NULL AND convidados IS NOT NULL
       GROUP BY faixa, destino, convidados
       HAVING COUNT(*) >= v_min
       ORDER BY COUNT(*) DESC LIMIT 10
    ) g;
...
GRANT EXECUTE ON FUNCTION public.ww_perfil_lead_ideal(TIMESTAMPTZ, TIMESTAMPTZ, UUID, TIMESTAMPTZ, TIMESTAMPTZ, INT, INT) TO authenticated;
