-- ============================================================================
-- Corrige reset_agent_conversations_with_phone pra também limpar
-- cards.produto_data e cards.ai_pause_config.
--
-- VERIFICAÇÃO DAS 3 MIGRATIONS ANTERIORES (preservadas integralmente):
--   - 20260423c: versão inicial do RPC (escopo conversa + memória)
--   - 20260423d: expandiu pra full reset (whatsapp_messages + ai_resumo/ai_contexto
--                + contatos.nome='Cliente' + contatos.email/cpf/data_nasc NULL)
--   - 20260423e: corrigiu nome='Cliente' → nome='' (Estela respondia "Oi, Cliente!")
--                + passaporte NULL + ai_outbound_queue check coluna contato_id
--                + ai_conversations filter por current_agent_id OR primary_agent_id
--                + phone match com/sem DDI 55
--
-- DELTA desta migration vs 423e:
--   UPDATE cards SET ai_resumo=NULL, ai_contexto=NULL, updated_at=NOW()
--   → adiciona produto_data='{}'::jsonb, ai_pause_config=NULL
--
-- Por que: bug descoberto em 2026-05-22 durante auditoria de testes
-- incansáveis. produto_data (ww_orcamento_faixa, ww_num_convidados, etc.)
-- e ai_pause_config (handoff invisível permanent=true) persistiam entre
-- testes, contaminando lead novos e travando handoff permanente.
-- ============================================================================
...
BEGIN
  ...
  IF v_contact_ids IS NOT NULL AND array_length(v_contact_ids, 1) > 0 THEN
    ...
    -- Cards: limpa memória da IA + form_data + pause_config.
    -- ATUALIZADO 2026-05-22 — agora limpa produto_data e ai_pause_config.
    UPDATE cards
       SET ai_resumo = NULL,
           ai_contexto = NULL,
           produto_data = '{}'::jsonb,
           ai_pause_config = NULL,
           updated_at = NOW()
