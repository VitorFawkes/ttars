-- v7 — daily_timeline COMPLETO: adiciona no_reply, reply_rate, frt_median por dia
--
-- REBASE CONFIRMADO. Releitura das migrations anteriores:
--   - 20260519c (v1): SUPERSEDED.
--   - 20260519d (v2): assinatura TEXT[]. Lógica core.
--   - 20260519f (v3): breakdowns.
--   - 20260519g (v4): frt + heatmap + time_metrics.
--   - 20260519h (v5): daily_timeline + drill-down heatmap.
--   - 20260519i (v6): tudo por PESSOAS únicas (heatmap, timeline). ESTA é a base.
--
-- A v7 PRESERVA assinatura e lógica idênticas. ADICIONA campos no daily_timeline:
--   - no_reply (novo): pessoas contatadas no dia que NÃO responderam ainda
--                      (mesmo no dia ou subsequente). Quantifica silêncio diário.
--   - reply_rate_pct (novo): % das pessoas contatadas no dia que responderam
--   - frt_median_minutes (novo): mediana de FRT (em minutos) das pessoas contatadas
--                                 pela 1ª vez naquele dia. Mede velocidade de
--                                 resposta cohort-style.
--
-- Mantém: outbound, inbound, msgs_out, msgs_in, new_contacts, new_replies, wins
-- (todos os campos da v6 permanecem)
...
BEGIN
  ...
  daily_timeline AS (
    ...
    FROM (
      ...
      -- Pessoas contatadas no dia (DISTINCT phone)
      contacted_by_day AS (
        SELECT
          sent_at::DATE AS day,
          customer_phone,
          phone_line_label,
          MIN(sent_at) AS first_outbound_in_day
        FROM visible_msgs
        WHERE direction = 'outbound'
        GROUP BY 1, 2, 3
      ),
      -- Pessoas que responderam no dia
      replied_by_day AS (
        SELECT
          sent_at::DATE AS day,
          customer_phone,
      ...
      day_aggregates AS (
        SELECT
          ...
          -- Quem foi contatado no dia E também respondeu (em qualquer momento) ao mesmo phone_line
      ...
      -- FRT mediano por dia (cohort: pessoas cuja 1ª outbound foi naquele dia)
      frt_by_day AS (
        SELECT
          first_outbound_at::DATE AS day,
      ...
      SELECT
        ...
        COALESCE(mv.msgs_out, 0)                                              AS msgs_out,
        COALESCE(mv.msgs_in, 0)                                               AS msgs_in,
...
COMMENT ON FUNCTION analytics_weddings_conversations IS
  'v7 (rebase confirmado da v6 - 20260519i): daily_timeline ganha no_reply, reply_rate_pct, frt_median_minutes por dia.';
