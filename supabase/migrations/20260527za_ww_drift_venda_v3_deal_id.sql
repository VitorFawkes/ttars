-- ============================================================================
-- Analytics-Weddings — ww_drift_venda v3
--
-- Preserva tudo da v2 (20260527v_ww_drift_venda_v2.sql):
--   - vendas_lista enriquecida (contato_nome, contato_external_id, origem,
--     valor_final, consultor_nome, monde_venda)
--   - drift_por_consultor (mantém-se)
--   - drift_por_mes (mantém-se)
--   - investimento/destino/convidados matrizes (mantém-se)
--
-- NOVO v3:
--   - ac_deal_id em vendas_lista (de marketing_data->>'active_campaign_id')
--   - REMOVIDO drift_por_origem propositalmente (a pedido — análise de
--     marketing tem lugar próprio em Marketing.tsx, não em Entrada × Realidade).
-- ============================================================================

DROP FUNCTION IF EXISTS public.ww_drift_venda(TIMESTAMPTZ, TIMESTAMPTZ, UUID, TEXT[], TEXT, TEXT[]);

CREATE FUNCTION public.ww_drift_venda(
...
BEGIN
    ...
    -- INVESTIMENTO (preservado v2)
    WITH dados AS (SELECT faixa_e, fechou, CASE WHEN fechou THEN faixa_v END AS faixa_v FROM _ww_dv),
    cobertura AS (
    ...
    -- DESTINO (preservado v2)
    WITH dados AS (SELECT dest_e, CASE WHEN fechou THEN dest_v END AS dest_v, fechou FROM _ww_dv),
    cobertura AS (
        SELECT COUNT(*) AS total_leads, COUNT(*) FILTER (WHERE fechou) AS total_fechados,
    ...
    -- drift_por_consultor (preservado v2)
    SELECT COALESCE(json_agg(json_build_object(
      'consultor_id', consultor_id, 'consultor_nome', consultor_nome,
      'vendas', vendas, 'manteve', manteve, 'subiu', subiu, 'desceu', desceu,
    ...
    FROM (
      SELECT dono_atual_id AS consultor_id, consultor_nome,
        ...
        FROM _ww_dv WHERE dono_atual_id IS NOT NULL
       GROUP BY dono_atual_id, consultor_nome
       HAVING COUNT(*) FILTER (WHERE fechou) >= 1
    ) g;

    -- drift_por_mes (preservado v2)
    SELECT COALESCE(json_agg(json_build_object(
      'mes', mes, 'vendas', vendas,
      'manteve_pct', CASE WHEN vendas > 0 THEN ROUND(100.0 * manteve / vendas, 1) END,
      'subiu_pct',   CASE WHEN vendas > 0 THEN ROUND(100.0 * subiu / vendas, 1) END,
      'desceu_pct',  CASE WHEN vendas > 0 THEN ROUND(100.0 * desceu / vendas, 1) END
    ...
    FROM (
      SELECT TO_CHAR(date_trunc('month', data_venda), 'YYYY-MM') AS mes,
             ...
             COUNT(*) FILTER (WHERE faixa_e IS NOT NULL AND faixa_v IS NOT NULL AND _ww_faixa_ordem(faixa_v) > _ww_faixa_ordem(faixa_e)) AS subiu,
             COUNT(*) FILTER (WHERE faixa_e IS NOT NULL AND faixa_v IS NOT NULL AND _ww_faixa_ordem(faixa_v) < _ww_faixa_ordem(faixa_e)) AS desceu
        FROM _ww_dv WHERE fechou AND data_venda IS NOT NULL
       GROUP BY 1
    ) m;
    ...
    FROM (
        SELECT COALESCE(tipo_casamento, 'Não classificado') AS tipo,
          ...
          FROM _ww_dv WHERE fechou GROUP BY tipo_casamento
    ) x;

    -- Lista de vendas — NOVO v3: ac_deal_id
    SELECT json_agg(json_build_object(
