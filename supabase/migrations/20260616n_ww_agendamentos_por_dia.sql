-- Quantas reuniões foram MARCADAS por dia (quando o consultor agendou, não para quando a
-- reunião é). Fonte: ww_ac_deal_funnel_cache.sdr_agendado_em / closer_agendado_em
-- (= updatedTimestamp dos campos 6/18 do Active, preenchido pelo sync 20260616m).
-- Respeita os mesmos filtros das outras RPCs WW (tipo DW/Elopement, período, origem, faixa,
-- destino, convidados, consultor). Não aplica entrada_valida: um agendamento é um agendamento,
-- o recorte de produto é o filtro de tipo — mesma régua de ww_agenda_reunioes.
...
BEGIN
    -- Universo: deals WW com algum agendamento dentro do período (por dia DE AGENDAMENTO),
    ...
    -- Por dia (Brasília), SDR e Closer separados, contando cada agendamento na sua data.
    ...
    FROM (
        ...
        FROM (
            SELECT (sdr_agendado_em AT TIME ZONE 'America/Sao_Paulo')::DATE AS dia, 'sdr'::TEXT AS reuniao
              FROM _ww_agd WHERE sdr_agendado_em BETWEEN p_date_start AND p_date_end
            UNION ALL
            SELECT (closer_agendado_em AT TIME ZONE 'America/Sao_Paulo')::DATE, 'closer'
              FROM _ww_agd WHERE closer_agendado_em BETWEEN p_date_start AND p_date_end
        ) x
        GROUP BY x.dia
    ) d;

    SELECT COUNT(*) FILTER (WHERE sdr_agendado_em    BETWEEN p_date_start AND p_date_end),
...
-- Fecha o default Postgres (anon herda EXECUTE em funções do public). Só authenticated.
