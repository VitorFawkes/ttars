-- ============================================
-- Report Engine v3 — Comprehensive Fix
-- ============================================
-- Fixes:
-- 1. FUNNEL ORDERING: ps.nome ordena por posição do pipeline (pp.order_index, ps.ordem)
-- 2. COMPUTED MEASURES: ticket_medio e margem_pct filtram só status_comercial='ganho'
-- 3. CADENCIA: global date filter usa ci.started_at (não ci.created_at)
-- 4. GAP-FILL: LIMIT fora do CTE, não dentro
-- 5. SECURITY: SET search_path em SECURITY DEFINER
-- 6. STABILITY: helper functions marcadas como STABLE
-- ============================================
...
-- Whitelist of allowed field keys per source
...
BEGIN
    CASE p_source
        ...
        WHEN 'cadencia' THEN
            v_allowed := ARRAY['cdt.name', 'ci.status', 'ci.started_at', 'ci.id', 'ci.successful_contacts'];
        WHEN 'historico' THEN
...
-- FIX 2: Computed measures — ticket_medio e margem_pct filtram status_comercial='ganho'
    ...
    IF p_source = 'cards' THEN
        CASE p_key
            ...
            WHEN 'ticket_medio' THEN
                -- FIX: SUM apenas valor_final de ganhos, não todos
            ...
            WHEN 'margem_pct' THEN
                -- FIX: SUM apenas receita/valor_final de ganhos
...
DECLARE
    ...
    v_ts_field_sql TEXT;
    -- FIX 1: track if we're grouping by stage name for funnel ordering
...
BEGIN
    ...
    LOOP
        ...
        -- FIX 1: detect stage dimension for funnel ordering
        IF v_source = 'cards' AND v_field = 'ps.nome' THEN
    ...
    ELSIF v_has_stage_dim THEN
        -- FIX 1: Funnel ordering — quando dimensão é ps.nome, ordenar por posição do pipeline
    ...
    -- 12. Wrap with gap-filling for timeseries (FIX 4: LIMIT outside CTE)
    IF v_has_timeseries AND p_date_start IS NOT NULL AND p_date_end IS NOT NULL THEN
        ...
        ',
            ...
            '1 ' || v_ts_granularity,
            v_query,  -- inner query WITHOUT LIMIT
    ...
    ELSE
        -- Non-timeseries: add LIMIT to query directly
...
-- ============================================
-- DRILL DOWN v3 — with all sources + null handling
-- ============================================
