-- ============================================================================
-- LIGA o is_perdido na ww_funil_casal (antes era FALSE fixo / sempre 0).
--
-- Perdido = NAO ganhou (nao entrou no Planejamento / nao assinou Elopement) E
-- tem motivo de perda no Active (DEAL 56 SDR ou DEAL 47 Closer). Sinal AC-only.
--
-- Recria refresh_ww_funil_casal IDENTICA a 20260602l (preserva o fix do
-- 20260602k: fez SDR por andamento so etapas 8/61, reagendamento 201 NAO conta;
-- e o token canonico 'Elopement'). Unica mudanca: cf.tem_motivo_perda + a
-- expressao de is_perdido no SELECT final.
-- ============================================================================
...
BEGIN
    ...
    WITH ev AS (
        ...
        SELECT contact_id,
            ...
            (array_agg(faixa_raw)      FILTER (WHERE faixa_raw IS NOT NULL))[1]           AS faixa_raw,
            (array_agg(convidados_raw) FILTER (WHERE convidados_raw IS NOT NULL))[1]      AS convidados_raw,
            (array_agg(destino_raw)    FILTER (WHERE destino_raw IS NOT NULL))[1]         AS destino_raw,
            (array_agg(COALESCE(utm_source,origem_conversao)) FILTER (WHERE COALESCE(utm_source,origem_conversao) IS NOT NULL))[1] AS origem_raw,
            (array_agg(consultor_id) FILTER (WHERE consultor_id IS NOT NULL))[1]          AS consultor_id,
            (array_agg(owner_nome)   FILTER (WHERE owner_nome IS NOT NULL))[1]            AS consultor_nome,
    ...
    SELECT
        ...
        faixa, convidados, destino, origem, consultor_id, consultor_nome,
        -- is_perdido: NAO ganhou E tem motivo de perda no Active
