-- ============================================================================
-- MIGRATION: find_duplicate_tasks_tiered passa a retornar nome COMPLETO do
-- contato principal (nome + sobrenome) em contato_nome.
--
-- Antes: ct.nome AS contato_nome (só primeiro nome)
-- Depois: TRIM(ct.nome || ' ' || ct.sobrenome) AS contato_nome
--
-- Mesmo padrão usado em view_cards_acoes e v_meu_dia_concierge.
--
-- Base: 20260506i_duplicate_tasks_data_vencimento_signal.sql (última versão).
-- Só uma linha muda (linha 305) — o restante é idêntico ao original.
-- ============================================================================
...
BEGIN
    ...
    SELECT
        ...
        TRIM(BOTH FROM (COALESCE(ct.nome, ''::text) || ' '::text) || COALESCE(ct.sobrenome, ''::text)) AS contato_nome,
        u.tipo,
        u.titulos_distintos,
        u.titulo_exemplo,
        u.ids AS task_ids,
