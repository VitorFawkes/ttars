-- ============================================================================
-- Marco B — Listar cards abertos do contato + opção de forçar criação
-- Date: 2026-04-20
--
-- Objetivo: quando o operador clica "Criar card" no Echo e o contato já tem
-- cards abertos no mesmo produto, a UI mostra os cards existentes e deixa
-- ele escolher (continuar em um ou criar nova viagem). Sem esse reforço, a
-- RPC atual reusa automaticamente, bloqueando segunda viagem legítima.
--
-- Duas mudanças:
--   1. Nova RPC `listar_cards_abertos_do_contato_echo` — read-only, devolve
--      lista de cards abertos (com contexto útil) para a UI.
--   2. RPC `criar_card_de_conversa_echo` ganha parâmetro `p_force_create`.
--      Se TRUE, pula o bloco de dedup e cria novo. Mantém compat com API
--      pública que não passa o parâmetro (default FALSE).
-- ============================================================================

-- ============================================================================
-- 1) listar_cards_abertos_do_contato_echo
-- ============================================================================
...
BEGIN
    ...
    -- Resolver produto via whatsapp_linha_config (quando possível)
    ...
    -- Resolver contato (mesma lógica de camadas da RPC principal)
    ...
    -- Cards abertos do contato, no produto da linha
    SELECT COALESCE(
        jsonb_agg(
            jsonb_build_object(
                'id', c.id,
...
GRANT EXECUTE ON FUNCTION public.listar_cards_abertos_do_contato_echo(TEXT, TEXT, TEXT, TEXT)
    TO authenticated;

COMMENT ON FUNCTION public.listar_cards_abertos_do_contato_echo IS
'Marco B: lista cards abertos do contato Echo para UI de escolha (continuar ou criar nova viagem).';

-- ============================================================================
-- 2) criar_card_de_conversa_echo com p_force_create
-- ============================================================================
...
BEGIN
    ...
    IF length(v_phone_clean) < 10 OR length(v_phone_clean) > 15 THEN
        RAISE EXCEPTION 'Telefone inválido. Bruto: "%" | Dígitos: "%" (length=%)',
            COALESCE(p_phone, '<null>'), v_phone_clean, length(v_phone_clean)
            USING ERRCODE = '22023';
    END IF;
    ...
    ELSE
        -- Contato existente: atualiza nome se fraco (Marco A)
    ...
    -- 6. Dedup card (só quando p_force_create = FALSE — Marco B)
...
'Marco A+B: cria card via Echo. Atualiza nome fraco do contato. Se p_force_create=TRUE, pula dedup e cria novo (Marco B).';
