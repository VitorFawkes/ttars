-- 20260612c — "Fez a reunião" só com registro REAL: vazio novo '{}' não conta
--
-- Achado do Vitor (12/06, deal 30091 "DW | Rafaela"): casal em StandBy contando
-- como "fez a 1ª reunião". Auditoria:
--   • As etapas que contam "fez" por andamento JÁ são só 8 (Qualificado pela SDR)
--     e 61 (Aguardando pagamento TAXA) — ambas pós-reunião. StandBy (60), Follow Up (3)
--     e Reagendamento (201) NUNCA contam por andamento (regra do Vitor confirmada:
--     follow-up/standby são estados, não marcos — memory feedback_etapa_vs_acao_vendas).
--     Follow Up só conta quando os campos da reunião feita estão preenchidos (caminho
--     do campo 17) — que é exatamente a regra abaixo.
--   • O bug REAL: quando o campo 17 ("Como foi feita a 1ª reunião") ficou TEXT[]
--     (20260611a), o vazio passou a serializar como '{}' — e a guarda de vazio só
--     conhecia ('[]','""',''). 45 casais com canal vazio contavam como "fez"
--     (23 deles na janela de 30 dias — inflando o comparecimento pra ~98%).
--
-- FIX: vazio é vazio em qualquer forma — btrim(canal, ' {}[]"') = '' cobre
-- '{}', '[]', '""', '', '{""}'; e '{NULL}' explícito. Vale pra SDR e Closer.
--
-- DILIGÊNCIA (TOP-5 #5): cadeia 20260602j → k → l → 04b → 04e/f → 12a (régua 24h)
-- → 12b (só is_ww, DEFINIÇÃO VIVA — base deste arquivo, byte a byte). Únicas
-- mudanças: as duas linhas de *_canal_real.
...
-- Reconstrói a tabela já com a régua corrigida
