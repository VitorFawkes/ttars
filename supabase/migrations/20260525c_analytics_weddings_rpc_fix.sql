-- Fix: a RPC anterior usava CTE leads_norm em múltiplas queries (CTE não persiste).
-- Refazendo com TEMPORARY TABLE.
...
BEGIN
    ...
    -- Pool normalizado em TEMP TABLE (vive até final do function call)
    ...
    SELECT c.id,
           ...
           CASE
             WHEN (c.produto_data->>'ww_mkt_destino_form') IS NULL THEN NULL
             ELSE INITCAP(TRIM(REPLACE(LOWER((c.produto_data->>'ww_mkt_destino_form')), '_', ' ')))
           END AS destino,
           CASE
             WHEN c.status_comercial = 'ganho' THEN 'ganho'
    ...
    FROM (
        SELECT ph.id AS phase_id, COALESCE(ph.label, ph.name) AS phase_name, ph.order_index AS phase_order,
               s.id AS stage_id, s.nome AS stage_name, s.ordem AS stage_order,
         ...
         GROUP BY ph.id, ph.label, ph.name, ph.order_index, s.id, s.nome, s.ordem
    ) sc;
    ...
    -- 5) Top destinos
    SELECT json_agg(json_build_object(
        'destino', destino, 'qtd', qtd, 'pct', pct
    ...
    -- 8) Tempo médio por fase (activities stage_changed)
    SELECT json_agg(json_build_object(
        'phase_name', phase_name, 'avg_dias', avg_dias, 'mediana_dias', mediana_dias, 'amostra', amostra
          ...
          FROM (
            SELECT a.card_id,
              ...
              WHERE a.tipo = 'stage_changed'
                AND a.card_id IN (SELECT id FROM _ww_leads)
          ) si
    ...
    -- 9) Conversão por faixa
    SELECT json_agg(json_build_object(
        'faixa', faixa, 'total', total, 'ganhos', ganhos, 'perdidos', perdidos, 'taxa_ganho', taxa_ganho
    ...
    FROM (
        SELECT faixa,
               ...
               CASE WHEN COUNT(*) > 0
                    THEN ROUND(100.0 * COUNT(*) FILTER (WHERE status_simples = 'ganho') / COUNT(*), 1)
                    ELSE 0 END AS taxa_ganho
          FROM _ww_leads
         WHERE faixa IS NOT NULL
         GROUP BY faixa
    ) cf;

    -- 10) Conversão por destino (top 10, min 5 leads)
    SELECT json_agg(json_build_object(
        'destino', destino, 'total', total, 'ganhos', ganhos, 'taxa_ganho', taxa_ganho
