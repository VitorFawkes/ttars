-- Adiciona filtro p_phase em analytics_saude_summary e analytics_saude_list.
-- Permite responder "como está a saúde dos cards em fase SDR?" / Planner / Pós-venda.
--
-- AUDITORIA (memory/feedback_function_rebase_cuidado.md):
-- analytics_saude_summary → versão MAIS RECENTE em 20260420j (briefing_inicial JSONB)
--   preserva: 20260420f (foundation)
-- analytics_saude_list → versão MAIS RECENTE em 20260420j (mesmo fix briefing)
--   preserva: 20260420f
--
-- Mudança: param novo p_phase TEXT DEFAULT NULL.
-- NULL → comportamento atual (todos cards abertos).
-- 'sdr' / 'planner' / 'pos_venda' → filtra cards cuja etapa atual está nessa fase.
...
BEGIN
    FOR r IN
        ...
        WHERE n.nspname = 'public'
          AND p.proname IN ('analytics_saude_summary','analytics_saude_list')
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || quote_ident(r.schema) || '.' || quote_ident(r.name) || '(' || r.args || ') CASCADE';
    END LOOP;
END $cleanup$;

-- ═══ 1. analytics_saude_summary com p_phase ═══
...
BEGIN
    ...
    WITH cards_abertos AS (
        ...
        WHERE c.org_id = v_org
          ...
          AND (p_phase IS NULL OR pp.slug = p_phase)
    ),
    stages AS (
...
-- ═══ 2. analytics_saude_list com p_phase ═══
...
BEGIN
    ...
    WITH filtered AS (
        SELECT
            ...
            c.stage_entered_at,
            c.updated_at,
            GREATEST(EXTRACT(DAY FROM NOW() - c.updated_at)::INT, 0) AS dias_parado,
            s.sla_hours,
            CASE
                WHEN s.sla_hours IS NOT NULL AND c.stage_entered_at IS NOT NULL
                THEN GREATEST(EXTRACT(EPOCH FROM (NOW() - c.stage_entered_at)) / 3600 - s.sla_hours, 0)::INT
                ELSE NULL
