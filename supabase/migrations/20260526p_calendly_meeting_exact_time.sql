-- data_reuniao dispara no TIMESTAMP exato (data+hora) com offset em minutos:
-- antes (negativo), na hora (0) ou depois (positivo) da reunião.
-- 1) Remove data_reuniao do motor diário (compara só DATE).
-- 2) fn_enqueue_calendly_meeting_reminders: compara horário exato. Cron a cada 5 min.
-- data_reuniao guarda ISO local de Brasília; converte pra timestamptz antes de comparar.

-- 1) Restaura motor diário SEM o bloco data_reuniao
...
BEGIN
  ...
  -- (cards.contato_id foi removido em migration anterior; o modelo atual
  -- suporta múltiplos contatos por card via junction).
  WITH inserted AS (
    INSERT INTO public.cadence_entry_queue (org_id, card_id, trigger_id, event_type, event_data, execute_at, status)
    SELECT c.org_id, c.id, t.id, 'time_offset_from_date',
           jsonb_build_object(
    ...
    FROM public.cadence_event_triggers t
    JOIN public.cards c ON c.org_id = t.org_id
    JOIN public.cards_contatos cc ON cc.card_id = c.id AND cc.eh_principal = true
...
-- 2) Processador por horário exato da reunião (offset em minutos)
...
BEGIN
  WITH inserted AS (
    ...
    SELECT c.org_id, c.id, t.id, 'time_offset_from_date',
           jsonb_build_object(
             'source', 'card.data_reuniao',
             'minutes_offset', COALESCE((t.event_config->>'minutes_offset')::INT, 0),
             'data_reuniao', c.produto_data->>'data_reuniao'
           ),
           NOW(), 'pending'
    FROM public.cadence_event_triggers t
    JOIN public.cards c ON c.org_id = t.org_id
    WHERE t.is_active = true
      AND t.event_type = 'time_offset_from_date'
      AND t.event_config->>'source' = 'card.data_reuniao'
      AND (c.produto_data->>'data_reuniao') IS NOT NULL
      AND (c.produto_data->>'data_reuniao') <> ''
      AND (
        ((c.produto_data->>'data_reuniao')::timestamp
          + (COALESCE((t.event_config->>'minutes_offset')::INT, 0) || ' minutes')::interval)
...
SELECT cron.unschedule('calendly-meeting-reminders')
WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'calendly-meeting-reminders');

SELECT cron.schedule('calendly-meeting-reminders', '*/5 * * * *',
  $cron$SELECT public.fn_enqueue_calendly_meeting_reminders();$cron$);
