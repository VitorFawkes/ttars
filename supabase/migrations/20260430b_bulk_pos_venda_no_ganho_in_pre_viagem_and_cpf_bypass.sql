-- ============================================================================
-- MIGRATION: bulk_create_pos_venda_cards — não criar como ganho fora do Pós-viagem + bypass CPF
-- Date: 2026-04-30
--
-- Duas correções no caminho INSERT (criação de cards):
--
-- 1. status_comercial agora é 'aberto' quando o stage-destino é App & Conteúdo,
--    Pré-Embarque ou Em Viagem (a viagem ainda não aconteceu, há risco de
--    cancelamento/reembolso). Apenas Pós-viagem nasce com status='ganho'.
--    Antes: TODAS as criações eram com status='ganho', bagunçando a auditoria.
--
-- 2. Bypass de CPF: se v_trip->>'cpf_raw' está vazio/null, ainda cria o contato
--    e o card, mas:
--      - produto_data ganha flag precisa_cpf=true
--      - tarefa de alta prioridade "Atualizar CPF do contato principal" é criada
--    Para o usuário ver no Kanban que aquele card precisa ser completado.
--
-- Mantém comportamento do UPDATE inalterado.
-- ============================================================================
...
DECLARE
    ...
    -- Novos: decisão de status comercial e bypass de CPF
    v_target_stage UUID;
    v_status_comercial TEXT;
    v_cpf_raw TEXT;
    v_precisa_cpf BOOLEAN;

    -- Constante: ID da etapa Pós-viagem & Reativação (única que aceita ganho na criação)
    ...
    LOOP
        BEGIN
            ...
            ELSE
                ...
                -- Status comercial: 'ganho' SOMENTE em Pós-viagem (viagem finalizada).
                -- Em App & Conteúdo, Pré-Embarque, Em Viagem → 'aberto' (viagem ainda
                -- não aconteceu, há risco de cancelamento/reembolso, ainda não é ganho).
                ...
                -- Bypass CPF: cpf_raw pode vir vazio. Marca o card com flag pra UI alertar.
                ...
                INSERT INTO cards (
                    ...
                    v_status_comercial,        -- 'aberto' fora de Pós-viagem
                    true, NOW(), false, NULL,
                ...
                -- Tarefa de alta prioridade quando o card foi criado sem CPF —
                -- usuário atualiza ao passar pelo card no Kanban.
