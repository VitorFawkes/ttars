-- 20260617a — "Nascimento certo" na entrada do Analytics Weddings
-- Credita o lead ao funil onde NASCEU (passou pela porta), não à esteira atual.
-- Adiciona colunas tipo_entrada/entrou_valido (+ entrou_sdr/entrou_elopement) a
-- ww_funil_casal e popula via refresh_ww_funil_casal (base = def VIVA de prod).
-- Regra: DW = passou/está em SDR Weddings (g1) OU em funil de casamento (cadeia DW);
--        Elopement = passou/está em Elopment (g12); convidados (título "- W -") e
--        cadastros vazios saem da conta; ganho NUNCA é descartado.
-- Validado em prod (mar-mai/2026): 665 DW / 195 Elopement / 860 válidos;
--   ganhos all-time 172 intactos (0 dropados); 0 convidado vazando.

ALTER TABLE ww_funil_casal
  ADD COLUMN IF NOT EXISTS entrou_sdr       BOOLEAN,
  ADD COLUMN IF NOT EXISTS entrou_elopement BOOLEAN,
  ADD COLUMN IF NOT EXISTS tipo_entrada     TEXT,
  ADD COLUMN IF NOT EXISTS entrou_valido    BOOLEAN;
...
BEGIN
    ...
    WITH ww_contacts AS (
        ...
        SELECT c.contact_id,
            ...
            -- 20260617: cadeia DW (sales+pós-venda, não-convidado) e funis de convidado
            ...
            -- 20260617: FAKE/DUPLICADO do Active — só exclui quando TODOS os negócios
            -- do contato são fake/dup (bool_and). Contato com 1 negócio limpo = casal
            -- real com cópia extra → mantém (não derruba). Ganho é protegido no CASE.
        ...
        SELECT
            ...
            -- 20260617: NASCIMENTO CERTO — porta de entrada do funil (cache esteira atual + jornada de movimento)
            ...
            -- is_ganho = MESMA lógica do ganho do SELECT final (MANTER OS DOIS IGUAIS)
            (CASE
