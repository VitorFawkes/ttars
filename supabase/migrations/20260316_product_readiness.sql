-- ============================================================
-- Product Readiness: controle operacional de produtos no pós-vendas
-- Adiciona is_ready/notes em card_financial_items
-- Cria product_requirements para sub-itens flexíveis
-- Atualiza view_cards_acoes com contadores de produtos
-- ============================================================

-- 1. Colunas de controle no produto
ALTER TABLE card_financial_items
  ADD COLUMN IF NOT EXISTS is_ready BOOLEAN DEFAULT false,
  ADD COLUMN IF NOT EXISTS notes TEXT;

CREATE INDEX IF NOT EXISTS idx_cfi_card_ready ON card_financial_items(card_id, is_ready);

-- 2. Sub-itens flexíveis por produto
...
CREATE INDEX IF NOT EXISTS idx_prod_req_financial ON product_requirements(financial_item_id);
CREATE INDEX IF NOT EXISTS idx_prod_req_card ON product_requirements(card_id);

-- 3. RLS
ALTER TABLE product_requirements ENABLE ROW LEVEL SECURITY;

CREATE POLICY "product_requirements_select" ON product_requirements
  FOR SELECT USING (true);

CREATE POLICY "product_requirements_insert" ON product_requirements
  FOR INSERT WITH CHECK (true);

CREATE POLICY "product_requirements_update" ON product_requirements
  FOR UPDATE USING (true);

CREATE POLICY "product_requirements_delete" ON product_requirements
  FOR DELETE USING (true);

-- 4. Atualizar view com contadores de produtos
...
SELECT c.id,
    ...
    -- NEW: product readiness counters
