-- Trigger Calendly agora preenche cards.produto_data->>'data_reuniao' e
-- também data_viagem_inicio/data_viagem_fim quando cria card.
--
-- - Card criado: já vai com data_reuniao no produto_data.
-- - Card existente (match): UPDATE produto_data pra incluir/sobrescrever data_reuniao.
...
DECLARE
  ...
  v_data_reuniao_iso     TEXT;
  v_extra_produto_data   JSONB;
BEGIN
  IF NEW.event_type <> 'invitee.created' THEN
    RETURN NEW;
  END IF;

  -- Dados pra produto_data (ISO 8601 — UniversalFieldRenderer aceita)
  v_data_reuniao_iso := NEW.event_start_time::text;
  v_extra_produto_data := jsonb_build_object(
    'data_reuniao', v_data_reuniao_iso,
  ...
  LOOP
    ...
    IF NEW.card_id IS NOT NULL THEN
      ...
      -- Card existente: mergeia data_reuniao no produto_data
      UPDATE cards
      SET produto_data = coalesce(produto_data, '{}'::jsonb) || v_extra_produto_data,
          updated_at = NOW()
    ...
    ELSIF coalesce((v_trigger.event_config->>'create_card_if_missing')::BOOLEAN, FALSE) THEN
      ...
      INSERT INTO cards (
        org_id, pessoa_principal_id, pipeline_stage_id, pipeline_id, titulo, produto, produto_data,
        dono_atual_id,
        sdr_owner_id,
        vendas_owner_id,
        pos_owner_id,
