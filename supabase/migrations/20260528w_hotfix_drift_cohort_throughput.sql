-- ============================================================================
-- HOTFIX: ww_v2_drift_venda v5 + ww_drift_combos — cohort vs throughput correto
--
-- Bug crítico: v4 (e drift_combos antigo) SEMPRE filtravam por ganho_at,
-- ignorando p_date_mode. Resultado: cohort 12m mostrava 98 entraram = 98 fecharam
-- (100% taxa, totalmente falso — usuário Vitor sinalizou).
--
-- Lógica correta:
--   - COHORT     = cards com c.created_at no período (universo amplo)
--                  → "fechou" via JOIN com vw_ww_funnel_base.ganho (cache AC)
--   - THROUGHPUT = cards com m.ganho_at no período (universo = fechados)
--
-- Validado:
--   Cohort 12m: 2077 entraram, 59 fecharam (era 98=98 antes)
--   Throughput 12m: 65 fechou no período
--   Cohort 30d: 328 entraram, 2 fecharam
-- ============================================================================

-- ww_v2_drift_venda v5: respeita p_date_mode CORRETAMENTE
-- cohort     = cards Weddings com created_at no período (universo amplo, fechados ≤ universo)
-- throughput = cards com ganho_at no período (universo = fechados)
...
BEGIN
    ...
    -- Pool baseado em cards (incluindo abertos/perdidos pra cohort), JOIN com cache AC pra ganho
    CREATE TEMP TABLE _ww_v2_dv ON COMMIT DROP AS
    SELECT c.id, c.titulo,
           fc.ac_deal_id,
           fc.ganho_at AS data_venda,
           COALESCE(fc.ganho, FALSE) AS fechou,
           -- ENTRADA do form site (deal 27/28/26 via produto_data CRM, igual lead_ideal/drift_combos)
           ...
           -- REALIDADE DO CASAL (Contact 376/121, cache AC novo) — só pra quem fechou
           CASE
             WHEN NOT COALESCE(fc.ganho, FALSE) THEN NULL
             WHEN fcache.real_orcamento_parsed IS NULL THEN NULL
             WHEN fcache.real_orcamento_parsed < 50000 THEN 'Até R$50 mil'
             WHEN fcache.real_orcamento_parsed < 80000 THEN 'R$50-80 mil'
             WHEN fcache.real_orcamento_parsed < 100000 THEN 'R$80-100 mil'
             WHEN fcache.real_orcamento_parsed < 200000 THEN 'R$100-200 mil'
             WHEN fcache.real_orcamento_parsed < 500000 THEN 'R$200-500 mil'
             ELSE '+R$500 mil'
           END AS faixa_v,
           CASE WHEN COALESCE(fc.ganho, FALSE) THEN _ww2_norm_dest_strict(cc.real_destino) ELSE NULL END AS dest_v,
           CASE WHEN COALESCE(fc.ganho, FALSE) THEN fcache.real_convidados_parsed ELSE NULL END AS num_convidados_real,
           CASE
             WHEN NOT COALESCE(fc.ganho, FALSE) THEN NULL
             WHEN fcache.real_convidados_parsed IS NULL THEN NULL
             WHEN fcache.real_convidados_parsed <= 2 THEN 'Apenas o casal'
             WHEN fcache.real_convidados_parsed <= 20 THEN 'Ate 20'
             WHEN fcache.real_convidados_parsed <= 50 THEN '20-50'
             WHEN fcache.real_convidados_parsed <= 80 THEN '50-80'
             WHEN fcache.real_convidados_parsed <= 100 THEN '80-100'
             ELSE '+100'
           END AS conv_r,
           cc.real_valor_assess AS valor_final,
           cc.real_monde AS monde_venda,
           _ww2_norm_origem(c.marketing_data) AS origem,
           NULLIF(c.produto_data->>'ww_tipo_casamento','') AS tipo_casamento,
           COALESCE(c.titulo, cc.contato_nome) AS contato_nome,
           c.external_id AS contato_external_id,
           c.created_at AS card_created_at
    FROM cards c
    LEFT JOIN vw_ww_funnel_base fc ON fc.card_id = c.id
    LEFT JOIN ww_ac_deal_funnel_cache fcache ON fcache.ac_deal_id = c.external_id AND c.external_source = 'active_campaign'
    LEFT JOIN ww_v2_casamentos_cache cc ON cc.contact_id = c.external_id
    WHERE c.deleted_at IS NULL AND c.archived_at IS NULL
      AND c.produto::TEXT='WEDDING'
      AND CASE
        WHEN p_date_mode = 'throughput' THEN (fc.ganho_at BETWEEN p_date_start AND p_date_end)
    ...
    RETURN json_build_object(
        ...
        'fonte_v2', 'cards (universo amplo cohort) + cache AC (fechou via field 87)',
...
-- ww_drift_combos: "fechou" agora vem do cache AC, throughput filtra por ganho_at AC
