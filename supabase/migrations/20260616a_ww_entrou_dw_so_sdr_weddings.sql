-- 20260616a_ww_entrou_dw_so_sdr_weddings.sql
--
-- REGRA DE NEGÓCIO (Vitor, 2026-06-16): no funil Weddings, "Lead/Entrou" para casais DW
-- conta APENAS quem passou pela esteira de entrada do DW no Active — "SDR Weddings"
-- (dealGroup id 1). Casais DW que entraram direto em outra esteira (Produção, Closer,
-- Planejamento, Convidados…) NÃO contam como "entrou". Demais tipos (Elopement etc) seguem
-- contando pela porta deles — só DW exige SDR Weddings.
--
-- Impacto medido (junho/2026): DW 114 → 84 ; Elopement 19 → 19 ; total 133 → 103.
--
-- ESCOPO: ajusta SÓ o "Entrou" (KPI "Leads" + 1ª etapa do funil + base do funil cohort).
-- NÃO mexe em perdidos (fake/duplicado fica pra depois). Marcos seguintes (reunião/fechado)
-- seguem contando dentro do universo cohort, que agora nasce da entrada válida — funil coerente.
--
-- Base: definição VIVA de produção (pg_get_functiondef, 2026-06-16) — guard de rebase TOP-5 #5.
-- ww2_overview mantém a MESMA assinatura (só corpo muda) → CREATE OR REPLACE, grants persistem.
