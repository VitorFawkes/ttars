-- ============================================================================
-- DEFESA EM PROFUNDIDADE: bloquear cross-workspace nos dispatchers de automacao
-- ============================================================================
-- Hoje as 5 funcoes process_cadence_entry_on_* filtram cadence_event_triggers
-- por applicable_stage_ids e applicable_pipeline_ids, mas NAO por org_id.
-- Resultado: se um trigger fica com esses filtros NULL (ex: regressao via
-- save antigo do editor visual, importacao manual, etc), ele dispara pra
-- qualquer card de qualquer workspace.
--
-- Esta migration adiciona em cada dispatcher o filtro:
--     AND EXISTS (SELECT 1 FROM cadence_templates ct
--                 WHERE ct.id = target_template_id AND ct.org_id = <card.org_id>)
--
-- Triggers sem target_template_id (raro) ficam silenciosamente bloqueados.
-- Caso futuro com cross-org legitimo precisa de coluna org_id propria na
-- tabela cadence_event_triggers — out of scope desta migration.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- 4) tag_added (NEW e card_tag_links — precisa puxar org_id de cards)
-- ----------------------------------------------------------------------------
...
DECLARE
    ...
    v_pending_count INT;
BEGIN
    SELECT c.pipeline_id, c.org_id INTO v_card_pipeline_id, v_card_org_id
        FROM cards c
...
-- ----------------------------------------------------------------------------
-- 5) tag_removed (OLD e card_tag_links — precisa puxar org_id de cards)
-- ----------------------------------------------------------------------------
...
BEGIN
    ...
    LEFT JOIN cadence_templates ct ON ct.id = cet.target_template_id
    WHERE cet.is_active = true
      AND (cet.target_template_id IS NULL OR ct.org_id IS NULL);

    RAISE NOTICE 'Triggers ativos total: %', v_total;
    RAISE NOTICE 'Triggers ativos SEM target_template_id/ct.org_id (bloqueados pelo novo filtro): %', v_blocked;
END $$;
