-- ============================================================================
-- Fase C — aba PERFIL (ww_v2_lead_ideal) lê do armazém AC (ww_funil_casal).
-- Antes: histórico de ww_v2_casamentos_cache, atual de cards (CRM).
-- Agora: histórico = ww_funil_casal WHERE ganho; atual = leads do período.
-- SÓ as 2 fontes internas mudam — todo o resto (comparação/lift/cruzamentos/top)
-- é idêntico. Dimensões já vêm normalizadas no armazém. DROP+CREATE.
-- ============================================================================
...
BEGIN
    ...
    -- HISTÓRICO: casais que FECHARAM (ww_funil_casal, ganho) — AC, dims já normalizadas
    ...
    -- ATUAL: leads novos no período (ww_funil_casal) — AC
