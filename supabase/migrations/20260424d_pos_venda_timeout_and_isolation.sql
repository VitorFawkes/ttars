-- ============================================================================
-- MIGRATION: Resiliência da importação pós-venda
-- Date: 2026-04-24
--
-- PROBLEMAS:
-- 1. Statement timeout: processando muitas viagens de uma vez, os triggers
--    em cascata no INSERT em cards/tarefas estouram o statement_timeout
--    padrão de 8s do PostgREST → "canceling statement due to statement
--    timeout".
-- 2. Falhas propagam: se um trip falha, a RPC inteira faz rollback —
--    perde tudo.
--
-- FIXES:
-- 1. SET LOCAL statement_timeout = '300s' no início da RPC (5 min por call).
-- 2. Cada trip processado em BEGIN/EXCEPTION isolado — um trip que falhar
--    é registrado no results com action='error' mas não derruba os outros.
--
-- Obs.: o frontend vai fazer batching de 15 trips por chamada pra manter
-- cada call rápida e permitir progresso visível ao usuário.
-- ============================================================================
...
BEGIN
    -- Importação em lote pode demorar (triggers cascata). Evita timeout curto do PostgREST.
    ...
    LOOP
        -- Cada trip em um sub-bloco isolado: se um falha, os outros continuam.
        BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
            -- Isola o erro: regista e segue pro próximo trip.
