-- Fix: ROUND(double precision, int) não existe — castear AVG/PERCENTILE_CONT pra NUMERIC.

DROP FUNCTION IF EXISTS public.analytics_planner_stage_x_owner(TIMESTAMPTZ, TIMESTAMPTZ, UUID[], UUID[], TEXT);

CREATE FUNCTION public.analytics_planner_stage_x_owner(
...
BEGIN
    ...
    SELECT
        ...
        ROUND(COALESCE(
            (SELECT AVG(dias_na_etapa)::NUMERIC FROM transicoes t
             WHERE t.stage_id = s.id AND t.owner_id = pl.id),
            0
        ), 1) AS tempo_medio_dias,
        ROUND(COALESCE(
            (SELECT PERCENTILE_CONT(0.9) WITHIN GROUP (ORDER BY dias_na_etapa)::NUMERIC
             FROM transicoes t
             WHERE t.stage_id = s.id AND t.owner_id = pl.id),
            0
        ), 1) AS tempo_pior_dias,
