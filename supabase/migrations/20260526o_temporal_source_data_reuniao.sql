-- Adiciona source 'card.data_reuniao' ao motor temporal (time_offset_from_date).
-- Permite automação "X dias antes/depois da reunião agendada no Calendly".
-- data_reuniao vive em cards.produto_data (ISO local de Brasília).
...
BEGIN
  ...
  -- ── time_offset_from_date: source = card.data_reuniao (Calendly) ──
  -- data_reuniao mora em cards.produto_data (ISO local). Dispara X dias antes/depois.
  WITH inserted AS (
    INSERT INTO public.cadence_entry_queue (org_id, card_id, trigger_id, event_type, event_data, execute_at, status)
    SELECT c.org_id, c.id, t.id, 'time_offset_from_date',
           jsonb_build_object(
    ...
    WHERE t.is_active = true
      ...
      AND DATE((c.produto_data->>'data_reuniao')::timestamp)
          + ((t.event_config->>'offset_days')::INT) = CURRENT_DATE
      AND (
        t.applicable_pipeline_ids IS NULL
        OR array_length(t.applicable_pipeline_ids, 1) IS NULL
