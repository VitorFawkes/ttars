-- v6 — UNIDADE = PESSOAS em TODAS as agregações
--
-- REBASE CONFIRMADO. Releitura das migrations anteriores:
--   - 20260519c (v1): SUPERSEDED.
--   - 20260519d (v2): assinatura TEXT[].
--   - 20260519f (v3): breakdowns.
--   - 20260519g (v4): frt_distribution + heatmap + time_metrics.
--   - 20260519h (v5): daily_timeline + drill-down do heatmap. ESTA é a base.
--
-- A v6 PRESERVA assinatura idêntica e lógica de estado/FRT/KPIs.
-- ÚNICA MUDANÇA: contar PESSOAS ÚNICAS (customer_phone distinct) em vez
-- de contar MENSAGENS em:
--   - weekday_hour_heatmap: count agora é pessoas únicas que responderam
--     naquele dia/hora (não mais total de mensagens recebidas)
--   - daily_timeline.outbound: pessoas contatadas no dia (não total de msgs)
--   - daily_timeline.inbound:  pessoas que responderam no dia (não total)
--   - daily_timeline.msgs_out (novo): total de mensagens enviadas no dia
--   - daily_timeline.msgs_in  (novo): total de mensagens recebidas no dia
--
-- Motivação: "Toda análise deve ser por quantidade de pessoas, não de
-- mensagens" (regra do Vitor). Heatmap clicado em Seg 12h mostrava 60
-- (msgs) mas a tabela filtrada mostrava 9 pessoas. Inconsistência grave.

DROP FUNCTION IF EXISTS analytics_weddings_conversations(
  DATE, DATE, TEXT[], TEXT[], TEXT[], INT, BOOLEAN, INT, INT, INT, INT
) CASCADE;
...
BEGIN
  ...
  -- msgs visíveis após filtros (mas SEM dropar pelo filtro de estado, pra
  -- timeline mostrar atividade contínua mesmo quando filtra estado)
  visible_msgs AS (
    SELECT fm.*
  ...
  -- ★ MUDOU v6: heatmap conta PESSOAS ÚNICAS, não mensagens
  ...
  -- ★ MUDOU v6: timeline em PESSOAS, msgs como auxiliares
  daily_timeline AS (
    SELECT json_agg(jsonb_build_object(
      'day', day,
      'outbound', outbound,       -- pessoas contatadas no dia
      'inbound', inbound,         -- pessoas que responderam no dia
      'msgs_out', msgs_out,       -- total de msgs enviadas no dia
      'msgs_in',  msgs_in,        -- total de msgs recebidas no dia
    ...
    FROM (
      ...
      people_by_day AS (
        SELECT
          sent_at::DATE AS day,
          COUNT(DISTINCT customer_phone) FILTER (WHERE direction = 'outbound')::INT AS outbound,
          COUNT(DISTINCT customer_phone) FILTER (WHERE direction = 'inbound')::INT  AS inbound,
      ...
      wins_by_day AS (
        ...
        JOIN card_lookup cl ON cl.card_id = cf.card_id
        WHERE cf.state = 'won'
        GROUP BY COALESCE(cl.ganho_sdr_at::DATE, cl.created_at::DATE)
      )
      SELECT
        d.day,
        COALESCE(pd.outbound, 0)  AS outbound,
        COALESCE(pd.inbound, 0)   AS inbound,
        COALESCE(pd.msgs_out, 0)  AS msgs_out,
        COALESCE(pd.msgs_in, 0)   AS msgs_in,
        COALESCE(nc.cnt, 0)       AS new_contacts,
        COALESCE(nr.cnt, 0)       AS new_replies,
        COALESCE(wb.cnt, 0)       AS wins
      FROM days d
      LEFT JOIN people_by_day pd ON pd.day = d.day
...
COMMENT ON FUNCTION analytics_weddings_conversations IS
  'v6 (rebase confirmado da v5 - 20260519h): unidade = PESSOAS UNICAS em heatmap e daily_timeline. msgs como auxiliares no timeline.';
