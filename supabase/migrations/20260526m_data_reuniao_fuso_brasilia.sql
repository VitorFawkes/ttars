-- data_reuniao estava sendo gravado em UTC cru e renderizado sem conversão de fuso
-- (campo type=date não mostra hora nem ajusta timezone). Correções:
--   1) Muda type do campo pra 'datetime' (renderer mostra hora).
--   2) Grava data_reuniao no fuso de Brasília (America/Sao_Paulo), formato
--      ISO local 'YYYY-MM-DDTHH:MI:SS' que o UniversalFieldRenderer entende.
--   3) Backfill: reconverte os cards já preenchidos.

-- 1) tipo do campo -> datetime
UPDATE system_fields SET type = 'datetime' WHERE key = 'data_reuniao';

-- helper: timestamp UTC -> texto ISO local Brasília
CREATE OR REPLACE FUNCTION calendly_local_dt(ts TIMESTAMPTZ)
RETURNS TEXT AS $f$
  SELECT to_char(ts AT TIME ZONE 'America/Sao_Paulo', 'YYYY-MM-DD"T"HH24:MI:SS');
$f$ LANGUAGE sql IMMUTABLE;

-- 2a) sync trigger (sempre atualiza card existente)
...
-- 2b) trigger principal — só a montagem do v_extra_data muda (resto idêntico à 20260526l)
...
-- 3) Backfill: reconverte data_reuniao já gravado (que está em UTC) pra local SP.
-- Detecta valores com sufixo de timezone (+00) ou espaço, e regrava ISO local.
UPDATE cards
SET produto_data = jsonb_set(produto_data, '{data_reuniao}',
       to_jsonb(calendly_local_dt((produto_data->>'data_reuniao')::timestamptz))),
    briefing_inicial = CASE WHEN briefing_inicial ? 'data_reuniao'
       THEN jsonb_set(briefing_inicial, '{data_reuniao}',
              to_jsonb(calendly_local_dt((briefing_inicial->>'data_reuniao')::timestamptz)))
       ELSE briefing_inicial END
WHERE produto_data ? 'data_reuniao'
  AND (produto_data->>'data_reuniao') ~ '\+00|\s';
