-- 20260602s — ww_drift_combos: universo deixa de ser CARDS do CRM e passa a ser a
-- vw_ww_funnel_base (cache Active), espelhando exatamente a ww_v2_drift_venda (528x).
--
-- VERIFICAÇÃO REBASE (TOP 5 #5): parte da 20260528w (def mais recente; 527zc é a anterior).
-- Só o bloco do TEMP TABLE _ww_dc muda (FROM cards c → FROM vw_ww_funnel_base v; dims
-- declaradas de v.* em vez de c.produto_data). Todas as agregações (top_combos, 3 matrizes)
-- permanecem idênticas a 528w. Nenhuma correção revertida.
--
-- POR QUÊ: Analytics-Weddings = só Active. As dims declaradas já vêm do cache na view
-- (20260602q), e o universo agora também (mesmas linhas da drift_venda → contagens batem).
...
BEGIN
    ...
    -- Universo = vw_ww_funnel_base (cache Active), igual à ww_v2_drift_venda. Dims declaradas
    -- (faixa/destino/convidados) já são Active na view. fechou/ganho_at idem.
    ...
    RETURN json_build_object(
      ...
      'fonte_marcos', 'vw_ww_funnel_base (cache AC) — universo Active, igual drift_venda'
    );
END $func$;
