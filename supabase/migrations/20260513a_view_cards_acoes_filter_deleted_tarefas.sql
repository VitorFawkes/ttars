-- ============================================================================
-- MIGRATION: view_cards_acoes — filtrar tarefas soft-deleted (deleted_at)
-- Date: 2026-05-13
--
-- Bug: kanban mostrando "Atrasada X dias" em cards sem tarefas reais.
-- Causa: a view conta tarefas com deleted_at preenchido como pendentes/atrasadas.
-- Identificadas 244 tarefas soft-deleted em produção sendo contadas.
--
-- Fix: adicionar `tarefas.deleted_at IS NULL` nos 4 subqueries de tarefas
-- (proxima_tarefa, tarefas_pendentes, tarefas_atrasadas, ultima_interacao).
-- Outras queries do projeto já filtram deleted_at — esta view foi a única
-- que ficou sem.
--
-- Mantém a coluna sdr_qualification_score_latest da migration anterior
-- (20260512f) e preserva ordem de colunas para não quebrar consumers.
-- ============================================================================
...
SELECT c.id,
    ...
    ( SELECT row_to_json(t.*) AS row_to_json
           FROM ( SELECT tarefas.id,
                    ...
                    tarefas.tipo
                   FROM tarefas
                  WHERE tarefas.card_id = c.id
                    AND tarefas.deleted_at IS NULL
                    AND COALESCE(tarefas.concluida, false) = false
                    AND (tarefas.status IS NULL OR tarefas.status <> 'reagendada'::text)
    ...
    ( SELECT count(*) AS count
          ...
          WHERE tarefas.card_id = c.id
            AND tarefas.deleted_at IS NULL
            AND COALESCE(tarefas.concluida, false) = false
            AND (tarefas.status IS NULL OR tarefas.status <> 'reagendada'::text)) AS tarefas_pendentes,
    ( SELECT count(*) AS count
           FROM tarefas
          WHERE tarefas.card_id = c.id
            AND tarefas.deleted_at IS NULL
            AND COALESCE(tarefas.concluida, false) = false
            AND tarefas.data_vencimento < CURRENT_DATE
            AND (tarefas.status IS NULL OR tarefas.status <> 'reagendada'::text)) AS tarefas_atrasadas,
    ( SELECT row_to_json(t.*) AS row_to_json
           FROM ( SELECT tarefas.id,
                    tarefas.titulo,
