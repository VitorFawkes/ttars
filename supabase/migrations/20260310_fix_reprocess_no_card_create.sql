-- ============================================================
-- MIGRATION: Fix retroactive reprocess creating unwanted cards
-- Date: 2026-03-10
--
-- Problema:
-- Ao criar contato de dentro de um card (ex: adicionar viajante),
-- os triggers trg_reprocess_whatsapp_on_new_phone e
-- trg_reprocess_whatsapp_on_contato_phone reprocessam mensagens
-- WhatsApp órfãs. process_whatsapp_raw_event_v2 não encontra
-- card por pessoa_principal_id (o contato é secundário) e cria
-- um card novo indevidamente.
--
-- Fix:
-- 1. reprocess_orphan_whatsapp_for_phone seta session var
--    app.skip_card_creation = 'true' antes de reprocessar
-- 2. process_whatsapp_raw_event_v2 checa essa var e pula
--    card creation se estiver setada
--
-- Bonus fix:
-- Restaura lógica per-line override (criar_card) que foi
-- regredida por 20260309_fix_whatsapp_status_fallthrough.sql
-- ============================================================

-- 1. Recriar reprocess_orphan_whatsapp_for_phone com set_config
...
-- 2. Recriar process_whatsapp_raw_event_v2 com check de session var
--    + restaurar per-line override (criar_card) regredido
...
BEGIN
    ...
    IF v_contact_id IS NULL THEN
        -- Per-line override for contact creation
