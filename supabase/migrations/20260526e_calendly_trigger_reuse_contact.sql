-- Fix: contatos tem unique (org_id, email). Se contato existe, reutiliza em vez de
-- tentar criar e falhar com 23505. Edge case real: webhooks duplicados deixaram
-- contato órfão sem card.
  ...
  LOOP
    ...
    ELSIF coalesce((v_trigger.event_config->>'create_card_if_missing')::BOOLEAN, FALSE) THEN
      ...
      IF v_resolved_contato_id IS NULL THEN
        -- Reutiliza contato existente por (org_id, email) antes de tentar criar
