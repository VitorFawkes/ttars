-- ============================================================================
-- MIGRATION: Fix origem dos contatos criados na importação pós-venda
-- Date: 2026-04-24
--
-- BUG: RPC bulk_create_pos_venda_cards cria contatos com origem='importacao'
-- para pagante e acompanhantes que não têm match por CPF/nome. A trigger
-- de validação (validate_contato_required_fields) isenta apenas origens
-- automáticas (echo, integracao, trigger, whatsapp, monde, ai_extraction).
-- 'importacao' não está na lista → erro "Telefone é obrigatório".
--
-- FIX: trocar origem='importacao' por origem='monde' nas duas criações
-- (pagante e acompanhante). É semanticamente correto — o dado vem do
-- relatório Monde.
--
-- Também ajusta o matching de acompanhantes: além de nome+sobrenome exato,
-- faz fallback por primeira palavra + última palavra (cobre casos onde o
-- nome foi cadastrado sem middle names).
-- ============================================================================
    ...
    LOOP
        ...
        ELSE
            ...
            IF v_contato_id IS NULL THEN
                ...
                -- origem='monde' isenta da trigger que exige telefone (import vem do Monde)
            ...
            IF v_trip->'acompanhantes' IS NOT NULL THEN
                ...
                LOOP
                    ...
                    -- 1. Match exato (nome + sobrenome completo)
                    ...
                    -- 2. Fallback: nome + última palavra do sobrenome (cobre casos onde contato
                    --    foi cadastrado sem middle names, ex: "Diego Oliveira" vs "Diego Barbosa Oliveira")
                    ...
                    -- 3. Criar novo (origem='monde' isenta da exigência de telefone)
