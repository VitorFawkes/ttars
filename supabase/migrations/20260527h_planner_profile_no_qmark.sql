-- Fix v3: o operador `?` do JSONB (key existence) estava sendo interpretado como
-- placeholder de bind variable quando a função é invocada via PostgREST/RPC API,
-- causando "invalid input syntax for type json" em runtime.
-- Substituição: usar `(jsonb->>'key') IS NOT NULL` que tem semântica equivalente.
...
BEGIN
    ...
    cards_pessoa AS (
        SELECT c.id, c.status_comercial, c.valor_final, c.valor_estimado, c.receita,
               ...
               -- Sem `?` JSONB operator (conflita com bind placeholder via PostgREST)
               CASE
...
-- Limpa helpers de teste deixados pra debug
DROP FUNCTION IF EXISTS public.test_planner_profile_minimal(UUID);
DROP FUNCTION IF EXISTS public.test_pp_header(UUID, TIMESTAMPTZ, TIMESTAMPTZ, TEXT);
DROP FUNCTION IF EXISTS public.test_pp_agora(UUID, TEXT);
DROP FUNCTION IF EXISTS public.test_pp_periodo(UUID, TIMESTAMPTZ, TIMESTAMPTZ, TEXT);
DROP FUNCTION IF EXISTS public.test_pp_motivos(UUID, TIMESTAMPTZ, TIMESTAMPTZ, TEXT);
DROP FUNCTION IF EXISTS public.test_pp_forecast(UUID, TEXT);
