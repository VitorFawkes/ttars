-- A UI lê campos via TripInformation.tsx que escolhe entre cards.briefing_inicial
-- (fase SDR) ou cards.produto_data (fase Planner+). Como cards do Calendly nascem
-- em fase SDR, precisamos gravar em ambos.

-- 1) Atualiza sync trigger pra escrever em briefing_inicial também
...
-- 2) Atualiza o trigger principal: tanto no UPDATE de card existente quanto
-- no INSERT de card novo, preencher briefing_inicial junto.
...
-- 3) Backfill: copia data_reuniao + meeting_link + event_name de produto_data
-- pra briefing_inicial em todos os cards já criados pelo Calendly.
UPDATE cards
SET briefing_inicial = coalesce(briefing_inicial, '{}'::jsonb)
                    || jsonb_build_object(
                         'data_reuniao', produto_data->>'data_reuniao',
                         'calendly_meeting_link', produto_data->>'calendly_meeting_link',
                         'calendly_event_name', produto_data->>'calendly_event_name'
                       )
WHERE produto_data ? 'data_reuniao'
  AND coalesce(briefing_inicial->>'data_reuniao', '') = '';
