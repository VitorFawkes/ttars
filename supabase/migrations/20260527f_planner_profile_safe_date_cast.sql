-- Fix: analytics_planner_profile estava crashando com "invalid input syntax for type json"
-- quando algum card tinha produto_data->>'data_prevista_fechamento' como string que não
-- castea pra DATE (ex: vazio, formato inválido). Adiciona regex guard antes do cast
-- e usa CASE pra fallback seguro.
...
BEGIN
    ...
    cards_pessoa AS (
        SELECT c.id, c.status_comercial, c.valor_final, c.valor_estimado, c.receita,
               ...
               c.produto_data,
               -- Cast seguro de data_prevista_fechamento: só vira DATE se for YYYY-MM-DD
               CASE
                   WHEN c.produto_data IS NOT NULL
                    AND c.produto_data ? 'data_prevista_fechamento'
                    AND (c.produto_data->>'data_prevista_fechamento') ~ '^\d{4}-\d{2}-\d{2}$'
                   THEN (c.produto_data->>'data_prevista_fechamento')::DATE
                   ELSE NULL
               END AS data_prev_fech
        FROM cards c
    ...
    forecast AS (
        SELECT
            COUNT(*) FILTER (
                WHERE cp.data_prev_fech IS NOT NULL
                  AND cp.data_prev_fech BETWEEN v_today AND v_prox_7d
            )::INT AS prox_7d_qtd,
            ...
            COUNT(*) FILTER (
                WHERE cp.data_prev_fech IS NOT NULL
                  AND cp.data_prev_fech BETWEEN v_today AND v_prox_30d
            )::INT AS prox_30d_qtd,
            COALESCE(SUM(COALESCE(cp.valor_estimado, cp.valor_final, 0)) FILTER (
                WHERE cp.data_prev_fech IS NOT NULL
                  AND cp.data_prev_fech BETWEEN v_today AND v_prox_30d
    ...
    ranking AS (
        SELECT
            ct.vendas_owner_id,
            ROW_NUMBER() OVER (
                ORDER BY COALESCE(SUM(ct.receita) FILTER (WHERE ct.status_comercial = 'ganho'
                                                            AND ct.data_fechamento >= p_date_start
                                                            AND ct.data_fechamento < p_date_end), 0) DESC
            ) AS posicao
        FROM cards_time ct
