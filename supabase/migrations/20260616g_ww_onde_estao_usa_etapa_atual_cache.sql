-- 20260616g_ww_onde_estao_usa_etapa_atual_cache.sql
--
-- Parte B: "Onde estão agora" passa a classificar a posição ATUAL do casal pela etapa atual
-- da cache (ww_ac_deal_funnel_cache.ac_current_stage_id = deal.stage do Active), em vez do
-- last_stage derivado da timeline ww_deal_event (que estava INCOMPLETA — StandBy mostrava 25
-- vs ~63 reais no Active).
--
-- Mudanças no bloco "Onde estão agora" do ww2_overview:
--  - nova CTE cur_stage_agg: etapa atual por casal (prioriza deal da esteira SDR Weddings group 1).
--  - pipe (SDR/CLO/STANDBY/OUT) e flags de posição (r_taxa/r_reagenda/r_standby/...) derivam de
--    cur_stage (atual, confiável) em vez de bool_or da timeline.
--  - StandBy (cur_stage=60) tem prioridade absoluta → linha "SDR · StandBy" conta pipe='STANDBY'.
-- Mantém ev_agg só pra last_event_at (janela de atividade). Marcos (agendou/fez) seguem do casal.
--
...
BEGIN
    ...
    base AS (
        ...
        SELECT t.agendou_sdr, t.fez_sdr, t.agendou_closer, t.fez_closer,
               ...
               CASE
                 WHEN cs.cur_stage = '60' THEN 'STANDBY'   -- StandBy atual vence qualquer outra coisa
                 WHEN t.agendou_closer OR t.fez_closer THEN 'CLO'
                 WHEN cs.cur_stage IS NULL THEN 'SDR'
                 WHEN cs.cur_stage = ANY(ARRAY['1','3','7','8','60','61','186','198','201']) THEN 'SDR'
                 WHEN cs.cur_stage = ANY(ARRAY['13','14','15','16','37','163','193','221','222']) THEN 'CLO'
                 ELSE 'OUT'
               END AS pipe
          FROM _ww2c t
