-- ============================================================================
-- MIGRATION: bulk_create_pos_venda_cards — flags skip_financials + update_financials_on_existing
-- Date: 2026-04-30
--
-- Cobre o fluxo "agregada primeiro (sem valor), detalhada depois (com valor)":
--
-- 1. skip_financials (default false)
--    Quando a planilha agregada vem SEM valor/receita, o frontend manda
--    skip_financials=true e products=[]. RPC NÃO cria card_financial_items
--    nem recalcula valor_final/receita do card. Card nasce com valor 0
--    (placeholder) — vai ser atualizado quando vier o relatório detalhado.
--
-- 2. update_financials_on_existing (default false)
--    Quando a planilha detalhada vem pra um card que JÁ EXISTE, o frontend
--    manda update_financials_on_existing=true. RPC então:
--      - DELETA items existentes do card com product_type='custom' (vieram da
--        importação anterior — ainda não tinham valor real ou tinham rateio)
--      - INSERE novos items a partir de products (com valores reais)
--      - RECALCULA valor_final e receita do card baseado nos novos items
--
-- Items manuais (product_type != 'custom') ficam intocados.
-- Sem nenhuma flag, comportamento legado preservado (cria items no CREATE,
-- não toca em items no UPDATE).
-- ============================================================================
...
DECLARE
    ...
    v_update_dates BOOLEAN;
    v_sync_monde_nums BOOLEAN;

    v_target_stage UUID;
    v_status_comercial TEXT;
    v_cpf_raw TEXT;
    v_precisa_cpf BOOLEAN;

    -- Novas flags pra controlar atualização financeira
    v_skip_financials BOOLEAN;
    v_update_fin_existing BOOLEAN;
    v_has_products BOOLEAN;

    c_stage_pos_viagem CONSTANT UUID := '2c07134a-cb83-4075-bc86-4750beec9393';
BEGIN
    ...
    LOOP
        BEGIN
            ...
            v_update_dates := COALESCE((v_trip->>'update_dates')::BOOLEAN, false);
            v_sync_monde_nums := COALESCE((v_trip->>'sync_monde_nums')::BOOLEAN, false);
            v_skip_financials := COALESCE((v_trip->>'skip_financials')::BOOLEAN, false);
            v_update_fin_existing := COALESCE((v_trip->>'update_financials_on_existing')::BOOLEAN, false);
            v_has_products := v_trip->'products' IS NOT NULL AND jsonb_array_length(v_trip->'products') > 0;
            ...
            IF v_existing_card_id IS NOT NULL THEN
                ...
                UPDATE cards SET
                    ...
                    END,
                    data_viagem_inicio = CASE
                        WHEN v_update_dates AND v_trip->>'data_viagem_inicio' IS NOT NULL
                        THEN (v_trip->>'data_viagem_inicio')::DATE
                        ELSE COALESCE(data_viagem_inicio, (v_trip->>'data_viagem_inicio')::DATE)
                    END,
                    data_viagem_fim = CASE
                        WHEN v_update_dates AND v_trip->>'data_viagem_fim' IS NOT NULL
                        THEN (v_trip->>'data_viagem_fim')::DATE
                        ELSE COALESCE(data_viagem_fim, (v_trip->>'data_viagem_fim')::DATE)
                    END,
                    updated_at = NOW(),
                ...
                IF v_sync_monde_nums THEN
                    ...
                    LOOP
                        v_historico := v_historico || jsonb_build_array(jsonb_build_object(
                            ...
                            'motivo', 'importacao_pos_venda_sync'
                        ));
                    END LOOP;

                    UPDATE cards SET
                        produto_data = COALESCE(v_existing_pd, '{}'::JSONB)
                            || jsonb_build_object('numeros_venda_monde_historico', v_historico)
                            || jsonb_build_object(
                                'numero_venda_monde',
                                CASE
                                    WHEN jsonb_array_length(v_trip->'venda_nums') > 0
                                    THEN v_trip->'venda_nums'->>0
                                    ELSE NULL
                                END
                            )
                    WHERE id = v_card_id;
                ...
                -- Atualização de items financeiros + valor_final/receita SOMENTE quando
                -- update_financials_on_existing=true E há products no payload (caso típico:
                -- 2º upload com relatório detalhado por produto).
                -- Apaga só items 'custom' (vieram da importação) — preserva items manuais.
                IF v_update_fin_existing AND v_has_products AND NOT v_skip_financials THEN
                    -- Apaga passageiros dos items custom existentes pra evitar FK órfã
                    DELETE FROM financial_item_passengers
                    WHERE financial_item_id IN (
                        SELECT id FROM card_financial_items
                        WHERE card_id = v_card_id AND product_type = 'custom'
                    );
                    DELETE FROM card_financial_items
                    WHERE card_id = v_card_id AND product_type = 'custom';
                    ...
                    -- Recalcula valor_final/receita do card pelos items totais
            ...
            ELSE
                -- ─── PATH B: CREATE ─────────────────────────────────────────
                v_target_stage := (v_trip->>'pipeline_stage_id')::UUID;

                IF v_target_stage = c_stage_pos_viagem THEN
                    v_status_comercial := 'ganho';
                ELSE
                    v_status_comercial := 'aberto';
                END IF;

                v_cpf_raw := NULLIF(TRIM(COALESCE(v_trip->>'cpf_raw', '')), '');
                v_precisa_cpf := (v_cpf_raw IS NULL);
                ...
                IF v_contato_id IS NULL THEN
                    ...
                    VALUES (COALESCE(v_nome, v_trip->>'pagante_nome'), v_sobrenome, v_cpf_raw, p_created_by, 'monde')
                ...
                INSERT INTO cards (
                    ...
                    jsonb_build_object(
                        ...
                        'precisa_cpf', v_precisa_cpf,
                        'precisa_valores', v_skip_financials,
                ...
                -- Items financeiros: pula se skip_financials=true (planilha agregada sem
                -- valor — vai ser preenchido depois pela detalhada). Senão, cria normal.
                IF NOT v_skip_financials AND v_has_products THEN
                    FOR v_product IN SELECT * FROM jsonb_array_elements(v_trip->'products')
                    LOOP
                        INSERT INTO card_financial_items (
                            card_id, product_type, description, sale_value, supplier_cost,
                ...
                IF v_precisa_cpf THEN
                    INSERT INTO tarefas (
                        ...
                        v_card_id,
                        'Atualizar CPF do contato principal',
                        'tarefa',
                        'alta',
                        false, NULL,
                        (v_trip->>'pos_owner_id')::UUID,
                ...
                -- Tarefa quando o card foi criado SEM valores (relatório detalhado vai
                -- atualizar depois). Permite o user verificar no Kanban.
                IF v_skip_financials THEN
                    INSERT INTO tarefas (
                        card_id, titulo, tipo, prioridade,
                        concluida, concluida_em,
                        responsavel_id, created_by
                        ...
                        v_card_id,
                        'Atualizar valores e receita (planilha detalhada)',
                        'tarefa',
                        'media',
                        false, NULL,
                        (v_trip->>'pos_owner_id')::UUID,
                ...
                -- Recalcula valor_final/receita SOMENTE quando há items (skip_financials=false).
                -- Quando skip_financials=true, mantém os valores que vieram no INSERT (zeros).
                IF NOT v_skip_financials THEN
                    SELECT COALESCE(SUM(sale_value), 0), COALESCE(SUM(supplier_cost), 0)
                    INTO v_total_venda, v_total_custo FROM card_financial_items WHERE card_id = v_card_id;
                    v_receita := v_total_venda - v_total_custo;
