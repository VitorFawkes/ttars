-- 20260612c — tipo do casal: campo 30 > jornada > "Apenas o casal" > DW
--
-- PROBLEMA (achado pelo Vitor): "Lead ideal × Pipeline" filtrado por DW mostrava
-- leads novos com "Apenas o casal" — tipo era binário com default DW: lead ainda
-- não classificado (campo 30 vazio, nunca entrou na esteira Elopment 12) contava
-- como DW. "DW" significava "DW + não classificado".
--
-- EVIDÊNCIA: de quem declarou "Apenas o casal" e FECHOU, 15/16 viraram Elopement.
-- Contra-prova: de 141 ganhos que declararam convidados, só 1 virou Elopement.
-- O único ganho "Apenas o casal"→DW (Victoria e Marco Aurélio) tem campo 30 = 'DW
-- com convidados' → regra do campo explícito o mantém DW (zero erro nos ganhos).
--
-- FIX: tipo_final = campo 30 explícito ('Elopment Wedding'/'DW com convidados')
-- > jornada/pipeline Elopment (is_elo) > declarou 'Apenas o casal' (Elopement)
-- > default DW. is_elopement alinhado ao tipo_final. ~695 casais (1 ganho=0 após
-- regra do campo, 667 perdidos) migram de DW para Elopement.
--
-- DILIGÊNCIA (TOP-5 #5): cadeia 20260602j→k→20260604e/f→b→20260612a→20260612b.
-- Base = DEFINIÇÃO VIVA de prod (pg_get_functiondef 2026-06-12, com 612a+612b);
-- muda APENAS a derivação do tipo (cf.campo30 + tipo_final no j + SELECT final).
-- Aplicar em 3 statements separados (promote-to-prod engole erro de multi-statement
-- com $function$ — ver 20260612b).
