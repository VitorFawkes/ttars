-- ============================================================================
-- Copiar acompanhantes (cards_contatos) do card pai ao criar sub-card
--
-- Problema: criar_sub_card copiava pessoa_principal_id mas não copiava os
-- acompanhantes da tabela cards_contatos. Sub-cards de adição de produto
-- precisam das mesmas pessoas do card original.
--
-- Solução: Adicionar INSERT INTO cards_contatos SELECT após criar o sub-card.
-- Exclui pessoa_principal_id para não violar trigger enforce_single_role.
-- ============================================================================

BEGIN;

-- 1. Dropar overloads existentes
...
-- 2. Recriar com cópia de acompanhantes
...
BEGIN
    ...
    -- Exclui pessoa_principal_id: trigger enforce_single_role_cards_contatos
    -- bloqueia inserir quem já é contato principal do card
    INSERT INTO cards_contatos (card_id, contato_id, tipo_viajante, ordem, tipo_vinculo, created_at)
    SELECT v_new_card_id, contato_id, tipo_viajante, ordem, tipo_vinculo, now()
    FROM cards_contatos
