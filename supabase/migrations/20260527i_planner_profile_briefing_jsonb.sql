-- Fix v4 (final): briefing_inicial é JSONB, não TEXT. Comparar `briefing_inicial = ''`
-- causa "invalid input syntax for type json" porque PostgreSQL tenta parsear ''
-- como JSON. Substitui por checagem `IS NULL OR briefing_inicial = '{}'::JSONB`.
-- Preserva todas as correções de `h` (sem `?` operator, regex guard, string compare ISO).
