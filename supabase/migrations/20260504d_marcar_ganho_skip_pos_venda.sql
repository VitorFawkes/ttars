-- ============================================================================
-- MIGRATION: marcar_ganho — Ganho sem Pós-Venda agora MOVE pra fase pos_venda
-- Date: 2026-05-04
--
-- Mudança de regra (alinhamento com Vitor 2026-05-04):
--
-- ANTES:
--   p_skip_pos_venda=true → card fica na fase planner com status='ganho'.
--
-- DEPOIS:
--   p_skip_pos_venda=true → card vai pra fase pos_venda na ETAPA CORRETA pela
--   data da viagem (via fn_calcular_etapa_pos_venda). status='ganho' e
--   skip_pos_venda=true permitem distinguir do fluxo normal.
--
-- Regras pra cards skip_pos_venda=true:
--   - SEM pos_owner_id (ninguém da operação cuida)
--   - dono_atual_id permanece com vendas_owner_id (rastreabilidade)
--   - Cadências/automações de pós-venda NÃO disparam
--   - Cron continua avançando entre etapas pela data da viagem
--   - Reversível via ativar_pos_venda
-- ============================================================================
...
BEGIN
    ...
    ELSIF v_phase_slug = 'planner' THEN
        ...
        IF p_skip_pos_venda THEN
            -- ─── GANHO SEM PÓS-VENDA (NOVA REGRA): move pra fase pos_venda
            -- na etapa correta pela data da viagem, marca skip_pos_venda=true
            ...
            UPDATE cards SET
                ...
                stage_entered_at = NOW(),
                -- mantém dono_atual_id com quem está cuidando (vendas/planner)
                dono_atual_id = COALESCE(p_novo_dono_id, vendas_owner_id, dono_atual_id),
                -- NÃO atribui pos_owner_id (sem responsável Pós-Venda)
                updated_at = NOW()
            WHERE id = p_card_id;
