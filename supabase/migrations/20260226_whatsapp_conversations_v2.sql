-- =====================================================
-- Analytics WhatsApp Conversations V2 — Add search
--
-- Adds p_search parameter for filtering by contact
-- name or phone number (ILIKE).
-- =====================================================
...
CREATE OR REPLACE FUNCTION analytics_whatsapp_conversations(
  ...
  p_search    TEXT DEFAULT NULL   -- Search by name or phone (ILIKE)
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Summary KPIs (over ALL convos, not filtered — so search doesn't distort KPIs)
  summary_base AS (
    SELECT * FROM convos
  ),
  summary AS (
