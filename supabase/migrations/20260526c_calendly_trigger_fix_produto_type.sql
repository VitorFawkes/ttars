-- Fix: column "produto" em cards é do tipo enum app_product, não text.
-- A versão anterior fazia SELECT produto::TEXT e depois INSERT INTO cards (produto)
-- com o text, o que quebra. Causa eventos calendly_invitee_created ficarem
-- presos em processed_status='pending' (UPDATE reverte por causa do erro no trigger).
...
DECLARE
  ...
  v_pipeline_id          UUID;
  v_stage_id             UUID;
  v_produto              app_product;  -- corrigido: tipo enum
  v_pending_count        INT;
  v_card_title           TEXT;
  v_owner_mode           TEXT;
...
BEGIN
  ...
  LOOP
    ...
    ELSIF coalesce((v_trigger.event_config->>'create_card_if_missing')::BOOLEAN, FALSE) THEN
      ...
      -- Pega produto direto do pipeline (sem cast pra text)
