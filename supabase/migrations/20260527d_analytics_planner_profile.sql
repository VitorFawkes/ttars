-- analytics_planner_profile: perfil completo de um Travel Planner (Sprint 4).
-- Retorna JSONB com 5 blocos pro drawer "Perfil do TP":
--   A. AGORA       — snapshot do que ele tem em mãos
--   B. PERIODO     — performance no range filtrado + comparativo com time
--   C. PREENCHIMENTO — % cards dele sem briefing/contato/parados
--   D. MOTIVOS     — top motivos de perda DELE (não do time)
--   E. ORIGEM + FORECAST — % origem dos leads dele + valor previsto fechar
--
-- Filtros: vendas_owner_id = p_user_id (regra G: nunca dono_atual_id em tela Planner).
-- SECURITY DEFINER, escopo por requesting_org_id().
-- Nada de threshold hardcoded — só números crus, frontend faz rank/comparativo.

CREATE OR REPLACE FUNCTION public.analytics_planner_profile(
    p_user_id    UUID,
...
DECLARE
    ...
    v_prox_30d DATE := CURRENT_DATE + 30;
BEGIN
    WITH
    -- Base: cards onde o user é vendas_owner (TP) no range temporal das métricas históricas
    cards_pessoa AS (
    ...
    -- Cards do time inteiro (qualquer planner) — pra calcular médias do time
    cards_time AS (
        SELECT c.id, c.status_comercial, c.data_fechamento,
               c.created_at, c.ganho_sdr_at, c.vendas_owner_id
        FROM cards c
        WHERE c.org_id = v_org
    ...
    -- ─── BLOCO A: AGORA (snapshot, sem range temporal) ───
    cards_abertos AS (
        SELECT cp.*
    ...
    -- ─── BLOCO B: PERIODO + comparativo time ───
    cards_pessoa_periodo AS (
        SELECT cp.*,
               EXTRACT(EPOCH FROM (
                   cp.data_fechamento - COALESCE(cp.ganho_sdr_at, cp.created_at)
               )) / 86400.0 AS dias_ciclo
        FROM cards_pessoa cp
    ...
    kpis_periodo AS (
        SELECT
            ...
            COALESCE(
                ...
                FILTER (WHERE status_comercial = 'ganho'), 0)::NUMERIC AS dias_ate_ganho,
            COALESCE(
                PERCENTILE_CONT(0.9) WITHIN GROUP (ORDER BY dias_ciclo)
                FILTER (WHERE status_comercial = 'ganho'), 0)::NUMERIC AS dias_ate_ganho_pior,
            COALESCE(
                PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY dias_ciclo)
                FILTER (WHERE status_comercial = 'perdido'), 0)::NUMERIC AS dias_ate_perda
        FROM cards_pessoa_periodo
    ),
    cards_time_periodo AS (
        SELECT ct.*,
               EXTRACT(EPOCH FROM (
                   ct.data_fechamento - COALESCE(ct.ganho_sdr_at, ct.created_at)
               )) / 86400.0 AS dias_ciclo
        FROM cards_time ct
    ...
    kpis_time AS (
        SELECT
            CASE WHEN (COUNT(*) FILTER (WHERE status_comercial = 'ganho')
                       + COUNT(*) FILTER (WHERE status_comercial = 'perdido')) > 0
                THEN ROUND(
                    COUNT(*) FILTER (WHERE status_comercial = 'ganho')::NUMERIC
                    / (COUNT(*) FILTER (WHERE status_comercial = 'ganho')
                       + COUNT(*) FILTER (WHERE status_comercial = 'perdido'))::NUMERIC
                    * 100, 1)
                ELSE 0 END::NUMERIC AS win_rate_team,
            COALESCE(
                PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY dias_ciclo)
                FILTER (WHERE status_comercial = 'ganho'), 0)::NUMERIC AS dias_ate_ganho_team
        FROM cards_time_periodo
    ),

    -- ─── BLOCO C: PREENCHIMENTO (dos cards abertos) ───
    preenchimento AS (
        SELECT
            COUNT(*) FILTER (
                WHERE briefing_inicial IS NULL OR briefing_inicial = ''
            )::INT AS sem_briefing,
    ...
    -- ─── BLOCO D: MOTIVOS DE PERDA (top 5 dela, no range) ───
    motivos AS (
    ...
    forecast AS (
        SELECT
            COUNT(*) FILTER (
                WHERE (cp.produto_data->>'data_prevista_fechamento')::DATE
                      BETWEEN v_today AND v_prox_7d
            )::INT AS prox_7d_qtd,
            ...
            COUNT(*) FILTER (
                WHERE (cp.produto_data->>'data_prevista_fechamento')::DATE
                      BETWEEN v_today AND v_prox_30d
            )::INT AS prox_30d_qtd,
            COALESCE(SUM(COALESCE(cp.valor_estimado, cp.valor_final, 0)) FILTER (
                WHERE (cp.produto_data->>'data_prevista_fechamento')::DATE
                      BETWEEN v_today AND v_prox_30d
        ...
        WHERE cp.produto_data IS NOT NULL
          AND cp.produto_data->>'data_prevista_fechamento' IS NOT NULL
          AND cp.produto_data->>'data_prevista_fechamento' != ''
    ),

    -- ─── HEADER: nome + foto + posição no ranking + delta vs time ───
    membro AS (
    ...
    ranking AS (
        SELECT
            ct.vendas_owner_id,
            ROW_NUMBER() OVER (
                ORDER BY COALESCE(SUM(c2.receita) FILTER (WHERE c2.status_comercial = 'ganho'), 0) DESC
            ) AS posicao
        FROM cards_time ct
        LEFT JOIN cards c2 ON c2.id = ct.id AND c2.status_comercial = 'ganho'
                          AND c2.data_fechamento >= p_date_start
                          AND c2.data_fechamento < p_date_end
    ...
    SELECT JSONB_BUILD_OBJECT(
        ...
        'agora', JSONB_BUILD_OBJECT(
            ...
            'delta_semana', COALESCE((SELECT qtd FROM atendimentos_semana_atual), 0)
                          - COALESCE((SELECT qtd FROM atendimentos_semana_anterior), 0),
            'por_etapa', COALESCE(
                (SELECT JSONB_AGG(
                    JSONB_BUILD_OBJECT(
                        'stage_id', stage_id,
        ...
        'periodo', JSONB_BUILD_OBJECT(
            ...
            'win_rate', CASE
                ...
                THEN ROUND(
                    ...
                    / ((SELECT ganhos FROM kpis_periodo) + (SELECT perdidos FROM kpis_periodo))::NUMERIC
                    * 100, 1)
                ELSE 0
            END,
...
COMMENT ON FUNCTION public.analytics_planner_profile IS
'Retorna JSON com perfil completo de um Travel Planner: AGORA, PERIODO + comparativo time, PREENCHIMENTO, MOTIVOS de perda, ORIGEM dos leads, FORECAST 7d/30d. Usado pelo PlannerProfileDrawer (Sprint 4 do plano analytics).';
