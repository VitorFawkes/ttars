-- ============================================================
-- MIGRATION: Fix WhatsApp Message Status Updates
-- Date: 2026-03-03
--
-- Problema:
-- Eventos message.status do Echo (delivered/read/failed) são
-- recebidos e salvos em whatsapp_raw_events mas NÃO atualizam
-- o campo status/has_error/error_message em whatsapp_messages.
-- Resultado: todas as mensagens ficam com status='delivered'
-- (default) e falhas de envio nunca são exibidas na UI.
--
-- Fix:
-- Adiciona short-circuit em process_whatsapp_raw_event_v2 para
-- eventos message.status e message.sent: em vez de rodar o
-- pipeline completo (contato/card/insert), apenas faz UPDATE
-- no status da mensagem existente via external_id.
--
-- Campos atualizados:
--   status      → status_name do payload (sent/delivered/read/failed)
--   has_error   → error do payload (boolean)
--   error_message → error_message do payload (text)
-- ============================================================
...
DECLARE
    ...
    v_message_type TEXT;
    v_media_url TEXT;
    -- NOVO: variáveis para status update
    v_msg_status TEXT;
...
BEGIN
    ...
    -- message.status (delivered/read/failed) and message.sent
    -- apenas atualizam status da mensagem existente, sem rodar
    -- o pipeline completo de contato/card/insert.
    -- ========================================
    IF v_platform.provider = 'echo' AND (v_payload->>'event') = 'message.status' AND v_external_id IS NOT NULL THEN
        ...
        -- Atualizar mensagem existente pelo external_id
        ...
        -- Marcar raw event como processado
