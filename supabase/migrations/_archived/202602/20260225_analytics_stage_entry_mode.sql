-- ============================================================
-- Analytics: Stage Entry Mode
-- Adds p_stage_id UUID DEFAULT NULL to 13 RPCs
-- When p_mode = 'stage_entry' AND p_stage_id IS NOT NULL,
-- filters cards by when they entered a specific pipeline stage.
-- Existing activity/cohort modes are UNCHANGED.
-- ============================================================

-- ============================================================
-- 1) Performance Indexes
-- ============================================================
CREATE INDEX IF NOT EXISTS idx_activities_stage_changed_new
  ON activities ((metadata->>'new_stage_id'), created_at)
  WHERE tipo = 'stage_changed';

CREATE INDEX IF NOT EXISTS idx_activities_stage_changed_old
  ON activities ((metadata->>'old_stage_id'))
  WHERE tipo = 'stage_changed';

-- ============================================================
-- 2) Helper Function: get_card_ids_by_stage_entry
-- Returns card_ids that entered a specific stage during a date range
-- ============================================================
CREATE OR REPLACE FUNCTION get_card_ids_by_stage_entry(
    p_stage_id   UUID,
...
BEGIN
    ...
    -- Scenario 1: Cards that TRANSITIONED into the stage via stage_changed activity
    ...
    -- Scenario 2: Cards CREATED directly in the stage (1st stage, no stage_changed activity)
    SELECT c.id
    FROM cards c
    WHERE c.created_at >= p_date_start
      AND c.created_at < p_date_end
      AND c.deleted_at IS NULL
      AND c.archived_at IS NULL
      AND (p_product IS NULL OR c.produto::TEXT = p_product)
      AND (
          -- 2a: Card never moved (still in creation stage)
          (c.pipeline_stage_id = p_stage_id
           AND NOT EXISTS (
               SELECT 1 FROM activities a
               WHERE a.card_id = c.id AND a.tipo = 'stage_changed'
           ))
          OR
          -- 2b: Card was moved — detect initial stage via first transition's old_stage_id
...
GRANT EXECUTE ON FUNCTION get_card_ids_by_stage_entry TO authenticated;

-- ============================================================
-- 3) Drop old function signatures (all possible overloads)
-- ============================================================
-- Group A: TIMESTAMPTZ-based RPCs (from mode_toggle migration)
DROP FUNCTION IF EXISTS analytics_overview_kpis(TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT);
DROP FUNCTION IF EXISTS analytics_revenue_timeseries(TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS analytics_team_performance(TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS analytics_funnel_conversion(TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT);
DROP FUNCTION IF EXISTS analytics_loss_reasons(TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT);

-- Group A: DATE-based RPCs (from mode_toggle + filters_fix migrations)
DROP FUNCTION IF EXISTS analytics_financial_breakdown(DATE, DATE, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS analytics_top_destinations(DATE, DATE, INT, TEXT);
DROP FUNCTION IF EXISTS analytics_top_destinations(DATE, DATE, INT, TEXT, TEXT);
DROP FUNCTION IF EXISTS analytics_revenue_by_product(DATE, DATE, TEXT);
DROP FUNCTION IF EXISTS analytics_revenue_by_product(DATE, DATE, TEXT, TEXT);
DROP FUNCTION IF EXISTS analytics_operations_summary(DATE, DATE, TEXT, TEXT);

-- Group B: from filters_fix migration
DROP FUNCTION IF EXISTS analytics_funnel_live(TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT);
DROP FUNCTION IF EXISTS analytics_sla_violations(TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT, INT);
DROP FUNCTION IF EXISTS analytics_sla_summary(TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT);

-- Group C: WhatsApp
DROP FUNCTION IF EXISTS analytics_whatsapp_metrics(DATE, DATE, TEXT);
DROP FUNCTION IF EXISTS analytics_whatsapp_metrics(DATE, DATE);

-- ============================================================
-- 4) Recreate all RPCs with p_stage_id parameter
-- ============================================================

-- ── 4.1 analytics_overview_kpis ──────────────────────────────
...
-- ── 4.2 analytics_revenue_timeseries ─────────────────────────
...
-- ── 4.3 analytics_team_performance ───────────────────────────
...
BEGIN
    ...
    INNER JOIN cards c ON c.sdr_owner_id = p.id
        ...
        AND CASE
            WHEN p_mode = 'stage_entry' AND p_stage_id IS NOT NULL THEN
                c.id IN (SELECT card_id FROM get_card_ids_by_stage_entry(p_stage_id, p_date_start, p_date_end, p_product))
            WHEN p_mode = 'cohort' THEN c.created_at >= p_date_start AND c.created_at < p_date_end
            WHEN c.data_fechamento IS NOT NULL THEN c.data_fechamento::TIMESTAMPTZ >= p_date_start AND c.data_fechamento::TIMESTAMPTZ < p_date_end
...
-- ── 4.4 analytics_funnel_conversion ──────────────────────────
...
-- ── 4.6 analytics_financial_breakdown ────────────────────────
      ...
      AND CASE
          WHEN p_mode = 'stage_entry' AND p_stage_id IS NOT NULL THEN
              c.id IN (SELECT card_id FROM get_card_ids_by_stage_entry(
                  p_stage_id,
                  COALESCE(p_date_start, '2020-01-01'::DATE)::TIMESTAMPTZ,
                  COALESCE(p_date_end + 1, '2099-01-01'::DATE)::TIMESTAMPTZ,
                  p_product))
          WHEN p_mode = 'cohort' THEN
...
-- ── 4.7 analytics_top_destinations ───────────────────────────
...
-- ── 4.8 analytics_revenue_by_product ─────────────────────────
...
-- ── 4.9 analytics_operations_summary ─────────────────────────
...
BEGIN
    ...
    sub_cards AS (
        SELECT sc.parent_card_id, sc.id, sc.sub_card_mode, sc.sub_card_status, sc.created_at, sc.updated_at
        FROM cards sc
...
-- ── 4.10 analytics_funnel_live ───────────────────────────────
-- Note: This RPC only shows OPEN cards. Both activity and cohort use created_at (correct).
...
-- ── 4.11 analytics_sla_violations ────────────────────────────
-- Note: Only OPEN cards. Both activity and cohort use created_at (correct).
...
-- ── 4.13 analytics_whatsapp_metrics ──────────────────────────
-- Note: Adding p_mode + p_stage_id. In stage_entry mode, filters messages
-- to only those from cards in the cohort. In other modes, no card filter.
...
BEGIN
    WITH msg_base AS (
        ...
        WHERE m.canal = 'whatsapp'
          ...
          AND CASE
              WHEN p_mode = 'stage_entry' AND p_stage_id IS NOT NULL THEN
                  m.card_id IN (SELECT card_id FROM get_card_ids_by_stage_entry(
                      p_stage_id, v_start, v_end, p_product))
              ELSE TRUE
          END
    ),
    volume AS (
