-- ============================================================
-- Analytics: Stage Entry Mode v2 — Fix funnel + fallback
--
-- Changes from v1:
-- 1. Funnel RPCs (funnel_live, funnel_conversion): NEW path for
--    generic stage_entry (stageId=NULL) that counts ENTRIES per stage
-- 2. All other RPCs: When stage_entry + stageId=NULL, fallback to
--    activity mode behavior (data_fechamento / created_at)
-- ============================================================

-- ── 1. analytics_funnel_live (Overview funnel) ─────────────────
-- Generic stage_entry: count entries per stage
-- Specific stage_entry: show cohort distribution across stages
...
AS $$
BEGIN
    IF p_mode = 'stage_entry' AND p_stage_id IS NULL THEN
        -- GENERIC: Count how many cards ENTERED each stage during the period
        RETURN QUERY
    ...
    ELSE
        -- SPECIFIC stage_entry, activity, cohort: current distribution of OPEN cards
        RETURN QUERY
        SELECT
            s.id            AS stage_id,
            s.nome          AS stage_nome,
...
BEGIN
    IF p_mode = 'stage_entry' AND p_stage_id IS NULL THEN
        -- GENERIC: Count entries per stage during the period
        RETURN QUERY
        WITH transition_entries AS (
            SELECT
                (a.metadata->>'new_stage_id')::UUID AS entered_stage_id,
        ...
        ORDER BY pp.order_index, s.ordem;
    ELSE
        -- SPECIFIC stage_entry, activity, cohort: current distribution
        RETURN QUERY
        SELECT
            s.id AS stage_id,
            s.nome AS stage_nome,
...
-- ── 3. analytics_overview_kpis — add fallback for stageId=NULL ──
        ...
        WHERE c.deleted_at IS NULL
          ...
          AND CASE
              ...
              WHEN p_mode = 'cohort' THEN
                  c.created_at >= p_date_start AND c.created_at < p_date_end
              ELSE
                  -- activity mode OR stage_entry with stageId=NULL (fallback)
...
-- ── 4. analytics_revenue_timeseries — fallback ─────────────────
    ...
    WHERE c.deleted_at IS NULL
      ...
      AND CASE
          ...
          WHEN p_mode = 'cohort' THEN
              c.created_at >= p_date_start AND c.created_at < p_date_end
          ELSE
              -- activity OR stage_entry with stageId=NULL
...
-- ── 5. analytics_team_performance — fallback ───────────────────
...
-- ── 6. analytics_loss_reasons — fallback ───────────────────────
...
-- ── 7. analytics_financial_breakdown — fallback ────────────────
...
CREATE OR REPLACE FUNCTION analytics_financial_breakdown(
    ...
    p_mode TEXT DEFAULT 'activity', p_stage_id UUID DEFAULT NULL
)
...
-- ── 8. analytics_top_destinations — fallback ───────────────────
...
CREATE OR REPLACE FUNCTION analytics_top_destinations(
    p_date_start DATE DEFAULT NULL, p_date_end DATE DEFAULT NULL,
    p_limit INT DEFAULT 10, p_mode TEXT DEFAULT 'activity',
...
-- ── 9. analytics_revenue_by_product — fallback ─────────────────
...
CREATE OR REPLACE FUNCTION analytics_revenue_by_product(
    p_date_start DATE DEFAULT NULL, p_date_end DATE DEFAULT NULL,
    p_mode TEXT DEFAULT 'activity', p_product TEXT DEFAULT NULL,
    p_stage_id UUID DEFAULT NULL
)
...
-- ── 10. analytics_operations_summary — fallback ────────────────
...
CREATE OR REPLACE FUNCTION analytics_operations_summary(
    p_date_start DATE DEFAULT NULL, p_date_end DATE DEFAULT NULL,
    p_product TEXT DEFAULT NULL, p_mode TEXT DEFAULT 'activity',
    p_stage_id UUID DEFAULT NULL
)
    ...
    WITH won_cards AS (
        ...
        WHERE c.status_comercial = 'ganho' AND c.deleted_at IS NULL AND c.archived_at IS NULL
          AND CASE
              ...
              ELSE
                  c.data_fechamento >= v_start AND c.data_fechamento <= v_end
          END
          AND (p_product IS NULL OR c.produto::TEXT = p_product)
    ),
    kpis AS (
...
-- ── 11. analytics_sla_violations — fallback ────────────────────
...
-- ── 12. analytics_sla_summary — fallback ───────────────────────
...
-- ── 13. analytics_whatsapp_metrics — fallback ──────────────────
