-- ============================================================
-- Analytics Multi-Filter — p_owner_ids UUID[] em todas as RPCs
--
-- Adiciona p_owner_ids UUID[] DEFAULT NULL como último parâmetro
-- em todas as 14 RPCs de analytics. Quando p_owner_ids é
-- fornecido e não-vazio, tem precedência sobre p_owner_id.
--
-- Helper: _a_owner_ok(actual_id, single_id, arr_ids)
-- ============================================================

-- ── Helper ───────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION _a_owner_ok(
    actual_id UUID, single_id UUID, arr_ids UUID[]
) RETURNS BOOLEAN
LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
SELECT CASE
    WHEN arr_ids IS NOT NULL AND array_length(arr_ids, 1) > 0 THEN actual_id = ANY(arr_ids)
    WHEN single_id IS NOT NULL THEN actual_id = single_id
    ELSE TRUE
END;
$$;

-- ── Drop ALL overloads of every analytics function ──────────
DO $$
DECLARE
    fn_names TEXT[] := ARRAY[
...
DECLARE
    ...
    v_viagem_id UUID;
BEGIN
    -- Buscar IDs dos stages milestone (via milestone_key dinâmico)
...
BEGIN
    WITH msg_base AS (
        ...
        WHERE m.canal = 'whatsapp'
          ...
          AND (p_owner_id IS NULL AND (p_owner_ids IS NULL OR array_length(p_owner_ids, 1) = 0) OR EXISTS (
              SELECT 1 FROM cards c WHERE c.id = m.card_id AND _a_owner_ok(c.dono_atual_id, p_owner_id, p_owner_ids) AND c.deleted_at IS NULL
          ))
          AND CASE
              WHEN p_mode = 'stage_entry' AND p_stage_id IS NOT NULL THEN
...
-- Grants
-- ============================================================
GRANT EXECUTE ON FUNCTION _a_owner_ok TO authenticated;
