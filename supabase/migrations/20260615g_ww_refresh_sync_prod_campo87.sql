-- 20260615g_ww_refresh_sync_prod_campo87.sql
-- RECONCILIAÇÃO: materializa no disco a definição VIVA de produção de
-- refresh_ww_funil_casal(), que JÁ contém a lógica de ganho por campo 87 (AC field 87)
-- desde 2026-01-01 (promovida em 2026-06-15 via 20260615b_ww_ganho_campo87_desde_2026.sql,
-- que vive na branch fix/ww-visao-geral-grafico-periodo-fases e não estava nesta branch).
--
-- POR QUÊ: o disco desta branch tinha a versão ANTIGA (ganho = entrada no Planejamento).
-- Sem esta migration, um refresh futuro REGREDIRIA produção (maio/2026: 5 → 6). Esta migration
-- NÃO muda produção (a função viva já é esta); só alinha disco↔prod.
