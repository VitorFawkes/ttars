-- ============================================================================
-- Analytics-Weddings v2 — ww_v2_lead_ideal (Perfil do Lead Ideal × Pipeline atual)
--
-- Compara:
--   HISTÓRICO = perfil dos 150 casamentos FECHADOS no AC (fonte:
--               ww_v2_casamentos_cache, universo lógica weddings-kpi.vercel.app)
--   ATUAL    = perfil dos leads NOVOS no CRM dentro do período p_atual_start..end
--
-- Diferenças vs v1 (que usava cards.produto_data com universo de 69 cards):
--   - Histórico agora vem do cache AC (150 vs 69 — 2.2× mais)
--   - Atual continua vindo de cards (leads novos é informação do CRM)
--
-- Contrato JSON IDÊNTICO ao ww_perfil_lead_ideal (compat com useWwLeadIdeal).
-- ============================================================================
...
BEGIN
    ...
    -- Histórico AC: como data_ganho ainda não populada no seed, ignoramos janela
    -- e usamos todo o cache (snapshot 28/05/2026). Quando sync incremental popular
    -- data_ganho, este filtro passa a ter efeito.
    ...
    -- HISTÓRICO: cache AC (150 fechados)
    CREATE TEMP TABLE _ww_v2_pli_h ON COMMIT DROP AS
    SELECT _ww2_norm_faixa_strict(entrada_invest)  AS faixa,
           _ww2_norm_dest_strict(entrada_destino)  AS destino,
           _ww2_norm_conv_strict(entrada_conv)     AS convidados
      FROM ww_v2_casamentos_cache
      WHERE (data_ganho IS NULL OR (data_ganho >= v_hist_start AND data_ganho <= v_hist_end));

    -- ATUAL: leads novos no CRM (mesma lógica v1)
    ...
    -- ── Comparação por dimensão (preservado v1) ──────────────────────────────
    WITH dims AS (
...
GRANT EXECUTE ON FUNCTION public.ww_v2_lead_ideal(TIMESTAMPTZ, TIMESTAMPTZ, UUID, TIMESTAMPTZ, TIMESTAMPTZ, INT, INT) TO authenticated;
