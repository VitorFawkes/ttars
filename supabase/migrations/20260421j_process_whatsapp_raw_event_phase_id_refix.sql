-- ============================================================
-- MIGRATION: process_whatsapp_raw_event_v2 — refix phase_id
-- Date: 2026-04-21
--
-- A migration 20260419_remove_fase_label redefiniu a função usando phase_id
-- (fase_label foi dropada). Mas a migration 20260420d_process_whatsapp_update_weak_name
-- (Marco A — atualização de nome fraco) foi feita a partir de uma versão
-- anterior do código, sem tomar o fix de 20260419. Resultado: 20260420d
-- reintroduziu `v_fase_label := v_linha_config.fase_label;` em produção, e
-- como a coluna foi dropada, TODA chamada explode com
-- "record v_linha_config has no field fase_label". Todos os raw_events dos
-- últimos dias estão com status=error — criação automática de contato/card
-- via Echo está quebrada em produção.
--
-- Este refix aplica a função do Marco A (atualização de nome fraco) COM
-- phase_id no lugar de fase_label. Mesmas 6 referências trocadas:
--   1. declare v_fase_label TEXT → v_phase_id UUID
--   2. v_fase_label := v_linha_config.fase_label → v_phase_id := v_linha_config.phase_id
--   3. INSERT columns: fase_label → phase_id
--   4. INSERT values: v_fase_label → v_phase_id
--   5. UPDATE conflict: fase_label → phase_id
--   6. RETURN jsonb: 'fase_label' → 'phase_id'
-- ============================================================
...
'Processa evento WhatsApp. Marco A (20260420d) — atualiza contatos.nome quando fraco e sender_name é válido (inbound only), respeitando nome_locked_at. Refix 20260421j — usa phase_id no lugar de fase_label (coluna foi dropada em 20260419_remove_fase_label e o Marco A havia reintroduzido referência à coluna inexistente).';
