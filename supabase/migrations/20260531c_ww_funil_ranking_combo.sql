-- ============================================================================
-- ww_funil_ranking_combo — ranking de perfis por taxa de fechamento, com
-- CRUZAMENTOS (1, 2 ou 3 dimensões: faixa | convidados | destino).
--
-- Supersede o ww_funil_ranking_perfil (single-dim) — generaliza pra N dimensões.
-- Espelha o pool/ganho do ww_funil_conversao_v1 (mesmos rótulos strict, mesma
-- definição de ganho) → os números batem com o funil e os rótulos casam com o
-- filtro do funil.
--
-- ORDENAÇÃO ROBUSTA (anti-fluke): em vez de ordenar pela taxa crua (que joga
-- combos de 1 caso a 100% no topo), ordena por uma TAXA SUAVIZADA (shrinkage):
--     score = (ganho + k*p0) / (entrou + k)   com k=15 e p0 = taxa geral do pool
-- Combos com pouca amostra são puxados pra perto da média geral, então um
-- "48% com 12 casos" fica acima de "100% com 1 caso". Mostra TODOS (a UI marca
-- "poucos casos"); a ordenação só evita que o ruído domine o topo.
-- ============================================================================

DROP FUNCTION IF EXISTS public.ww_funil_ranking_perfil(TIMESTAMPTZ, TIMESTAMPTZ, TEXT, UUID, TEXT, TEXT[], TEXT[], UUID[]);

CREATE OR REPLACE FUNCTION public.ww_funil_ranking_combo(
    p_date_start    TIMESTAMPTZ DEFAULT (NOW() - INTERVAL '90 days'),
    p_date_end      TIMESTAMPTZ DEFAULT NOW(),
    p_date_mode     TEXT        DEFAULT 'cohort',
    p_org_id        UUID        DEFAULT NULL,
    p_dimensoes     TEXT[]      DEFAULT ARRAY['faixa'],  -- subconjunto de faixa/convidados/destino
...
BEGIN
    ...
    -- Sanitiza dimensões (só aceita as 3 válidas; fallback faixa)
    ...
    -- Base do cruzamento: só leads com TODAS as dimensões selecionadas preenchidas.
    -- Taxa geral (p0) dessa base, pra suavização (shrinkage) da ordenação.
    ...
    -- Agrega por combinação das dimensões escolhidas; ordena por taxa suavizada.
    SELECT json_agg(
             json_build_object(
               'faixa', faixa, 'convidados', convidados, 'destino', destino,
               'label', label, 'entrou', entrou, 'ganho', ganho, 'taxa_pct', taxa_pct
    ...
    FROM (
        ...
        FROM (
            SELECT g_faixa, g_conv, g_dest,
                   COUNT(*) AS entrou, COUNT(*) FILTER (WHERE ganho) AS ganho
            FROM (
                SELECT
