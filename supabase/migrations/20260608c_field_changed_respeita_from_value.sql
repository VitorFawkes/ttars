-- Fix: o builder v2 (trigger.field_changed) deixa o usuário preencher
-- "De (opcional)" → event_config->>'from_value', mas
-- process_cadence_entry_on_card_field_change só filtrava por 'to_value'.
-- Resultado: o filtro de valor de origem era silenciosamente ignorado
-- (disparava para qualquer valor anterior).
--
-- Correção: adiciona cláusula simétrica ao to_value comparando from_value
-- contra v_old_value (já disponível na função). NULL/'' = sem filtro, então
-- triggers antigos (builder antigo grava só field+to_value) seguem inalterados.
--
-- Recriação preserva integralmente a lógica viva (campos monitorados, dedup,
-- phase_id, applicable_*, guard de org no template). Ver 20260526j.
    ...
    FOREACH v_field IN ARRAY v_changed_fields LOOP
        ...
        FOR v_trigger IN
            ...
            WHERE event_type = 'field_changed' AND is_active = true
              ...
              AND (
                event_config->>'from_value' IS NULL
                OR event_config->>'from_value' = ''
                OR event_config->>'from_value' = COALESCE(v_old_value, '')
              )
              AND (
