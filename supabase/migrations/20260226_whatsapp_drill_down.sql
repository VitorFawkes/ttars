-- =====================================================
-- WhatsApp Drill-down: Conversation Messages + Filters V3
--
-- 1. get_whatsapp_conversation_messages: chat history
-- 2. analytics_whatsapp_conversations V3: phase/stage filters
-- =====================================================

-- ── 1. Conversation Messages RPC ──

CREATE OR REPLACE FUNCTION get_whatsapp_conversation_messages(
  p_contact_id UUID,
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'card', CASE WHEN ca.id IS NOT NULL THEN jsonb_build_object(
      'id', ca.id,
      'titulo', ca.titulo,
      'stage_name', ps.nome,
      'phase_slug', pp.slug,
      'phase_label', pp.label
    ) ELSE NULL END,
    'messages', COALESCE((
      SELECT jsonb_agg(row_to_json ORDER BY created_at ASC)
      FROM (
        SELECT jsonb_build_object(
          'id', wm.id,
          'direction', wm.direction,
          'body', wm.body,
          'type', COALESCE(wm.type, wm.message_type, 'text'),
          'created_at', wm.created_at,
          'sender_name', wm.sender_name,
          'sent_by_user_name', wm.sent_by_user_name,
          'sent_by_user_id', wm.sent_by_user_id,
          'media_url', wm.media_url,
          'is_ai', (wm.direction = 'outbound' AND wm.sent_by_user_name IS NULL AND wm.sent_by_user_id IS NULL)
        ) AS row_to_json,
        wm.created_at
    ...
    'total_count', (SELECT count(*) FROM whatsapp_messages WHERE contact_id = p_contact_id)
  ) INTO result
  FROM contatos ct
  LEFT JOIN LATERAL (
    SELECT c.id, c.titulo, c.pipeline_stage_id
    FROM cards c
    WHERE c.pessoa_principal_id = p_contact_id
  ...
  RETURN COALESCE(result, jsonb_build_object(
    'contact', NULL, 'card', NULL, 'messages', '[]'::jsonb, 'total_count', 0
  ));
END;
$$;


-- ── 2. Conversations V3 with Phase/Stage filters ──
-- Drop old overloaded signatures (V1 and V2) to avoid ambiguity
...
BEGIN
  ...
  convos AS (
    ...
    GROUP BY m.contact_id, lm.last_direction, lm.last_message_at, fr.frt_minutes,
             ca.id, ca.titulo, ps.id, ps.nome, pp.slug, pp.label
  ),
  ...
  -- Summary KPIs from ALL convos (not distorted by search/phase/stage)
  summary AS (
    SELECT jsonb_build_object(
      ...
      'by_phase', COALESCE((
        ...
        FROM (
          ...
          FROM convos
          GROUP BY phase_slug, phase_label
        ) sub
      ), '[]'::jsonb)
    ) AS val FROM convos
  ),
