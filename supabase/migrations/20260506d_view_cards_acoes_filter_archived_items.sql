-- ============================================================================
-- MIGRATION: view_cards_acoes — ignorar card_financial_items arquivados
-- Date: 2026-05-06
--
-- Bug: Kanban exibia "X/Y produtos prontos" mesmo quando todos os itens haviam
-- sido arquivados (archived_at IS NOT NULL), enquanto o FinanceiroWidget no
-- detalhe do card filtra archived_at IS NULL e mostra "Nenhum produto cadastrado".
--
-- Caso reproduzível: card 0f9e5c2c-fc95-4851-9fbd-7cf8ed0256b1 — 12 itens
-- arquivados pelo trigger trg_archive_orphan_monde_items (reason
-- 'monde_venda_removida') ainda apareciam como "3/12" no Kanban.
--
-- Fix: adicionar WHERE archived_at IS NULL na CTE prd. Reproduz a definição
-- da view em 20260504j_view_cards_acoes_skip_pos_venda.sql, mudando apenas
-- a sub-query prods_total/prods_ready.
-- ============================================================================
