-- ============================================================
-- Analytics Modes v3 — Redesign completo dos modos de análise
--
-- 3 modos principais:
--   entries          → Entradas por Etapa (quantos entraram em cada stage)
--   cohort           → Coorte por data de criação
--   stage_entry      → Coorte por entrada em etapa específica (stageId obrigatório)
--   ganho_sdr        → Reverso: cohort ganho SDR
--   ganho_planner    → Reverso: cohort ganho Planner
--   ganho_total      → Reverso: cohort ganho Total (pós-venda)
--
-- Backward compat: 'activity' mapeia para ELSE (created_at)
-- ============================================================

-- ── 0. Helper: retorna stage IDs em que o card "está" no milestone ──
-- Ganho = card está na etapa do milestone OU em qualquer etapa posterior
-- (excl. Resolução/Fechado-Perdido, pois um card perdido não "está" no ganho)
CREATE OR REPLACE FUNCTION get_ganho_stage_ids(p_mode TEXT)
RETURNS SETOF UUID
...
AS $$
    ...
    WHERE s.ativo = true
      AND (s.is_lost IS NOT TRUE)
      AND (pp.order_index * 1000 + s.ordem) >= (
          SELECT pp2.order_index * 1000 + s2.ordem
          FROM pipeline_stages s2
          JOIN pipeline_phases pp2 ON pp2.id = s2.phase_id
          WHERE CASE
              WHEN p_mode = 'ganho_sdr' THEN s2.is_sdr_won = true
              WHEN p_mode = 'ganho_planner' THEN s2.is_planner_won = true
              WHEN p_mode = 'ganho_total' THEN s2.is_pos_won = true
          END
          LIMIT 1
      );
$$;

GRANT EXECUTE ON FUNCTION get_ganho_stage_ids TO authenticated;

-- Indexes para ganho fields (mantidos para uso futuro do trigger)
CREATE INDEX IF NOT EXISTS idx_cards_ganho_sdr_at
    ON cards (ganho_sdr_at) WHERE ganho_sdr = true AND deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_cards_ganho_planner_at
    ON cards (ganho_planner_at) WHERE ganho_planner = true AND deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_cards_ganho_pos_at
    ON cards (ganho_pos_at) WHERE ganho_pos = true AND deleted_at IS NULL;

-- ── 1. analytics_funnel_live (Overview funnel) ───────────────
-- 3 branches: entries, ganho, default
...
BEGIN
    ...
    ELSIF p_mode IN ('ganho_sdr', 'ganho_planner', 'ganho_total') THEN
        -- GANHO (reverso): distribuição ATUAL do cohort ganho
        -- SEM filtro de status — mostra ganhos, perdidos e abertos
        RETURN QUERY
        SELECT
            s.id            AS stage_id,
            s.nome          AS stage_nome,
    ...
    ELSE
        -- COHORT / STAGE_ENTRY (específico) / ACTIVITY: distribuição de cards ABERTOS
        RETURN QUERY
        SELECT
            s.id            AS stage_id,
            s.nome          AS stage_nome,
...
-- ── 2. analytics_funnel_conversion (Funnel view) ─────────────
-- 3 branches: entries, ganho, default
...
BEGIN
    ...
    ELSIF p_mode IN ('ganho_sdr', 'ganho_planner', 'ganho_total') THEN
        -- GANHO (reverso): distribuição ATUAL sem filtro de status
        RETURN QUERY
        SELECT
            s.id AS stage_id,
            s.nome AS stage_nome,
...
-- Duas pools (leads + outcomes) com filtro canônico unificado
...
-- GROUP BY sempre por data_fechamento, filtro canônico no WHERE
...
-- Filtro canônico em cada UNION ALL (SDR/Vendas/Pós)
...
-- Para ganho modes, filtra mensagens de cards no cohort ganho
