-- ============================================================
-- Product Isolation — Filtrar pipeline_stages por produto
--
-- Problema: RPCs que listam stages fazem FROM pipeline_stages
-- sem filtrar por pipeline/produto. Com WEDDING + TRIPS,
-- stages duplicados aparecem nos gráficos de funil.
--
-- Fix: JOIN pipelines pip ON pip.id = s.pipeline_id
--      AND (p_product IS NULL OR pip.produto = p_product)
--
-- RPCs afetadas:
--   1. analytics_funnel_live (2 blocos)
--   2. analytics_funnel_conversion (2 blocos)
--   3. analytics_sla_summary (1 bloco + add p_tag_ids)
--   4. analytics_funnel_by_owner (2 blocos)
--   5. analytics_overview_kpis (milestone lookups)
-- ============================================================
...
BEGIN
    ...
    IF p_product IS NOT NULL THEN
        SELECT p.id INTO v_pipeline_id FROM pipelines p WHERE p.produto = p_product LIMIT 1;
    END IF;
