-- ============================================================================
-- ww_funil_perfil_slot v2 — perfil dos ganhos DENTRO do recorte
--
-- Mudanças vs v1 (20260528j):
--   - top_combos calculado SEMPRE (não só quando populacao='ganhos')
--   - Adiciona perfil_ganhos: { faixa: [...], convidados: [...], destino: [...] }
--     com top 5 buckets de cada dimensão, calculado APENAS sobre cards do
--     recorte (slot) cujo ganho=TRUE. Permite a UI mostrar "tipo de lead que
--     ganhou neste recorte" sem que o slot precise filtrar por populacao=ganhos.
--   - Aceita p_meses TEXT[] — quando preenchido, override do range de data
--     com OR semântico (YYYY-MM strings, comparado com to_char(created_at OR ganho_at, 'YYYY-MM')).
--     Mantém p_date_start/end como fallback quando p_meses é null.
-- ============================================================================
...
CREATE FUNCTION public.ww_funil_perfil_slot(
    ...
    p_meses        TEXT[]      DEFAULT NULL  -- ex: ARRAY['2024-01','2024-12'] (YYYY-MM)
)
RETURNS JSON
...
BEGIN
    ...
    -- ── Filtros por população + período (com suporte p_meses) ─────────────
    ...
    -- ── Top combos (sempre que houver ganhos no recorte) ──────────────────
    ...
    -- ── Perfil dos ganhos (top buckets por dimensão) ──────────────────────
    -- Calcula apenas sobre cards do recorte (slot) com ganho=TRUE.
    IF v_ganhos_total > 0 THEN
