...
-- 2. get_client_by_phone com fallback viajante
...
DECLARE
    ...
    v_contato RECORD;
    v_card RECORD;
    v_contact_role TEXT DEFAULT 'primary';
    v_pessoa_principal_nome TEXT DEFAULT '';
BEGIN
    ...
    -- Busca card ativo como contato principal
    ...
    IF v_card.id IS NULL THEN
        ...
        WHERE cc.contato_id = v_contato.id
        ...
        IF v_card.id IS NOT NULL THEN
            v_contact_role := 'traveler';
            SELECT TRIM(COALESCE(ct.nome, '') || ' ' || COALESCE(ct.sobrenome, ''))
            INTO v_pessoa_principal_nome
            FROM contatos ct WHERE ct.id = v_card.pessoa_principal_id;
        END IF;
    END IF;
    ...
    RETURN jsonb_build_object(
        ...
        -- Novos campos para viajantes
        'contact_role', v_contact_role,
        'pessoa_principal_nome', COALESCE(TRIM(v_pessoa_principal_nome), '')
    );
END;
...
BEGIN
    ...
    -- ========================================
    BEGIN
        -- LINK TO CARD (find existing as primary contact)
