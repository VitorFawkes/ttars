-- ============================================================================
-- MIGRATION: bulk_create_pos_venda_cards — flags update_dates e sync_monde_nums
-- Date: 2026-04-30
--
-- A página de Importação Pós-Venda passa a oferecer toggles por viagem:
--   - update_dates: quando true, sobrescreve data_viagem_inicio/fim com os
--     valores do arquivo. Comportamento anterior usava COALESCE (só preenchia
--     se vazio), bloqueando correção de datas erradas no CRM.
--   - sync_monde_nums: quando true, redefine produto_data.numero_venda_monde
--     e numeros_venda_monde_historico para refletir EXATAMENTE o arquivo
--     (adiciona os que faltam, remove os que sumiram). Comportamento anterior
--     era append-only, impedindo limpeza de números obsoletos.
--
-- Ambas as flags são opcionais (default false) — chamadas antigas continuam
-- funcionando exatamente como antes (COALESCE em datas, append em Monde).
--
-- Resto da função idêntico à 20260427c.
-- ============================================================================
...
DECLARE
    ...
    -- Flags novas (default false → backward compat)
    ...
    LOOP
        BEGIN
            ...
            IF v_existing_card_id IS NOT NULL THEN
                ...
                -- Datas: se update_dates=true, sobrescreve com o que o arquivo trouxe
                -- (quando o arquivo tem a data); senão preserva o COALESCE (preenche só se vazio).
                UPDATE cards SET
                ...
                IF v_sync_monde_nums THEN
                    -- Sync completo: histórico final = exatamente o que o arquivo trouxe.
                    -- Adiciona os que faltam, remove os que sumiram.
                    v_historico := '[]'::JSONB;
                    ...
                    WHERE id = v_card_id;
                ELSE
                    -- Append-only (comportamento anterior): mantém o existente,
                    -- adiciona o que falta, nunca remove.
