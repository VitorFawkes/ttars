-- Lista de Convidados — admin_list_v3: vê casais de child orgs quando logado em account pai
--
-- Antes: wedding_casal_admin_list_v2() filtra apenas WHERE org_id = requesting_org_id().
-- Resultado: admin logado em Welcome Group (account pai) não via casais cadastrados nos
-- workspaces filhos (Welcome Weddings, Trips, Courses).
--
-- Agora: v3 inclui casais cuja org_id seja a do usuário OU seja uma org filha
-- (parent_org_id = requesting_org_id()). Adiciona workspace_name na resposta
-- para o frontend mostrar de qual workspace é cada casal.
...
BEGIN
  ...
  WHERE
    c.org_id = v_org
    OR c.org_id IN (SELECT id FROM public.organizations WHERE parent_org_id = v_org)
  ORDER BY c.criado_em DESC;
END
$fn$;

GRANT EXECUTE ON FUNCTION public.wedding_casal_admin_list_v3() TO authenticated;

COMMIT;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname='wedding_casal_admin_list_v3') THEN
    RAISE EXCEPTION 'v3 não criada';
  END IF;
  RAISE NOTICE 'admin_list_v3 OK';
END $$;
