-- ============================================================
-- MIGRATION: Suporte a mensagens de grupo WhatsApp (Echo)
-- Date: 2026-03-19
--
-- 1. Tabela whatsapp_groups (mapeia group_jid → card_id)
-- 2. Colunas is_group, group_jid, group_name em whatsapp_messages
-- 3. process_whatsapp_raw_event_v2 atualizada para processar grupos
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.whatsapp_groups (
    ...
    platform_id uuid REFERENCES public.whatsapp_platforms(id),
    created_at timestamptz DEFAULT now(),
    updated_at timestamptz DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_whatsapp_groups_card_id ON public.whatsapp_groups(card_id);

ALTER TABLE public.whatsapp_groups ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Authenticated users can view whatsapp_groups"
    ON public.whatsapp_groups FOR SELECT TO authenticated USING (true);
CREATE POLICY "Service role full access whatsapp_groups"
    ON public.whatsapp_groups FOR ALL TO service_role USING (true) WITH CHECK (true);

-- 2. Colunas novas em whatsapp_messages
ALTER TABLE public.whatsapp_messages
    ADD COLUMN IF NOT EXISTS is_group boolean DEFAULT false,
    ADD COLUMN IF NOT EXISTS group_jid text,
    ADD COLUMN IF NOT EXISTS group_name text;

CREATE INDEX IF NOT EXISTS idx_whatsapp_messages_group_jid
    ON public.whatsapp_messages(group_jid) WHERE group_jid IS NOT NULL;

-- 3. Função process_whatsapp_raw_event_v2 atualizada
    ...
    BEGIN
        ...
        IF v_is_group AND v_group_jid IS NOT NULL THEN
            ...
            IF v_group_card_id IS NOT NULL THEN
                -- Verify the card still exists and is open
                SELECT id INTO v_card_id FROM cards
                WHERE id = v_group_card_id
                  AND status_comercial NOT IN ('ganho', 'perdido')
                  AND deleted_at IS NULL;

                -- If card was closed/deleted, clear local ref (will fall through to contact-based lookup)
        ...
        -- LINK TO CARD (find existing as primary contact) — skip if group mapping already resolved
        IF v_card_id IS NULL THEN
            ...
            -- FALLBACK: busca card onde é viajante (cards_contatos)
            IF v_card_id IS NULL AND v_link_to_card_enabled = true THEN
            ...
            -- ========================================
            -- FIX: Guard incondicional — SEMPRE buscar card existente antes de auto-criar
            -- ========================================
