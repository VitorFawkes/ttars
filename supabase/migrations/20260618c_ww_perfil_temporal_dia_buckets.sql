-- 20260618c_ww_perfil_temporal_dia_buckets.sql
--
-- Evolui ww_perfil_temporal (pedido do Vitor, print 2026-06-18):
--  • granularidade 'day' (por dia), além de week/month;
--  • p_buckets: lista de buckets escolhidos (ex: só Nordeste+Caribe+Itália) — quando vem,
--    a série traz SÓ esses (sem "Outros"); quando NULL, mantém top p_max_buckets + "Outros";
--  • buckets_all: universo completo de buckets (com total) pra UI montar o seletor.
-- O resto (por_categoria, modo cohort/throughput, filtros) é igual à 20260618b.
--
-- Assinatura muda (novo p_buckets) → DROP da versão antiga antes do CREATE pra não criar
-- overload ambíguo no PostgREST. Função NOVA recente (1 migration anterior). Só leitura.

DROP FUNCTION IF EXISTS public.ww_perfil_temporal(timestamptz, timestamptz, uuid, text, text, text, text, text[], text[], uuid[], text[], text[], text[], text[], text[], text, int);
...
REVOKE ALL ON FUNCTION public.ww_perfil_temporal(timestamptz, timestamptz, uuid, text, text, text, text, text[], text[], uuid[], text[], text[], text[], text[], text[], text, int, text[]) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.ww_perfil_temporal(timestamptz, timestamptz, uuid, text, text, text, text, text[], text[], uuid[], text[], text[], text[], text[], text[], text, int, text[]) FROM anon;
GRANT EXECUTE ON FUNCTION public.ww_perfil_temporal(timestamptz, timestamptz, uuid, text, text, text, text, text[], text[], uuid[], text[], text[], text[], text[], text[], text, int, text[]) TO authenticated, service_role;
