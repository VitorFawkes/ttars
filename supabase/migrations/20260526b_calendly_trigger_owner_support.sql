-- Adiciona suporte a "dono do card" na função do trigger Calendly.
-- Substitui process_cadence_entry_on_calendly_invitee criada em 20260526.
--
-- Novos campos no event_config:
--   owner_mode: 'none' | 'fixed' | 'organizer'
--     - none: card criado sem dono (default se ausente)
--     - fixed: usa owner_user_id configurado
--     - organizer: matchea profile pelo email do organizador da reunião
--   owner_user_id: UUID do profile (usado quando owner_mode='fixed')
...
BEGIN
  ...
  LOOP
    ...
    ELSIF coalesce((v_trigger.event_config->>'create_card_if_missing')::BOOLEAN, FALSE) THEN
      ...
      -- Resolver dono pro card que vai ser criado
      ...
      ELSIF v_owner_mode = 'organizer' AND NEW.organizer_email IS NOT NULL THEN
        -- Match profile do organizer dentro do workspace da automação
      ...
      -- Criar card (com dono se resolvido)
      ...
      INSERT INTO cards (
        ...
        dono_atual_id, sdr_owner_id, vendas_owner_id
      )
      VALUES (
        v_resolved_org_id,
...
COMMENT ON FUNCTION process_cadence_entry_on_calendly_invitee() IS
  'Enfileira automações de calendly_invitee_created. Suporta criação de card com dono (organizador do Calendly ou usuário fixo).';
