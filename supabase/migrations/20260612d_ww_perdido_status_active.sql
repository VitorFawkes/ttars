-- 20260612d — perdido conta o STATUS do Active (não só o motivo escrito)
--
-- PROBLEMA (achado pelo Vitor via deal 30000 "DW | Alice"): lead marcado como
-- PERDIDO no Active (status=2) SEM motivo escrito aparecia como "aberto" no painel.
-- A esteira SDR (grupo 1) não gera evento de jornada, então o único sinal de perda
-- é o campo status do deal — que o espelho NUNCA guardava. Junho mostrava 0 perdidos
-- sendo que 17 deals de junho estão perdidos no Active (15 leads reais + 2 lixo).
--
-- FIX: coluna ac_status no cache (backfill + edges) e is_perdido ADITIVO:
--   perdido = NÃO ganho E ( tem motivo escrito  [regra antiga, intacta]
--                           OR (tem deal status=perdido E nenhum deal aberto) ).
-- Puramente aditivo — nunca remove um perdido que já contava. Deal status NULL
-- (não sincronizado) cai na regra antiga. Ganho segue pela jornada (intacto).
--
-- DILIGÊNCIA (TOP-5 #5): cadeia ...612a→b→c→d. Base = def viva de prod
-- (pg_get_functiondef 2026-06-12, com 612a/b/c); muda só cf (2 aggs) + is_perdido.
-- Aplicar em 3 statements (promote engole multi-statement com $function$).

-- Coluna do status do Active. Backfill foi via script; daqui pra frente os edges
-- mantêm fresco: incremental grava no webhook; reconcile PASSO 1.5 varre mdate DESC
-- de hora em hora (rede de segurança p/ perdas de SDR que o webhook pode perder).
ALTER TABLE ww_ac_deal_funnel_cache ADD COLUMN IF NOT EXISTS ac_status SMALLINT;
COMMENT ON COLUMN ww_ac_deal_funnel_cache.ac_status IS 'Status do deal no ActiveCampaign: 0=aberto, 1=ganho, 2/3=perdido. Fonte primaria de perdido p/ esteira SDR (grupo 1) que nao gera evento de jornada. Add 20260612d.';
