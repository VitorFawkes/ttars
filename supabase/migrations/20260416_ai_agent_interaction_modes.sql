-- ============================================================================
-- MODOS DE INTERACAO — Agentes IA outbound/hybrid
-- ============================================================================
-- Adiciona: interaction_mode, first_message_config, outbound_trigger_config em ai_agents
-- Cria: ai_outbound_queue (fila de envios outbound)
-- Adiciona: maps_to_field em ai_agent_qualification_flow
-- Cria: trigger trg_card_outbound_queue
-- ============================================================================

-- ============================================================================
-- 1. COLUNAS NOVAS EM AI_AGENTS
-- ============================================================================
ALTER TABLE ai_agents
  ADD COLUMN IF NOT EXISTS interaction_mode TEXT NOT NULL DEFAULT 'inbound'
    CHECK (interaction_mode IN ('inbound', 'outbound', 'hybrid')),
  ADD COLUMN IF NOT EXISTS first_message_config JSONB DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS outbound_trigger_config JSONB DEFAULT NULL;

COMMENT ON COLUMN ai_agents.interaction_mode IS 'inbound=so responde, outbound=agente inicia, hybrid=ambos';
COMMENT ON COLUMN ai_agents.first_message_config IS 'Config da primeira mensagem quando mode=outbound/hybrid. type: fixed|ai_generated';
COMMENT ON COLUMN ai_agents.outbound_trigger_config IS 'Triggers que disparam outbound: card_created, stage_changed, idle_days + business_hours';

CREATE INDEX IF NOT EXISTS idx_ai_agents_outbound
  ON ai_agents(org_id, produto)
  WHERE ativa = true AND interaction_mode IN ('outbound', 'hybrid');

-- ============================================================================
-- 2. MAPS_TO_FIELD em AI_AGENT_QUALIFICATION_FLOW
-- ============================================================================
ALTER TABLE ai_agent_qualification_flow
  ADD COLUMN IF NOT EXISTS maps_to_field TEXT DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS skip_if_filled BOOLEAN DEFAULT true;

COMMENT ON COLUMN ai_agent_qualification_flow.maps_to_field IS 'Campo do card/contato que esta pergunta preenche. Se ja preenchido E skip_if_filled=true, pula.';
COMMENT ON COLUMN ai_agent_qualification_flow.skip_if_filled IS 'Se true, pula pergunta quando maps_to_field ja tem valor';

-- ============================================================================
-- 3. AI_OUTBOUND_QUEUE — Fila de envios outbound
-- ============================================================================
...
CREATE INDEX idx_outbound_queue_pending ON ai_outbound_queue(org_id, status, scheduled_for) WHERE status IN ('pending', 'scheduled');
CREATE INDEX idx_outbound_queue_agent ON ai_outbound_queue(agent_id, status);
CREATE INDEX idx_outbound_queue_card ON ai_outbound_queue(card_id);
...
-- ============================================================================
-- 4. TRIGGER: Enfileira outbound quando card criado com telefone + form
-- ============================================================================
...
-- ============================================================================
-- 5. RPCs para processar fila outbound
-- ============================================================================
