-- analytics_drill_down_cards — fonte 'current_stage' NÃO deve filtrar por data.
--
-- BUG (reportado pelo Vitor): clicar numa barra da "Carteira por etapa do Planner" (ex:
-- Oportunidade = 18) abria só ~4 cards. Idem cliques na tabela "Cards agora". Causa: o ramo
-- current_stage aplicava `c.created_at BETWEEN p_date_start AND p_date_end` quando em modo
-- 'entries' (default). Mas current_stage é um SNAPSHOT ("cards que estão NESSA etapa AGORA") —
-- os gráficos que o usam (Carteira por etapa, heatmap Cards agora, Pipeline atual, SLA, Team,
-- SDR, perfil) contam todos os cards abertos na etapa, sem recorte de data. O drill recortava
-- por data de criação e devolvia menos cards do que o número mostrado.
--
-- FIX: remover o filtro de created_at do ramo 'current_stage'. Assim o drill devolve EXATAMENTE
-- os mesmos cards que o gráfico conta. Nenhum outro ramo é alterado.
--
-- AUDITORIA DE REBASE (CLAUDE.md §TOP 5 #5): cópia EXATA de 20260422d (a versão mais recente,
-- que preserva: sub_card filter 20260313, date_ref 20260422c, e os filtros do Funil v3 —
-- root_stage_id, status_array, ganho_fase). Única diferença: as 3 linhas do created_at no ramo
-- current_stage foram removidas.

-- Dropa todas as overloads (staging pode estar defasado com assinatura diferente).
DO $cleanup$
DECLARE
    r RECORD;
BEGIN
    FOR r IN
        ...
        WHERE n.nspname = 'public'
          AND p.proname = 'analytics_drill_down_cards'
    LOOP
...
CREATE FUNCTION public.analytics_drill_down_cards(
    ...
    p_drill_root_stage_id uuid    DEFAULT NULL,
    p_drill_status_array  text[]  DEFAULT NULL,
    p_drill_ganho_fase    text    DEFAULT NULL
)
RETURNS TABLE(
...
BEGIN
    ...
    ELSIF v_source = 'current_stage' THEN
        -- SNAPSHOT: cards que estão na etapa AGORA. NÃO filtra por data (era o bug: created_at
        -- recortava o snapshot e devolvia menos cards do que o gráfico contava).
