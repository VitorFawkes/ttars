-- ============================================================================
-- MIGRATION: bulk_import_financial_items v11 — fix bug crítico de match
-- Date: 2026-05-19
--
-- Bug raiz (v10 e anteriores desde 15/05):
--   Em PL/pgSQL, `RECORD IS NOT NULL` só retorna TRUE quando TODOS os campos
--   do record são NOT NULL. Se qualquer campo for NULL (ex: representante),
--   tanto `IS NULL` quanto `IS NOT NULL` retornam FALSE.
--
--   Consequência: depois de `SELECT ... INTO v_match`, quando o registro era
--   encontrado mas tinha algum campo NULL, a função pensava que NÃO encontrou
--   match — e fazia INSERT em vez de UPDATE. Resultado: a cada re-upload,
--   items existentes eram arquivados e recriados, perdendo is_ready,
--   passageiros e qualquer estado operacional.
--
--   Confirmado em prod 2026-05-19:
--     SELECT (ROW(1, NULL, 'x')) IS NOT NULL  →  FALSE  (esperado TRUE)
--
-- Fix v11:
--   Substituir `IF v_match IS NOT NULL` por `IF FOUND` (variável booleana
--   de PL/pgSQL que indica se o último SELECT INTO encontrou linha).
--   Como pode haver múltiplos SELECT INTO em cascata (fallbacks), uso uma
--   variável booleana auxiliar `v_match_found` que captura FOUND após cada
--   tentativa.
--
-- Releitura confirmada de v4–v10. Nenhuma outra mudança de comportamento.
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- Tentativa 1: match por documento (quando venda_num + doc presentes)
      ...
      -- Tentativa 2: match por (desc + fornec + valor + datas) com venda_num
      ...
      -- Tentativa 3: match sem venda_num (items manuais legados)
    ...
    -- CLEANUP: arquiva produtos que sumiram do arquivo (regra v10, mantida).
...
COMMENT ON FUNCTION bulk_import_financial_items(JSONB) IS
  'v11 (2026-05-19): fix de detecção de match — usa FOUND em vez de v_match IS NOT NULL, que retornava FALSE quando RECORD tinha qualquer campo NULL (ex: representante). Bug fazia toda re-importação rodar como delete+recreate em vez de preserve+update, perdendo is_ready e gerando duplicação. Lógica de match/diff/cleanup idêntica à v10.';
