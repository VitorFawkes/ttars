-- v5 — adiciona daily_timeline ao RPC + params de drill-down do heatmap
--
-- REBASE CONFIRMADO. Releitura das migrations anteriores:
--   - 20260519c (v1): SUPERSEDED por v2.
--   - 20260519d (v2): nova assinatura (TEXT[]). Lógica de estado, FRT, KPIs.
--   - 20260519f (v3): adicionou by_line, state_distribution, depth_histogram.
--   - 20260519g (v4): adicionou frt_distribution, weekday_hour_heatmap,
--                     time_metrics, conversation_duration_days. ESTA é a base.
--
-- A v5 PRESERVA TUDO da v4 (assinatura ESTENDIDA, lógica idêntica) e ADICIONA:
--   - 2 params novos no fim (compat backward — defaults NULL):
--       p_weekday_filter INT  — filtra por dia da semana (0=Dom..6=Sáb)
--                              Aplica a contagem por (customer_phone, line)
--                              que tem inbound naquele weekday.
--       p_hour_filter    INT  — filtra por hora (0-23) — idem
--   - daily_timeline no retorno: vetor com data, outbound_count, inbound_count,
--     novos_contatos (1ª outbound naquele dia), novas_respostas (1ª inbound
--     naquele dia), ganhos (cards que ganharam naquele dia)
--   - Heatmap permanece igual (não muda comportamento atual)
--
-- Os campos ANTERIORES (kpis, funnel, by_line, etc) permanecem IDÊNTICOS.
...
BEGIN
  ...
  -- Conversas (customer_phone, phone_line_label) que tiveram inbound naquele
  -- (weekday, hour) do filtro — limita conversation_summary depois
  ...
  -- ★ NEW v5: timeline diária — volume por dia
  daily_timeline AS (
    ...
    FROM (
      ...
      new_each_day AS (
        SELECT
          first_outbound_at::DATE AS first_day,
          COUNT(*)::INT AS new_contacts,
          first_inbound_at::DATE AS first_in_day,
          COUNT(*) FILTER (WHERE first_inbound_at IS NOT NULL)::INT AS new_replies
        FROM conv_filtered
        WHERE first_outbound_at IS NOT NULL
        GROUP BY first_outbound_at::DATE, first_inbound_at::DATE
      ),
      ...
      SELECT
        d.day,
        COALESCE(mv.outbound, 0) AS outbound,
        COALESCE(mv.inbound, 0)  AS inbound,
...
COMMENT ON FUNCTION analytics_weddings_conversations IS
  'v5 (rebase confirmado da v4 - 20260519g): adiciona daily_timeline + params p_weekday_filter/p_hour_filter para drill-down do heatmap.';
