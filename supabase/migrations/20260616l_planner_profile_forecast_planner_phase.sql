-- analytics_planner_profile — alinha o bloco "forecast" (Previsto fechar próx. 7d/30d
-- do drawer de perfil do planner) à mesma regra da Previsão de fechamento principal:
--   • só conta cards que estão na etapa de Planner (fase owner_field='vendas_owner_id');
--   • valor por COALESCE(valor_final, valor_estimado, 0) — convenção do CRM
--     (antes usava valor_estimado primeiro, invertido).
--
-- Mesma causa raiz já corrigida em 20260616k para analytics_planner_forecast_by_dono.
-- O resto da função é cópia EXATA de 20260527i (preserva: briefing_inicial JSONB,
-- safe date cast por string, sem operador ?, comparação ISO em string). Só o bloco
-- forecast (e a nova CTE cards_abertos_planner) muda.
--
-- IMPORTANTE: este perfil já é filtrado por vendas_owner_id = p_user_id, e na etapa de
-- Planner vendas_owner == dono_atual, então o "previsto fechar" do drawer passa a bater
-- com o gráfico principal e com o quadro daquele planner. Os demais blocos (ganho/perda
-- por período, cards abertos totais, etc.) seguem como estavam — crédito ao vendedor.
...
BEGIN
    ...
    -- ✨ NOVO: subconjunto dos cards abertos que estão na etapa de Planner — base do forecast
    cards_abertos_planner AS (
        SELECT cp.*
        FROM cards_abertos cp
        JOIN pipeline_stages s ON s.id = cp.pipeline_stage_id
        JOIN pipeline_phases pp ON pp.id = s.phase_id
        WHERE pp.owner_field = 'vendas_owner_id'
    ),
    cards_abertos_em_risco AS (
        ...
        WHERE cp.status_comercial NOT IN ('ganho', 'perdido')
          AND (
              -- briefing_inicial é JSONB: vazio = NULL ou '{}'
              (cp.briefing_inicial IS NULL OR cp.briefing_inicial = '{}'::JSONB)
    ...
    preenchimento AS (
        SELECT
            COUNT(*) FILTER (WHERE briefing_inicial IS NULL OR briefing_inicial = '{}'::JSONB)::INT AS sem_briefing,
    ...
    forecast AS (
        SELECT
            ...
            COALESCE(SUM(COALESCE(cp.valor_final, cp.valor_estimado, 0)) FILTER (
                WHERE cp.data_prev_fech_str IS NOT NULL
                  AND cp.data_prev_fech_str >= v_today
                  AND cp.data_prev_fech_str <= v_prox_7d
        ...
        FROM cards_abertos_planner cp   -- ✨ só etapa de Planner
    ),
    membro AS (
