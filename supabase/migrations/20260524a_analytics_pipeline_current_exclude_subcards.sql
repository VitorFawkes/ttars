-- Fix: analytics_pipeline_current incluía sub_cards (27 de 657 em Trips).
-- Saúde e outras telas excluem sub_cards corretamente, gerando inconsistência
-- entre Pipeline (657) e Saúde (596).
--
-- AUDITORIA (memory/feedback_function_rebase_cuidado.md):
-- analytics_pipeline_current → versão MAIS RECENTE em 20260307_pipeline_current_tasks.sql
--   preserva: 20260304_enhance_pipeline_current, 20260304_pipeline_current_filters
-- Mudança nesta migration: UMA linha — adicionar AND COALESCE(c.card_type, 'standard') != 'sub_card'
-- no CTE open_cards. Resto da função idêntica.
...
BEGIN
    WITH open_cards AS (
        ...
        WHERE c.deleted_at IS NULL
          ...
          AND s.ativo = true
          AND COALESCE(c.card_type, 'standard') != 'sub_card'  -- ✨ FIX: exclui sub_cards
