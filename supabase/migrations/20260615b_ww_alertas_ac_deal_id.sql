-- 20260615b — Leads parados (alertas) ganham o ac_deal_id → link "abrir no Active" no painel
--
-- Pedido do Vitor: no bloco "Leads parados há mais de 7 dias", garantir que não há perdidos
-- (já garantido — a query exclui is_perdido/ganho/status_comercial/etapa resolução-pos_venda) e
-- que TODO alerta tenha o link pro Active. Único acréscimo: 'ac_deal_id' (= cards.external_id,
-- que é o id do deal no Active) no JSON dos alertas. Resto da ww2_overview v9 intacto (verbatim).
-- DROP + CREATE (assinatura igual). Drill inalterado.
...
COMMENT ON FUNCTION public.ww2_overview IS
  'Overview Weddings v10 (20260615b) — "Onde estão agora" pipeline-primeiro: pipeline atual pela última etapa do Active (ww_deal_event), depois etapa real (SDR: Lead/Reunião agendada/re-agendada/Aguardando taxa/Aguardando agendamento closer; Closer: Em contato/Contrato enviado/Negociação/Aguardando dados); universo por data de atividade; Pós-Venda fora; resto = v8.';
