-- ============================================================
-- MIGRATION: process_whatsapp_raw_event_v2 — não tratar #131026 como falha
-- Date: 2026-05-22
--
-- Quando Meta devolve #131026 "Message undeliverable" para um número BR,
-- o Echo já faz fallback automático: gera variação do número (tira o 9
-- se tiver 9 dígitos, ou põe se tiver 8) e reenvia. Na maioria dos casos
-- o usuário recebe pela variação.
--
-- Antes desta migration, a função marcava has_error=true imediatamente
-- ao receber o evento de erro #131026 — mesmo quando o Echo recuperava
-- via fallback. Relatório de envio mostrava "Falhas" eternamente.
--
-- Mudança cirúrgica no bloco de message.status (linhas ~123-154 da
-- migration 20260421j_process_whatsapp_raw_event_phase_id_refix.sql):
-- detecta "Message undeliverable" no error_message e, em vez de
-- has_error=true, marca status='pending_fallback' com has_error=false.
-- ============================================================
...
DECLARE
    ...
    v_update_contact_enabled BOOLEAN;
    v_phone TEXT;
    ...
    v_is_undeliverable_131026 BOOLEAN;
...
BEGIN
    ...
    IF v_platform.provider = 'echo' AND (v_payload->>'event') = 'message.status' AND v_external_id IS NOT NULL THEN
        ...
        -- Echo já faz fallback automático com variação do número (com/sem 9) quando
        -- Meta devolve #131026 "Message undeliverable". Não marcar como falha final:
        -- mantém status='pending_fallback' com has_error=false até confirmação.
        v_is_undeliverable_131026 := COALESCE(v_msg_error_message ILIKE '%message undeliverable%', false);

        UPDATE whatsapp_messages
        SET
            status = CASE
                WHEN v_is_undeliverable_131026 THEN 'pending_fallback'
                ELSE v_msg_status
            END,
            has_error = CASE
                WHEN v_is_undeliverable_131026 THEN false
                ELSE v_msg_has_error
            END,
        ...
        IF v_status_rows_updated > 0 THEN
            ...
            RETURN jsonb_build_object(
                ...
                'new_status', CASE WHEN v_is_undeliverable_131026 THEN 'pending_fallback' ELSE v_msg_status END,
                'has_error', CASE WHEN v_is_undeliverable_131026 THEN false ELSE v_msg_has_error END,
                'error_message', v_msg_error_message,
                'fallback_pending', v_is_undeliverable_131026,
    ...
    IF v_contact_id IS NOT NULL
       ...
       AND NOT v_from_me
       AND v_sender_name IS NOT NULL
       AND length(btrim(v_sender_name)) >= 3 THEN
        SELECT (value = 'true') INTO v_update_contact_enabled
          FROM integration_settings WHERE key = 'WHATSAPP_UPDATE_CONTACT';
        IF v_update_contact_enabled = true
           AND NOT is_weak_contact_name(v_sender_name) THEN
            UPDATE contatos
               SET nome = split_part(btrim(v_sender_name), ' ', 1),
                   sobrenome = CASE
                       WHEN position(' ' IN btrim(v_sender_name)) > 0
                       THEN btrim(substring(btrim(v_sender_name) FROM position(' ' IN btrim(v_sender_name)) + 1))
                       ELSE sobrenome
                   END,
                   updated_at = NOW()
             WHERE id = v_contact_id
               AND is_weak_contact_name(nome)
               AND nome_locked_at IS NULL;
            IF FOUND THEN v_contact_name_updated := TRUE; END IF;
        END IF;
    END IF;
    ...
    RETURN jsonb_build_object(
        ...
        'contact_name_updated', v_contact_name_updated,
...
COMMENT ON FUNCTION process_whatsapp_raw_event_v2 IS
'Processa evento WhatsApp. Marco A — atualiza contatos.nome quando fraco e sender_name é válido (inbound only). Refix 20260421j — phase_id no lugar de fase_label. Skip 20260522b — não marca has_error=true em #131026 "Message undeliverable" (Echo já faz fallback com variação do número); usa status="pending_fallback" até confirmação.';
