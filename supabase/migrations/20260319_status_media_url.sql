-- ============================================================
-- MIGRATION: Extrair media_url do message.status
-- Date: 2026-03-19
--
-- Contexto:
-- Echo agora inclui media.url no evento message.status (antes só vinha no message.sent).
-- O short-circuit de status update precisa aproveitar esse campo para preencher
-- media_url em mensagens que antes ficavam sem áudio.
--
-- Mudança:
-- Nos 2 UPDATEs do short-circuit (por external_id e por echo_message_id),
-- adicionar: media_url = COALESCE(v_media_url, whatsapp_messages.media_url)
-- Isso preenche media_url se estava NULL, sem sobrescrever se já existia.
-- ============================================================
...
BEGIN
    ...
    -- Se não encontra (rows_updated=0), tenta fallback por message_id.
    -- Se ainda não encontra, cai no pipeline completo.
    -- ========================================
    IF v_platform.provider = 'echo' AND (v_payload->>'event') = 'message.status' AND v_external_id IS NOT NULL THEN
        ...
        -- media_url: preserva existente (do message.sent), preenche se era NULL
        UPDATE whatsapp_messages
        SET
            status = v_msg_status,
            has_error = v_msg_has_error,
            error_message = v_msg_error_message,
            media_url = COALESCE(whatsapp_messages.media_url, v_media_url),
            message_type = COALESCE(whatsapp_messages.message_type, v_message_type),
            updated_at = NOW()
        ...
        IF v_status_rows_updated > 0 THEN
            ...
            RETURN jsonb_build_object(
                ...
                'has_error', v_msg_has_error,
                'error_message', v_msg_error_message,
                'media_url_from_status', v_media_url IS NOT NULL,
                'rows_updated', v_status_rows_updated
            );
        END IF;

        -- ========================================
        -- FALLBACK: Match por message_id do Echo
        -- O message.sent salva idempotency_key = message_id (UUID interno).
        -- O message.status traz o mesmo message_id no payload.
        -- JOIN raw_events.idempotency_key → messages.raw_event_id.
        -- ========================================
        ...
        SET
            ...
            has_error = v_msg_has_error,
            error_message = v_msg_error_message,
            media_url = COALESCE(wm.media_url, v_media_url),
            message_type = COALESCE(wm.message_type, v_message_type),
            updated_at = NOW()
        FROM whatsapp_raw_events wre
        WHERE wre.idempotency_key = (v_data->>'message_id')
          AND wre.platform_id = v_event.platform_id
          AND wm.raw_event_id = wre.id
          AND wm.external_id IS NULL;
        ...
        -- Se NÃO encontrou por nenhum método, cai no pipeline completo
