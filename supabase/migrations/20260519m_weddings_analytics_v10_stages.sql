-- v10 — adiciona STAGE do card a cada conversa + KPIs de reunião/proposta
--
-- REBASE CONFIRMADO. Releitura das migrations:
--   - v1..v7: histórico (assinatura, breakdowns, temporal, daily).
--   - v8/v9 (20260519k/l): view com phone normalize. ESTAS são a base.
--
-- A v10 PRESERVA assinatura e adiciona:
--   - stage_nome e stage_phase_slug em cada conversation (via JOIN card)
--   - meetings_kpis no retorno: scheduled, done, proposals_sent (contagem
--     por stage no funil interno Weddings)
--   - stage_funnel: distribuição de cards Weddings por stage atual
--
-- O Vitor pediu pra cruzar com AC. Descoberta: AC só roda em Welcome Trips
-- (não Weddings). A Weddings usa seu funil interno (pipeline_stages) com:
--   - "Reunião Agendada" (closer-1): lead aceitou marcar
--   - "Apresentação Feita" (closer-2): reunião aconteceu
--   - "Proposta Enviada" (closer-3): avançou
--   - "Contrato Assinado" (closer-5): fechou
-- A SDR move o card manualmente. Não há sync automático de "reunião realizada"
-- via AC porque AC não cobre Weddings.
...
BEGIN
  ...
  -- ★ v10: enriquecer card_lookup com stage info
  card_lookup AS (
  ...
  funnel AS (
    ...
    FROM (
      ...
      UNION ALL SELECT 4, 'Reunião Agendada', COUNT(*) FILTER (WHERE stage_nome = 'Reunião Agendada' OR meeting_state IS NOT NULL)::INT FROM conv_filtered
...
COMMENT ON FUNCTION analytics_weddings_conversations IS
  'v10 (rebase confirmado da v9 - 20260519l): adiciona stage_nome/stage_phase_slug/meeting_state em cada conversa + meetings_kpis. Funil agora tem etapas Reuniao Agendada / Reuniao Feita / Proposta / Ganhou.';
