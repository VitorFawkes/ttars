-- ============================================================================
-- M1 — Sofia grava a decisão de qualificação no card
-- Estende a allowlist DEFAULT do wsdr_persist_lead pra incluir ww_sdr_qualificado
-- (booleano vindo do Agente 2 Qualificador). Assim qualquer agente clonado já
-- grava a qualificação sem precisar configurar writable_fields.
-- Recriação fiel de 20260530c_wsdr_persist_lead.sql; ÚNICA mudança: v_default_allow.
-- (Rule #5: grep confirmou que 20260530c é a única definição anterior.)
-- ============================================================================
...
DECLARE
  ...
  -- + ww_sdr_qualificado: decisão booleana do Agente 2 (Qualificador), pro gestor ver no card.
  v_default_allow  TEXT[] := ARRAY['ww_destino','ww_num_convidados','ww_orcamento_faixa','ww_data_casamento','ww_nome_parceiro','ww_tipo_casamento','ww_mkt_como_conheceu','ww_sdr_qualificado'];
BEGIN
...
END $$;
COMMENT ON FUNCTION wsdr_persist_lead IS 'Sofia grava o casal no CRM (Weddings): acha/cria contato+card, grava ww_* + ww_sdr_qualificado validados por allowlist+system_fields. Org-safe (p_org_id), sem JWT.';
