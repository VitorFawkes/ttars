-- Fix: remover INSERT em cards_contatos para o titular.
-- O trigger check_single_role_cards_contatos bloqueia porque o contato já
-- está referenciado em cards.pessoa_principal_id. A tabela cards_contatos
-- é exclusivamente para acompanhantes, não para o titular.
...
BEGIN
    ...
    -- Nota: NÃO inserir em cards_contatos para o titular. O vínculo já existe
    -- via cards.pessoa_principal_id. A tabela cards_contatos é só para
    -- acompanhantes, e o trigger check_single_role_cards_contatos impede
    -- a duplicação do titular.
