-- 20260603a — ww_drift_combos: conversão SEMPRE por safra (cohort), nunca throughput.
--
-- BUG (#4 reportado pelo Vitor): em "Data de entrada na etapa (período)" (throughput) o
-- universo da função virava SÓ os fechados (ganho_at no período) → "entrou" == "fechou" em
-- toda célula → taxa de conversão = 100% em tudo. Conversão (entrou→fechou) SÓ faz sentido
-- sobre a população que ENTROU (safra). Então ignoramos o date_mode aqui e sempre usamos
-- data_entrada (quem entrou no período) como universo; "fechou" = quem ganhou.
--
-- VERIFICAÇÃO REBASE (TOP 5 #5): parte da 20260602s (def viva). Só o filtro do TEMP TABLE
-- muda (remove o ramo throughput); todas as agregações seguem idênticas. Nada revertido.
...
BEGIN
    ...
    -- date_mode é ignorado de propósito (taxa entrou→fechou exige a população de entrada).
    ...
    RETURN json_build_object(
      ...
      'fonte_marcos', 'vw_ww_funnel_base (cache AC) — conversão sempre por safra'
    );
END $func$;
