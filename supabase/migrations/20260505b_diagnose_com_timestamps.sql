-- ============================================================================
-- MIGRATION: fn_roteamento_pos_venda_trips_diagnose — incluir timestamps
-- Date: 2026-05-05
--
-- Adiciona c_criado_em e c_atualizado_em ao retorno, para a UI mostrar quando
-- cada caso bloqueado foi atualizado pela última vez (útil pra distinguir
-- legados abandonados de casos recentes).
-- ============================================================================
...
CREATE OR REPLACE FUNCTION public.fn_roteamento_pos_venda_trips_diagnose()
 RETURNS TABLE(
   ...
   c_viagem_fim date,
   c_criado_em timestamptz,
   c_atualizado_em timestamptz
 )
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FOR rec IN
    SELECT c.id, c.titulo, c.pipeline_stage_id, c.produto_data, c.created_at, c.updated_at,
  ...
  LOOP
    -- Prioridade: epoca_viagem (Data Viagem Completa); fallback data_exata_da_viagem (c/ Welcome)
    ...
    THEN
      v_travel := rec.produto_data -> 'epoca_viagem';
