    ...
    if (!blocked && finalMessages.length > 0) {
      ...
      const { error: turnInsertErr } = await supabase.from("ai_conversation_turns").insert({
        ...
        context_used: {
          ...
          // Auditoria: o que o agente decidiu neste turn (LLM output)
          self_analysis: singleAgentResult.output.self_analysis,
          tool_calls: singleAgentResult.output.tool_calls,
          card_patch: singleAgentResult.output.card_patch,
          contact_patch: singleAgentResult.output.contact_patch,
        },
