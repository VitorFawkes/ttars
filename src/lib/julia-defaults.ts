/**
 * Mirror dos defaults Julia — idêntico a `supabase/functions/_shared/julia_defaults.ts`.
 * Manter ambos em sincronia: a edge function não pode importar de src/ (Deno vs Vite),
 * então duplicamos. Se editar um, edite o outro.
 *
 * Usado pelo editor frontend para o botão "Usar padrão Julia".
 */
...
export const JULIA_VALIDATOR_RULES = [
  { id: 'no_ai_mention', condition: 'Menciona IA, modelo, prompt, agente, sistema ou bastidores', action: 'block' as const, enabled: true },
  { id: 'no_hallucination', condition: 'Inventa fatos não presentes no contexto da conversa', action: 'block' as const, enabled: true },
  { id: 'warm_tone', condition: 'Tom frio, robótico ou agressivo', action: 'correct' as const, enabled: true },
  { id: 'no_repeat_greeting', condition: 'Repete apresentação quando não é primeiro contato', action: 'correct' as const, enabled: true },
  { id: 'no_system_leak', condition: 'Menciona formulário, dados do sistema, ActiveCampaign ou CRM', action: 'block' as const, enabled: true },
  { id: 'no_early_rejection', condition: 'Rejeita lead na primeira mensagem ou sem investigar', action: 'block' as const, enabled: true },
  { id: 'no_assumed_isolated_service', condition: 'Diz "não trabalhamos com X isolado" sem que o cliente tenha confirmado que quer apenas isso', action: 'correct' as const, enabled: true },
  { id: 'club_med_no_fee_no_meeting', condition: 'Para lead Club Med, apresentou taxa R$ 500 ou tentou agendar reunião', action: 'correct' as const, enabled: true },
];
...
export const JULIA_INTELLIGENT_DECISIONS: Record<string, { enabled: boolean; config: Record<string, unknown> }> = {
  criar_reuniao: { enabled: true, config: { instructions: 'Só marque reunião depois de qualificar: destino + período + viajantes + orçamento (ou recusou informar). Use check_calendar, ofereça 2-3 slots reais em dias úteis 10-17h, nunca hoje. Solicite e-mail antes de criar. Para Club Med, NÃO marcar — Planner entra em contato por outro número.', gates: ['destino', 'periodo', 'viajantes', 'orcamento_ou_recusa'] } },
  ...
  escalar_agente_ia: { enabled: false, config: { instructions: 'Se a conta tiver múltiplos agentes de IA especializados, encaminhe ao correto. Não aplicável hoje — TRIPS tem um único agente.' } },
};
...
export const JULIA_CONTEXT_FIELDS = {
  visible_fields: [
    'card.titulo', 'card.produto', 'card.ai_resumo', 'card.ai_contexto', 'card.pipeline_stage_id',
    'contato.nome', 'contato.email', 'contato.telefone',
    'mkt_destino', 'mkt_buscando_para_viagem', 'mkt_quem_vai_viajar_junto', 'mkt_pretende_viajar_tempo',
    'mkt_hospedagem_contratada', 'mkt_valor_por_pessoa_viagem', 'mkt_mensagem_personalizada_formulario',
    'utm_source', 'pessoa_principal_nome', 'contact_role',
  ],
  updatable_fields: [
    'card.titulo', 'card.ai_resumo', 'card.ai_contexto', 'card.pipeline_stage_id',
    'contato.nome', 'contato.sobrenome', 'contato.email', 'contato.cpf',
    'contato.passaporte', 'contato.data_nascimento', 'contato.endereco', 'contato.observacoes',
  ],
  protected_fields: ['pessoa_principal_id', 'produto_data', 'valor_estimado', 'contato.telefone'],
  evidence_level: {
    'card.titulo': 'high',
    'card.ai_resumo': 'medium',
    'card.ai_contexto': 'medium',
    'card.pipeline_stage_id': 'high',
  } as Record<string, 'low' | 'medium' | 'high'>,
};

export const JULIA_HANDOFF_ACTIONS = {
  change_stage_id: null as string | null,
  apply_tag: { name: 'handoff-ia', color: '#f59e0b' } as { name: string; color: string } | null,
  notify_responsible: true,
  transition_message: null as string | null,
  pause_permanently: false,
};

export const JULIA_MULTIMODAL_CONFIG = { audio: true, image: true, pdf: true };
