...
import { useInternalInventoryProductMutations, type InternalInventoryProduct } from '@/hooks/useInternalInventoryProducts'
import { supabase } from '@/lib/supabase'
import { toast } from 'sonner'

interface Props {
    product: InternalInventoryProduct | null
    onClose: () => void
}

export default function InternalProductFormModal({ product, onClose }: Props) {
    const isEditing = !!product
    const { createProduct, updateProduct, deleteProduct } = useInternalInventoryProductMutations()
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const path = `internal/${crypto.randomUUID()}.${ext}`
                ...
                <form onSubmit={handleSubmit} className="p-4 space-y-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-3">
                            {imagePath ? (
                                <img src={`${import.meta.env.VITE_SUPABASE_URL}/storage/v1/object/public/inventory-images/${imagePath}`}
                                    alt="Preview" className="h-16 w-16 rounded-lg object-cover border border-slate-200" />
                            ) : (
