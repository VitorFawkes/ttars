...
import { useInternalInventoryStats } from '@/hooks/useInternalInventoryStats'
...
export default function InternalInventoryDashboard() {
    const { data: stats } = useInternalInventoryStats()
    const { products } = useInternalInventoryProducts()
    ...
    const toggle = (filter: AlertFilter) => setActiveFilter(prev => prev === filter ? null : filter)

    useEffect(() => {
        if (!activeFilter) return
        const handler = (e: MouseEvent) => {
            if (panelRef.current && !panelRef.current.contains(e.target as Node)) setActiveFilter(null)
        }
    ...
    return (
        <div className="space-y-0" ref={panelRef}>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard label="Produtos Ativos" value={stats?.totalProducts ?? 0} icon={Package} color="text-indigo-600 bg-indigo-50" />
                <StatCard label="Valor em Estoque" value={formatBRL(stats?.totalStockValue ?? 0)} icon={DollarSign} color="text-emerald-600 bg-emerald-50" />
                <StatCard label="Estoque Baixo" value={stats?.lowStockCount ?? 0} icon={AlertTriangle} color="text-amber-600 bg-amber-50"
                    active={activeFilter === 'low'} clickable={(stats?.lowStockCount ?? 0) > 0} onClick={() => toggle('low')} ringColor="ring-amber-400" />
                <StatCard label="Sem Estoque" value={stats?.outOfStockCount ?? 0} icon={XCircle} color="text-red-600 bg-red-50"
                    active={activeFilter === 'out'} clickable={(stats?.outOfStockCount ?? 0) > 0} onClick={() => toggle('out')} ringColor="ring-red-400" />
            </div>
            ...
            {activeFilter && visibleProducts.length > 0 && (
                <div className={cn(
                    ...
                    <div className={cn(
                        ...
                        {activeFilter === 'low' ? <AlertTriangle className="h-3.5 w-3.5 text-amber-600" /> : <XCircle className="h-3.5 w-3.5 text-red-600" />}
                        <span className={cn("text-xs font-semibold", activeFilter === 'low' ? 'text-amber-700' : 'text-red-700')}>
                            {activeFilter === 'low'
                    ...
                    <div className="divide-y divide-slate-100">
                        {visibleProducts.map(p => <ProductAlertRow key={p.id} product={p} type={activeFilter} />)}
                    </div>
                </div>
            )}
        </div>
...
function StatCard({ label, value, icon: Icon, color, active, clickable, onClick, ringColor }: {
    label: string; value: string | number; icon: typeof Package; color: string
    active?: boolean; clickable?: boolean; onClick?: () => void; ringColor?: string
}) {
    return (
...
function ProductAlertRow({ product, type }: { product: InternalInventoryProduct; type: 'low' | 'out' }) {
        ...
        <div className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 transition-colors">
            ...
            <div className="text-right flex-shrink-0">
                <span className={cn(
                    ...
                    type === 'out' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
                )}>
