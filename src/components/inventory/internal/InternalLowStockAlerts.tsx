...
import { useLowInternalStockProducts } from '@/hooks/useInternalInventoryStats'
...
export default function InternalLowStockAlerts() {
    const { data: products = [], isLoading } = useLowInternalStockProducts()
    const [reposProduct, setReposProduct] = useState<InternalInventoryProduct | null>(null)
    ...
    return (
        <div className="space-y-3">
                ...
                return (
                    <div key={product.id}
                        ...
                        <div className="flex items-center gap-3">
                            {isOut ? <XCircle className="h-5 w-5 text-red-500 shrink-0" /> : <AlertTriangle className="h-5 w-5 text-amber-500 shrink-0" />}
                            <div>
                        ...
                        <button onClick={() => setReposProduct(product)}
            ...
            {reposProduct && (
                <InternalStockMovementModal product={reposProduct} initialMode="entrada"
                    onClose={() => setReposProduct(null)} />
            )}
        </div>
    )
}
