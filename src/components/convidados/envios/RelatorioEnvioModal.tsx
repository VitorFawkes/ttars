...
import { X, CheckCircle2, AlertTriangle, Phone, Loader2 } from 'lucide-react'
  ...
  const loteQuery = useQuery<LoteRow | null>({
    ...
    queryFn: async () => {
      const { data, error } = await sbAny
        .from('envio_lotes')
        .select('id, status, total, sent, failed, started_at, finished_at, template_slug')
        .eq('id', loteId)
        .maybeSingle()
  ...
  const messagesQuery = useQuery<MessageRow[]>({
    ...
    queryFn: async () => {
      const { data, error } = await sbAny
        ...
        .select('id, contact_id, whatsapp_message_id, has_error, error_message, ack_status, status, created_at, contatos:contact_id(nome, sobrenome, telefone)')
