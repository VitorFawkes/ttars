...
export function ReenviarFalhasModal({
  ...
  // Detecta dinamicamente do template real se há placeholder em URL de botão.
  // Sem isso, mostrar a coluna "Botão" pra um template sem variável de URL
  // confunde o usuário (ele preenche e o valor é ignorado no envio).
  const { data: templates = [] } = useWhatsAppTemplates(phoneNumberId)
  const hasButtonVar = useMemo(() => {
    const tpl = templates.find(t => t.name === templateSlug)
    return tpl ? parseTemplateBody(tpl).buttonUrlParamCount > 0 : false
