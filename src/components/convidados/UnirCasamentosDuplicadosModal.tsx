...
import { X, Combine, Loader2, CheckCircle2, Users, Calendar, AlertTriangle, ChevronRight } from 'lucide-react'
...
export function UnirCasamentosDuplicadosModal({ open, onClose }: Props) {
  ...
  // Grupo selecionado na lista da esquerda + destino escolhido dentro dele.
  const [selectedKey, setSelectedKey] = useState<string | null>(null)
  const [destinoId, setDestinoId] = useState<string | null>(null)

  // Mantém uma seleção válida conforme os grupos mudam (re-detecção após unir).
  const group: DuplicateWeddingGroup | undefined =
  ...
  useEffect(() => {
    ...
    if (group.key !== selectedKey) setSelectedKey(group.key)
    setDestinoId(group.suggestedDestinoId)
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // Após unir, a lista re-detecta sozinha. Seleciona o próximo grupo (se houver).
      setSelectedKey(null)
    } catch {
  ...
  return createPortal(
    ...
    >
      <div
        className="w-full max-w-2xl bg-white border border-slate-200 shadow-2xl rounded-xl flex flex-col max-h-[90vh]"
      ...
      >
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-slate-500">
                {groups.length > 0
                  ? `${groups.length} ${groups.length === 1 ? 'casal duplicado encontrado' : 'casais duplicados encontrados'}`
                  : 'Nenhum casal duplicado encontrado'}
              </p>
            </div>
          </div>
          <button
        ...
        {!group ? (
          ...
          <div className="flex-1 min-h-0 flex">
            {/* Lista de grupos (esquerda) — só aparece quando há mais de um */}
            {groups.length > 1 && (
              <aside className="w-48 shrink-0 border-r border-slate-200 overflow-y-auto bg-slate-50/60">
                <p className="px-3 pt-3 pb-1 text-[10px] font-semibold uppercase tracking-wide text-slate-500">
                  Duplicados
                </p>
                {groups.map(g => {
                  const isSel = g.key === group.key
                  const total = g.weddings.reduce((s, w) => s + w.counts.total, 0)
                  return (
                    <button
                      key={g.key}
                      type="button"
                      ...
                      className={cn(
                        ...
                        isSel ? 'border-indigo-500 bg-white' : 'border-transparent hover:bg-white/70',
                      )}
                    >
                      <p className={cn('text-sm font-medium truncate', isSel ? 'text-indigo-700' : 'text-slate-800')}>
                        {g.displayTitle}
                      </p>
                      <p className="text-[11px] text-slate-500 truncate">
                        {g.weddings.length} cards · {total} convidados
                      </p>
                    </button>
                  )
                })}
            ...
            {/* Detalhe do grupo selecionado (direita) */}
            <div className="flex-1 min-w-0 overflow-y-auto px-5 py-4">
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-2 text-sm text-slate-700 flex-wrap">
                  ...
                  <span className="text-xs text-slate-500">{group.weddings.length} cadastros iguais</span>
                </div>

                <p className="text-xs text-slate-500">
                  Mesmo casal cadastrado mais de uma vez. Escolha qual casamento <strong>fica</strong> como
                  principal — os outros serão arquivados e suas listas de convidados entram nele.
                </p>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[11px] mt-1">
                              ...
                              <span className="inline-flex items-center gap-1 text-slate-500">
                                <Calendar className="w-3 h-3" /> {longDate(w.wedding_date)}
                              </span>
                            </p>
                          </div>
                        </div>
          ...
          <span className="text-[11px] text-slate-500">
            {group && groups.length > 1 && (
              <span className="inline-flex items-center gap-1">
                <ChevronRight className="w-3 h-3" /> {groups.length - 1} {groups.length - 1 === 1 ? 'outro casal' : 'outros casais'} na lista
              </span>
            )}
          </span>
