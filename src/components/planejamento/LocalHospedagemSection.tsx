...
import { MapPin, FileSignature, BedDouble, Plus, Trash2, Check } from 'lucide-react'
...
const SUB = "text-[11px] font-bold uppercase tracking-[0.1em] text-[#A88C57] [font-family:'Nunito',sans-serif]"
...
/**
 * Bloco unificado Local & Hospedagem — resolve a duplicação Hotel × Espaço.
 * Três partes claras: ONDE o casamento acontece (venue + pacote/regras),
 * a RESERVA & contrato (assinatura + sinal + valor) e ONDE os convidados se
 * hospedam (hotel — fonte única dos quartos a bloquear). Geralmente é o mesmo
 * lugar (resort), mas pode ser separado e ter mais de um hotel.
 */
export function LocalHospedagemSection({ wedding }: { wedding: WeddingPlanejamento }) {
  ...
  const usarEspacoComoHotel = () => {
    ...
    saveHotel({
      nome: espaco,
      categoria: null,
      localizacao: readStr(pd, PLANEJ_FIELD.regiao) || null,
      check_in: null,
      check_out: null,
      total_quartos: null,
  ...
  return (
    <section className={CARD}>
      <header className="flex items-center gap-2 mb-1">
        <MapPin className="w-5 h-5 text-[#BD965C]" />
        <h2 className="text-base font-semibold text-slate-900">Local &amp; Hospedagem</h2>
      </header>
      <p className="text-[12px] text-slate-500 mb-4 [font-family:'Roboto',sans-serif]">
        Onde o casamento acontece e onde os convidados se hospedam. Costuma ser o mesmo lugar (resort), mas pode ser separado.
      </p>

      {/* ── ONDE O CASAMENTO ACONTECE ───────────────────────────────── */}
      ...
      {/* ── ONDE OS CONVIDADOS SE HOSPEDAM ───────────────────────────── */}
      <div className="mt-5 pt-4 border-t border-[#F0E9DD]">
        <div className="flex items-center justify-between gap-2 flex-wrap mb-2">
          <p className={cn(SUB, 'flex items-center gap-1.5')}><BedDouble className="w-3.5 h-3.5" /> Onde os convidados se hospedam</p>
          <span className="text-[11px] text-slate-400">o “Total de quartos” é o bloco a reservar</span>
        </div>
        <BoolField
