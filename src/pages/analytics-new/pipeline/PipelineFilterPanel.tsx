import { User as UserIcon, Users as UsersIcon, X } from 'lucide-react'
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-3 flex-wrap bg-white border border-slate-200 rounded-xl px-4 py-2.5 shadow-sm">
        {/* Visão: Time todo / Meu Pipeline — define a ótica antes de qualquer filtro */}
        {profileId && (
          <div className="flex items-center gap-0.5 bg-slate-50 rounded-md p-0.5">
            <button
              type="button"
              onClick={() => { if (isMyPipeline) onToggleMyPipeline() }}
              className={cn(
              ...
              title="Ver o pipeline agregado do time"
            >
        ...
        {/* Pessoa focada (quando gestor clicou em alguém específico que não é ele mesmo) */}
        {selectedOwnerLabel && !isMyPipeline && (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium bg-indigo-50 text-indigo-700 rounded-md">
            <UserIcon className="w-3.5 h-3.5" />
            <span className="max-w-[120px] truncate">{selectedOwnerLabel}</span>
            <button
              onClick={onClearOwner}
              className="ml-0.5 text-indigo-400 hover:text-indigo-700"
              title="Voltar pra visão do time"
            >
              <X className="w-3 h-3" />
            </button>
          </span>
