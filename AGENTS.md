...
- IMPORTANT: Ao criar view/coluna nova usada pelo frontend, adicionar a query ao smoke test em `.Codex/hooks/schema-smoke-test.sh`
...
- Commits em português. Co-author: `Co-Authored-By: Codex <noreply@anthropic.com>`
...
```bash
bash .Codex/hooks/apply-to-staging.sh supabase/migrations/SEU_ARQUIVO.sql
```
...
```bash
bash .Codex/hooks/promote-to-prod.sh supabase/migrations/SEU_ARQUIVO.sql
```
Esse script aplica no banco de produção E roda o smoke test automaticamente.

**Passo 4 — Marcar:**
```bash
touch .Codex/.migration_applied
```
...
| "Aplique no staging" | `bash .Codex/hooks/apply-to-staging.sh <arquivo>` |
| "Promova para produção" / "Suba para produção" | `bash .Codex/hooks/promote-to-prod.sh <arquivo>` para CADA migration pendente |
| "Está tudo ok, pode subir" | Promover todas migrations pendentes + `touch .Codex/.migration_applied` |
...
- O Stop hook BLOQUEIA se detectar `.sql` novo/modificado sem registro no `.Codex/.migration_log`
- `promote-to-prod.sh` registra automaticamente cada arquivo no log
- Após terminar todas migrations, rodar `touch .Codex/.migration_applied` (backward compat)
...
O hook `.Codex/hooks/audit-rls-leaks.sh` roda no Stop e BLOQUEIA se detectar `USING (true)` para authenticated/public em tabela fora da GLOBAL_ALLOWLIST. Se você acabou de adicionar uma tabela genuinamente global, ADICIONE ela na allowlist do script ao mesmo tempo.
...
Ao adicionar uma nova FK `*_id UUID REFERENCES outra_tabela_por_org(id)` numa tabela por-org, escreva o trigger junto na mesma migration. Se a referência é opcional (nullable), o trigger pula quando a coluna é NULL. Adicione também uma query de auditoria em `.Codex/hooks/schema-smoke-test.sh` que conte divergências — se retornar > 0, o smoke test falha e bloqueia promoção.
...
**Regra:** Se a function tem `verify_jwt = false` no `config.toml`, SEMPRE usar `--no-verify-jwt` no deploy. O hook `.Codex/hooks/check-edge-deploy.sh` bloqueia deploys incorretos automaticamente.
